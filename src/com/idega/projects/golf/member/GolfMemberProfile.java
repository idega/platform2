...
public class GolfMemberProfile {
  ...
  private void fillProfile(){
    // UnionMemberInfos
    try {
      eUMIs = (UnionMemberInfo[]) new UnionMemberInfo().findAllByColumn(eMember.getIDColumnName(),eMember.getID());
    ...
    if(eUMI != null && eUMI.getFamilyId() > 0){
      // Family:
      try {
        eFamily = new Family(eUMI.getFamilyId());
        if(eFamily != null)
  ...
  private void saveEntity(GenericEntity entity)throws SQLException{
    if(entity.getID() > 0)
      entity.update();
    else
      entity.insert();
