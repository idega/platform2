...
public class ResultDataHandler {
  ...
  private String getSQLString() {
    StringBuffer sql = new StringBuffer();
      sql.append("select m.member_id,m.first_name,m.middle_name,m.last_name,u.abbrevation,tm.tournament_group_id");
  ...
  private String getMemberSQLString(int memberId) {
    StringBuffer sql = new StringBuffer();
      sql.append("select tournament_group_id,holes,rounds,stroke_count,point_count,hole_par,");
      ...
      sql.append(" and member_id = "+Integer.toString(memberId));
      if ( tournamentRounds_ != null ) {
        for ( int a = 0; a < tournamentRounds_.length; a++ ) {
          if ( a == 0 )
            sql.append(" and ( tournament_round_id = "+Integer.toString(tournamentRounds_[a]));
          if ( a > 0 && a < tournamentRounds_.length -1 )
            sql.append(" or tournament_round_id = "+Integer.toString(tournamentRounds_[a]));
          if ( a == tournamentRounds_.length -1 )
            sql.append(" or tournament_round_id = "+Integer.toString(tournamentRounds_[a])+" ) ");
        }
      }
      sql.append(" order by member_id,tr.round_number,t.hole_number");
