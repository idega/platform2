...
public class MailBusiness {
  ...
  public MailGroup saveLetterGroup(int id, String name, String info, int iCategoryId) {
    try {
      MailGroup group = (MailGroup) MailGroup.getEntityInstance(MailGroup.class);
      ...
      if (update) {
        ...
        group.setCreated(com.idega.util.idegaTimestamp.getTimestampRightNow());
        group.insert();
      }
   ...
   * @todo             Description of the Method
   */
  public MailTopic saveTopic(int id, String name, String info, int iGroupId) {
    try {
      MailTopic topic = (MailTopic) MailTopic.getEntityInstance(MailTopic.class);
      boolean update = false;
      if (id > 0) {
      ...
      topic.setGroupId(iGroupId);
      if (update) {
        topic.update();
      } else {
        MailList list = new MailList();
        list.setCreated(com.idega.util.idegaTimestamp.getTimestampRightNow());
        list.setName(topic.getName() + " list");
        list.setDescription(topic.getDescription());
        list.insert();
        topic.setListId(list.getID());
        topic.setCreated(com.idega.util.idegaTimestamp.getTimestampRightNow());
        topic.insert();
      }
  ...
  public MailAccount saveAccount(int id, String name, String host, String user, String pass, int protocol, int entityId, Class entityClass) {
    try {
      MailAccount account = (MailAccount) MailAccount.getEntityInstance(MailAccount.class);
      boolean update = false;
      if (id > 0) {
  ...
  private void deleteAccount(int id, Class entityClass) {
    try {
      MailAccount a = (MailAccount) MailAccount.getEntityInstance(MailAccount.class, id);
  ...
  public MailLetter saveMailLetter(int id, String fromName, String fromAddress, String subject, String body, int type, int entityId, Class entityClass) {
    try {
      MailLetter letter = (MailLetter) MailLetter.getEntityInstance(MailLetter.class);
      boolean update = false;
      if (id > 0) {
