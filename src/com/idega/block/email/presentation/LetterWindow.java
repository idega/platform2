...
public class LetterWindow extends IWAdminWindow{
  ...
  public PresentationObject getLetterForm(IWContext iwc){
    ...
    if(topics !=null && topics.size() > 0){
      T.add(iwrb.getLocalizedString("list.topi","Topic"),1,row);
    ...
    T.add(iwrb.getLocalizedString("letter.from_name","Sender name"),1,row);
    T.add(fromName,2,row++);
    T.add(iwrb.getLocalizedString("letter.from_address","Sender address"),1,row);
    T.add(fromAddress,2,row++);
    T.add(iwrb.getLocalizedString("letter.subject","Subject"),1,row);
    T.add(subject,2,row++);
    T.add(iwrb.getLocalizedString("letter.body","Body"),1,row);
    T.add(body,2,row++);
