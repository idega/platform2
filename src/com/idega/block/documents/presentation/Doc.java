  ...
  private void getBoxView(InformationFolder folder, InformationCategory[] categories,Table boxTable) {
    ...
    if(categories != null){
      for ( int a = 0; a < categories.length; a++ ) {
        //tmp
        String categoryString = categories[a].getName();//DocBusiness.getLocalizedString(categories[a],_iLocaleID);
  ...
  private void getCategoryView(InformationFolder folder,InformationCategory[] categories,Table boxTable,IWContext iwc) {
    ...
    for ( int a = 0; a < categories.length; a++ ) {
      ...
      Link categoryImageLink = new Link(categoryImage);
        categoryImageLink.addParameter(DocBusiness.PARAMETER_CATEGORY_ID,categories[a].getID());
      Link categoryLink = new Link(categoryText);
        categoryLink.addParameter(DocBusiness.PARAMETER_CATEGORY_ID,categories[a].getID());

      categoryTable.add(categoryImageLink,1,a+1);
      categoryTable.add(categoryLink,2,a+1);
    }

    InformationCategory category = DocFinder.getCategory(_catID);
    ...
    if ( _catID != -1 ) {
      ...
      DocLink[] links = DocFinder.getLinksInFolderCategory(folder,category);
