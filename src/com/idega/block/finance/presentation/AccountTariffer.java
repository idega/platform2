...
public class AccountTariffer extends Finance {
	...
	protected void control(IWContext iwc) throws java.rmi.RemoteException {
		if (isAdmin) {
			...
			T.setCellspacing(0);
			T.add(
					textFormat.format(iwrb.getLocalizedString("account_tariffer", "Account tariffer"),
							textFormat.HEADER), 1, 1);
			T.setHeight(2, "15");
			T.add(getAccountInfo(iwc), 1, 3);
			T.add(getGroupLinks(groups), 1, 4);
			if (group != null)
			...
			myForm.add(new HiddenInput(prmCategoryId, getFinanceCategoryId().toString()));
			if(groupId!=null)
			myForm.add(new HiddenInput(prmGroup, groupId.toString()));
			myForm.add(T);
			add(myForm);
		}
	}
	private void parse(IWContext iwc) throws java.rmi.RemoteException {
		if (iwc.isParameterSet(prmConfirm) && iwc.getParameter(prmConfirm).equals("true")) {
			...
			AssessmentBusiness assBuiz = (AssessmentBusiness) IBOLookup.getServiceInstance(iwc,
	...
	private PresentationObject getAccountInfo(IWContext iwc) throws java.rmi.RemoteException {
		...
		if (account != null) {
			T.add(textFormat.format(iwrb.getLocalizedString("account_number", "Account number"), textFormat.HEADER), 1,
					1);
			if (viewPage > 0) {
			...
			T
					.add(textFormat
							.format(iwrb.getLocalizedString("account_owner", "Account owner"), textFormat.HEADER), 1, 2);
			T.add(textFormat.format(user.getName()), 2, 2);
			T.add(textFormat.format(iwrb.getLocalizedString("account_balance", "Account balance"), textFormat.HEADER),
					1, 3);
