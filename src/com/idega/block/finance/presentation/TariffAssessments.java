...
public class TariffAssessments extends Finance {
	...
	protected void control(IWContext iwc) {
		if (this.isAdmin) {
			...
			Link showAllLink = new Link(
					getText(localize("showall", "Show all")));
	...
	private void initCollections(IWContext iwc) {
		if (this.groupID != null) {
			...
			if (this.roundID == null && this.accountID == null) {
				...
				try {
					int assessmentCount = getFinanceService()
							.getAssessmentRoundHome()
							.getCountByCategoryAndTariffGroup(
									getFinanceCategoryId(), this.groupID,
				...
				try {
					this.assessments = getFinanceService()
							.getAssessmentRoundHome()
							.findByCategoryAndTariffGroup(
									getFinanceCategoryId(), this.groupID,
									this.fromDate, this.toDate, null,
									getCollectionViewSize(),
									getCollectionIndex());
			...
			if (this.roundID != null) {
				try {
					int accountCount = getFinanceService().getAccountHome()
							.countByAssessmentRound(this.roundID);
				...
				if (this.accountID != null) {
					...
					try {
						this.accountEntries = getFinanceService()
								.getAccountEntryHome()
								.findByAccountAndAssessmentRound(
										this.accountID, this.roundID);
					} catch (RemoteException e) {
						e.printStackTrace();
					} catch (FinderException e) {
						e.printStackTrace();
					...
					if (iwc.isParameterSet(this.collectionNavigator
							.getParameterName())) {
						try {
							Collection accs = getFinanceService()
									.getAccountHome().findByAssessmentRound(
											this.roundID, 1,
											getCollectionIndex());
							if (accs != null && !accs.isEmpty()) {
								Account account = (Account) accs.iterator()
										.next();
								this.accountEntries = getFinanceService()
										.getAccountEntryHome()
										.findByAccountAndAssessmentRound(
												(Integer) account
														.getPrimaryKey(),
												this.roundID);
								this.accountID = (Integer) account
										.getPrimaryKey();
							}
						} catch (RemoteException e) {
							e.printStackTrace();
					...
					try {
						if (iwc.isParameterSet(PRM_SHOW_ALL)) {
							this.accounts = getFinanceService()
									.getAccountHome().findByAssessmentRound(
											this.roundID, -1, -1);
						} else {
							this.accounts = getFinanceService()
	...
	private void initGroups(IWContext iwc) {
		...
		try {
			this.groups = getFinanceService().getTariffGroupHome()
		...
		if (this.groupID != null && this.groupID.intValue() > 0) {
			try {
				this.group = getFinanceService().getTariffGroupHome()
		...
		if (this.group != null) {
			...
			if (this.group.getHandlerId() > 0) {
				try {
					this.handler = getFinanceService().getFinanceHandler(
							new Integer(this.group.getHandlerId()));
				} catch (RemoteException e2) {
				}
			}
		}
	...
	private PresentationObject getGroupLinks(IWContext iwc) {
		...
		if (this.groups != null) {
			...
			while (I.hasNext()) {
				...
				tab
						.addParameter(PRM_GROUP_ID, group.getPrimaryKey()
								.toString());
				T.add(tab, col++, 1);
			}
		}
		...
		if (sRoundId != null) {
			...
			try {
				if (this.handler != null) {
					...
					AssessmentBusiness assBuiz = (AssessmentBusiness) com.idega.business.IBOLookup
							.getServiceInstance(iwc, AssessmentBusiness.class);
	...
	private void doAssess(IWContext iwc) throws RemoteException {
		if (iwc.getParameter("pay_date") != null) {
			...
			Integer excessRoundID = Integer.valueOf(iwc
					.getParameter("excess_round_id"));
			if (date != null && date.length() == 10) {
				if (roundName != null && roundName.trim().length() > 1) {
					roundName = roundName == null ? "" : roundName;
					Integer accKeyId = accountKeyId != null ? Integer
							.valueOf(accountKeyId) : null;
					...
					debug("Starting Execution "
							+ IWTimestamp.RightNow().toString());
					this.handler.executeAssessment(iwc, getFinanceCategoryId(),
							this.groupID, roundName, new Integer(1), accKeyId,
							paydate, startdate, enddate, excessRoundID);
	...
	private PresentationObject getTableOfAssessments(IWContext iwc)
		...
		if (this.assessments != null && !this.assessments.isEmpty()) {
			...
			T.add(getHeader(localize("assessment_name", "Assessment name")), 1,
					1);
			...
			T.add(getHeader(localize("publish", "Publish")), 5, 1);
			...
			for (Iterator iter = this.assessments.iterator(); iter.hasNext();) {
				...
				T.add(getText((localize("assessmentstatus." + AR.getStatus(),
						AR.getStatus()))), 4, row);
	...
	private Map mapOfAccountUsers(Integer assessmentRoundId) {
		...
		try {
			users = getFinanceService().getAccountUserHome()
	...
	private PresentationObject getTableOfAssessmentAccounts(IWContext iwc)
		...
		if (this.roundID != null) {
			if (this.accounts != null) {
				T
						.add(
								getHeader(localize("account_name",
										"Account name")), 1, 1);
				...
				float total = 0;
				AccountUserHome uHome = getFinanceService()
						.getAccountUserHome();
				AccountEntryHome eHome = getFinanceService()
						.getAccountEntryHome();
				int count = 0;
				Integer accID;
				for (Iterator iter = this.accounts.iterator(); iter.hasNext();) {
					...
					T.add(getAccountEntryLink(A.getAccountName(), this.roundID,
							accID, count++), 1, row);
					try {
						User user = uHome.findByPrimaryKey(new Integer(A
								.getUserId()));
					...
					try {
						double balance = eHome
								.getTotalSumByAccountAndAssessmentRound(accID,
										this.roundID);
	...
	private Link getAccountEntryLink(String label, Integer roundID,
			Integer accountID, int number) {
		...
		li.addParameter(CollectionNavigator.getParameterName(),
				getCollectionIndex() * getCollectionViewSize() + number);
		return li;
	}
	...
	private Table getAssessmentInfoTable(IWContext iwc) {
		...
		if (this.roundID != null) {
			...
			try {
				assessment = getFinanceService().getAssessmentRoundHome()
						.findByPrimaryKey(this.roundID);
			} catch (RemoteException e) {
				e.printStackTrace();
			} catch (FinderException e) {
				e.printStackTrace();
			...
			if (assessment != null) {
				T.add(
						getHeader(localize("assessment_label",
								"Assessment label")), 1, 1);
				T
						.add(getHeader(localize("assessment_date",
								"Assessment date")), 1, 2);
				T.add(getText(assessment.getName()), 2, 1);
				T.add(getText(getDateTimeFormat(iwc.getCurrentLocale()).format(
						assessment.getRoundStamp())), 2, 2);
				T.add(
						getHeader(localize("assessment_total",
								"Assessment total")), 4, 1);
				try {
					double roundtotal = getFinanceService()
							.getAccountEntryHome()
							.getTotalSumByAssessmentRound(this.roundID);
					T.add(getAmountText(roundtotal), 5, 1);

				} catch (Exception e) {
					T.add(getErrorText(localize("amount_not_available",
							"Amount not available")), 5, 1);
				}
	...
	private Table getAccountInfoTable(IWContext iwc) {
		...
		if (this.roundID != null && this.accountID != null) {
			...
			try {
				account = getFinanceService().getAccountInfoHome()
						.findByPrimaryKey(this.accountID);
				accountOwner = getFinanceService().getAccountUserHome()
						.findByPrimaryKey(new Integer(account.getUserId()));
			...
			accountInfoTable.add(getHeader(localize("account_owner",
					"Account owner")), 1, 1);
			accountInfoTable.add(getHeader(localize("account_owner_pid",
					"Personal Id")), 1, 2);
			...
			accountInfoTable.add(getHeader(localize("account_balance",
					"Balance")), 4, 1);
			accountInfoTable.add(getHeader(localize("account_last_updated",
					"Last updated")), 4, 2);
			...
			if (account != null) {
				accountInfoTable.add(getAmountText((account.getBalance())), 5,
						1);
				accountInfoTable.add(getText(getDateTimeFormat(
						iwc.getCurrentLocale())
						.format(account.getLastUpdated())), 5, 2);
			}
			...
			accountInfoTable.setHorizontalZebraColored(getZebraColor2(),
					getZebraColor2());
	...
	private PresentationObject getTableOfAssessmentAccountEntries(IWContext iwc) {
		...
		if (this.accountEntries != null) {
			...
			for (Iterator iter = this.accountEntries.iterator(); iter.hasNext();) {
				...
				T.add(getText(new IWTimestamp(A.getPaymentDate())
						.getLocaleDate(iwc)), 3, row);
				T.add(getText(new IWTimestamp(A.getLastUpdated())
						.getLocaleDate(iwc)), 4, row);
	...
	private PresentationObject doMainTable(IWContext iwc) {
		...
		if (this.handler != null) {
			try {
				iAccountCount = getFinanceService().getAccountHome()
						.countByTypeAndCategory(this.handler.getAccountType(),
		...
		int row = 1;
		T.add(getHeader(localize("number_of_accounts", "Number of accounts")),
				1, row);
		...
		today.addMonths(1);
		di.setDate(new IWTimestamp(1, today.getMonth(), today.getYear())
				.getSQLDate());
		...
		today1.addMonths(1);
		start.setDate(new IWTimestamp(1, today.getMonth(), today.getYear())
				.getSQLDate());
		...
		end.setDate(new IWTimestamp(day, today2.getMonth(), today2.getYear())
				.getSQLDate());
		TextInput rn = new TextInput("round_name");
		Edit.setStyle(rn);
		SubmitButton sb = new SubmitButton("commit", localize("commit",
				"Commit"));
		Edit.setStyle(sb);
		...
		T
				.add(getHeader(localize("date_of_payment", "Date of payment")),
						1, row);
		T.add(di, 2, row);
		row++;
		...
		try {
			...
			Collection rounds = getFinanceService().getAssessmentRoundHome()
					.findByCategoryAndTariffGroup(getFinanceCategoryId(),
							this.groupID, this.fromDate, this.toDate, null, -1,
							-1);
			excessRounds.addMenuElement(-1, localize("old_batches",
					"Old batches"));
			...
			for (Iterator iter = rounds.iterator(); iter.hasNext();) {
				...
				if (from != null && to != null) {
					period = " ( " + df.format(round.getPeriodFromDate())
							+ " - " + df.format(round.getPeriodToDate()) + ")";
				}
				excessRounds.addMenuElement(round.getPrimaryKey().toString(),
						round.getName() + period);
			}
