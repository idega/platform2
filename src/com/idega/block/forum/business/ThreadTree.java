...
/*	else
  ...
  public ThreadTree(ModuleInfo modinfo)throws Exception{
    ...
    if (modinfo.getParameter("forum_id") != null){
       ForumID = Integer.parseInt(modinfo.getParameter("forum_id"));
    }
  ...
  protected Link closeLinks( ModuleObject LinkImage, int thread_id ){
    ...
    closeLink.addParameter( "state", modinfo.getParameter("state"));
    return closeLink;
  }
  ...
  protected Link openLinks( ModuleObject LinkImage, int thread_id ){
    ...
    openLink.addParameter( "state", modinfo.getParameter("state"));
    return openLink;
  }
...
/*
  ...
  */
  ...
  protected void updateOpenThreads(){
    String text = modinfo.getParameter("FTopen");
    if ( text != null &  !OpenThreads.contains(text))
     OpenThreads.add(text);

    if ( modinfo.getParameter("FTclose") != null ){
      OpenThreads.remove(modinfo.getParameter("FTclose"));
    }
  }
  ...
  protected Table ThreadLink(ForumThread theThread, Image Front, ModuleObject Icon, boolean Open){    //, boolean New){
      ...
      Text Thdate = new Text("<small>" + new idegaTimestamp(theThread.getThreadDate()).toString(true, false, false));
...
/*    if (this.modinfo!= null){
      if (this.modinfo.getParameter("forum_id") != null)
         NewThreadLink.addParameter("forum_id", this.modinfo.getParameter("forum_id"));
    }
*/
...
*/
  ...
  public void updateLastPageLink(){
    if (TreeNum <= 1){
      ...
      lastpage.addParameter("state", modinfo.getParameter("state"));
    }else{
      lastpage.clearParameters();
  ...
  protected void checkSettings(){
    ...
    if(modinfo.getParameter("FTOpenAll") != null)
      TreeOpen = true;

    if(modinfo.getParameter("FTCloseAll") != null){
      TreeOpen = false;
      OpenThreads = new Vector();
    }
    if (modinfo.getParameter("FTreePage") != null){
       TreeNum = Integer.parseInt(modinfo.getParameter("FTreePage"));
       OpenThreads = new Vector();
    }
    ...
    if (modinfo.getParameter("from") != null){
      if (modinfo.getParameter("from").equals("TFLink"))
       OpenThreads = new Vector();
    }
  ...
  public void main(ModuleInfo modinfo) throws SQLException, Exception{
    this.modinfo = modinfo;
    theThreadID = modinfo.getParameter("forum_thread_id");
    checkSettings();
    doTable();
    first = false;
  }
