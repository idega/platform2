...
public class DataTable extends PresentationObjectContainer {
	...
	private void drawTables(IWContext iwc) {
		...
		this.contentTable.setHorizontalZebraColored(this.ZEBRA_LIGHT_COLOR,
				this.ZEBRA_DARK_COLOR);

		if (this.infoTitles) {
			if (this.titlesVertical) {
				this.contentTable.setColumnColor(1, this.HEADING_COLOR);
			} else {
				this.contentTable.setRowColor(this.use_top ? 2 : 1,
						this.HEADING_COLOR);
			}
		}
		...
		if (this.use_top) {
			this.contentTable.mergeCells(1, 1, this.contentTable.getColumns(),
					1);
		...
		if (this.buttons != null) {
			...
			while (this.buttons.size() > 0) {
				this.contentTable.add((PresentationObject) this.buttons
						.remove(0), 1, lastrow);
