package com.idega.jmodule.timesheet.presentation;

import com.idega.jmodule.timesheet.business.*;
...
public class Timesheet extends JModuleObject{
	...
	private String nameOfMonth;
        ...
        private com.idega.data.genericentity.Member member;
	private int member_id;
	private String temp_member_id;
...
//                        entry = (com.idega.jmodule.timesheet.data.TimesheetEntry[]) (new com.idega.jmodule.timesheet.data.TimesheetEntry()).findAllByColumn("timesheet_entry_date",dags+"%");
                        entry = (com.idega.jmodule.timesheet.data.TimesheetEntry[]) (new com.idega.jmodule.timesheet.data.TimesheetEntry()).findAllByColumnOrdered("timesheet_entry_date",dags+"%","member_id",""+member_id,"timesheet_entry_id");
...
//                                                myTable.add(member.getName(),2,current_row);
		  				    ...
		  				    myTable.add(projects,3,current_row);
                                                    ...
                                                    Project userProject = new Project(userDefinedProjectId);
...
//        	myTable.add( new HiddenInput("idega_timesheet_entry_edit","save"),6,current_row);
      ...
      private void undirskyrsla(ModuleInfo modinfo) throws SQLException{
		...
		if ( manudur < 10) {
                ...
                TimesheetEntry[] entry = (TimesheetEntry[])(new TimesheetEntry()).findAll("select * from timesheet_entry where member_id="+member_id+" AND timesheet_entry_date >= '"+dags1+"' AND timesheet_entry_date <= '"+dags2+"' order by project_id,timesheet_entry_date");
			...
			timarverk=0;
              		...
              		currentrow++;
                        ...
                        for (int j=1; j <= dagariman; j++) {
                            ...
                            inside_entry = (TimesheetEntry[]) (new TimesheetEntry()).findAll("select * from timesheet_entry where project_id="+project_id+" AND timesheet_entry_date = '"+today+"' AND member_id = "+this.member_id+"");
	  ...
	  int rows= entry.length;
		...
		myTable.add(other,7,row);
              ...
              for (int i = 0; i < entry.length; i++) {
                    ...
                    if (viewAll) {
                      current_member = entry[i].getMember();
                    }
                      themMember = new Text(current_member.getName());
...
//                Text header = new Text(FunctColl.getNameOfMonth(manudur, modinfo) + " " +ar);
		...
		Table headerTable = this.getHeaderTable();
                        ...
                        Text memberName = new Text("Starfsmannaskýrsla&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + (new com.idega.data.genericentity.Member(this.member_id)).getName());
                            memberName.setFontSize(3);
                            memberName.setBold();
                            memberName.setFontColor(this.header_text_color);
		...
		myTable.add(other,7,row);
              ...
              for (int i = 0; i < entry.length; i++) {
                    project = new Project(entry[i].getProjectId());
                    resource_id = null;
                    resource_name = null;
                    resource_unit_name = null;
                    if (entry[i].getResource() != null) {
      ...
      public void henda(ModuleInfo modinfo) throws Exception{
          ...
          if (entry != null) {
            for (int i = 0 ; i < entry.length ; i++) {
              if ( (entry[i].getMemberId() == member_id_int) || (!entry[i].isBooked()) ) {
				...
				if ( entry_id[telja].equals("null") ){
                                     if (!timar[telja].equals("null")) {
                                        ...
                                        if ((Double.parseDouble(timarString) < 0) && (description[telja].equalsIgnoreCase("")) ){
                                          entry.setDescription("Leiðrétt af : "+member.getName());
                                        }
...
//          Text projectTxt_reusable;
          ...
          double over_time;
            ...
            for (int i = 1; i < dagariman+1; i++) {
                ...
                entry = (TimesheetEntry[])(new TimesheetEntry()).findAll("select * from timesheet_entry where member_id="+this.member_id+" AND timesheet_entry_date = '"+ar+"-"+manudur+"-"+i+"'  order by timesheet_entry_date,project_id");
...
/*
            TimesheetEntry[] entry = (TimesheetEntry[])(new TimesheetEntry()).findAll("select * from timesheet_entry where member_id="+this.member_id+" AND timesheet_entry_date >= '"+dags1+"' AND timesheet_entry_date <= '"+dags2+"' order by timesheet_entry_date,project_id");
//            com.idega.data.genericentity.Member member = new com.idega.data.genericentity.Member();
            com.idega.jmodule.timesheet.data.Project project;
            Days[] days = (Days[])(new Days()).findAllOrdered("days_id");
            double totalHours = 0;
            double totalOvertime = 0;
            double totalDay = 0;
...
*/
      ...
      private void projectReportHour(ModuleInfo modinfo, TimesheetEntry[] entry, String project_id, boolean viewAll) throws SQLException {
		...
		Table headerTable = this.getHeaderTable();
                        ...
                        Text memberName = new Text( (new Project(Integer.parseInt(project_id))).getName());
                            memberName.setFontSize(3);
                            memberName.setBold();
                            memberName.setFontColor(this.header_text_color);
          ...
          String member_name = current_member.getName();
              ...
              for (int i = 0; i < entry.length; i++) {
                  if (entry[i].getResourceId() == 0 ) {
                      ...
                      if (viewAll) {
                          current_member = entry[i].getMember();
                          member_name = current_member.getName();
                      }
                      total_hour = 0;
                      day_hour = 0;
                      over_hour = 0;
		...
		if ( manudur < 10) {
                ...
                if (viewPrevious) {
                    entry = TimesheetEntry.getPreviousEntries(1,manudur,ar,this.member_id);
                }
                else {
                    entry = (TimesheetEntry[])(new TimesheetEntry()).findAll("select * from timesheet_entry where member_id="+this.member_id+" AND timesheet_entry_date >= '"+dags1+"' AND timesheet_entry_date <= '"+dags2+"' AND booked='N' AND registered='N' order by timesheet_entry_date,project_id");
                    arePreviousEntries = TimesheetEntry.arePreviousEntries(1,manudur,ar,this.member_id);
                }
                com.idega.data.genericentity.Member member = new com.idega.data.genericentity.Member(this.member_id);
                Project project;
		...
		if ( manudur < 10) {
                ...
                TimesheetEntry[] entry = (TimesheetEntry[])(new TimesheetEntry()).findAll("select * from timesheet_entry where member_id="+this.member_id+" AND timesheet_entry_date >= '"+dags1+"' AND timesheet_entry_date <= '"+dags2+"' AND booked='Y' AND registered='N' order by timesheet_entry_date,project_id");
                com.idega.data.genericentity.Member member = new com.idega.data.genericentity.Member(this.member_id);
                Project project;
...
*/
    ...
    private void moveProjects(ModuleInfo modinfo) throws Exception{
            ...
            try {
                ...
                if (where_to.equals("<=")) {
                    for (int i = 0; i < project_id.length; i++) {
                        project = new Project(Integer.parseInt(project_id[i]));
                        member.addTo(project);
                    }
                }
                else if (where_to.equals("=>")) {
                    for (int i = 0; i < project_id.length; i++) {
                        project = new Project(Integer.parseInt(project_id[i]));
                        member.reverseRemoveFrom(project);
                    }
                }
            }
            catch (Exception e) {
        ...
        private void myProjects(ModuleInfo modinfo) throws SQLException{
            ...
            if (usedProjects.length > 0 ) {
                ...
                for (int i = 0 ; i < usedProjects.length ; i++ ) {
                    ...
                    for (int j = 0 ; j < projects_left.size() ; j++) {
                        if (( (Project) projects_left.elementAt(j)).equals(usedProjects[i])){
            ...
            if (projects_left.size() > 0 ) {
                ...
                for (int i = 0; i < projects_left.size(); i++) {
                    project = (Project) projects_left.elementAt(i);
                    project_number = project.getProjectNumber();
                    project_name = project.getName();
        ...
        private void setLink(Link link,String edit, int year_adjustment,int month_adjustment, int day_adjustment) {
                ...
                link.addParameter("i_timesheet_member_id",this.member_id);
        }
