...
public class ForumDBWriter {
  ...
  public int delThread(IWContext iwc)throws SQLException{
    ...
    ForumThread toDel = new ForumThread(Integer.parseInt(iwc.getRequest().getParameter("thread_id")));
  ...
  public int saveThread(IWContext iwc, String UserName, int UserID)throws Exception{
    ...
    int user_id;
    String emailAddress;
    String subject = iwc.getRequest().getParameter("thread_subject");
    String body = iwc.getRequest().getParameter("thread_body");

    if (iwc.getRequest().getParameter("parent_id") != null){
      parent_id = Integer.parseInt(iwc.getRequest().getParameter("parent_id"));
    } else {
      parent_id = -1;
    }

    if (iwc.getRequest().getParameter("forum_id") != null){
      forum_id = Integer.parseInt(iwc.getRequest().getParameter("forum_id"));
    } else {
      forum_id = 1;
    }

    if (iwc.getRequest().getParameter("thread_author") != null){
      Author = iwc.getRequest().getParameter("thread_author");
      user_id = -1;
    } else {
      Author = UserName;
      user_id = UserID;
    ...
    if (iwc.getRequest().getParameter("forum_email") != null){
      emailAddress = iwc.getRequest().getParameter("forum_email");
    } else {
      emailAddress = null;
    }
...
//    return service.saveThread(parent_id, forum_id, TextSoap.findAndRemoveHtmlTags(subject), TextSoap.formatString(body), Author, user_id, idegaTimestamp.RightNow());
      newThreadID =  service.saveThread(parent_id, forum_id, subject, body, Author, user_id, idegaTimestamp.RightNow());
