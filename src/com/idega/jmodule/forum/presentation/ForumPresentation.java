...
*/
  ...
  public void initTree(IWContext iwc) throws Exception{
    if((AttributeName != null) && (iwc.getRequest().getParameter("forum_id") == null)){
  ...
  protected void initializeForm(){
    ...
    if (iwc.getRequest().getParameter("parent_id") != null)
      entryForm.add(new HiddenInput("parent_id",iwc.getRequest().getParameter("parent_id")));
    if (iwc.getRequest().getParameter("forum_id") != null)
      entryForm.add(new HiddenInput("forum_id",iwc.getRequest().getParameter("forum_id")));
  ...
  protected void addProperState() throws Exception{
    ...
    if (iwc.getRequest().getParameter("state") == null){
     ...
     if (iwc.getRequest().getParameter("state").equals("same") && !(currentState == 2 || currentState == 3)){
       currentState = 2;
     }
  ...
  protected void State3() throws Exception{
    ...
    String id = iwc.getRequest().getParameter("forum_thread_id");
    if (id != null){
  ...
  protected void State5() throws Exception{
    ...
    if(!isUser && UseLogin){
      State2();   //  temp  - throw error - benda á að skrá sig inn
        ...
        String parent = iwc.getRequest().getParameter("parent_id");
        if(iwc.getRequest().getParameter("from").equals("ATLink") && (parent != null)){
  ...
  protected synchronized void State6() throws Exception{
    currentState = 6;
    String from = iwc.getRequest().getParameter("from");
    if (from == null)
      from = "";
  ...
  private void updateCurrentForumName()throws SQLException{
    if(iwc != null){
      String tempForumID = iwc.getRequest().getParameter("forum_id");
...
*/
  ...
  private void findeCurrentForumName()throws SQLException{
    String tempID = iwc.getRequest().getParameter("forum_id");
    if (tempID != null)
