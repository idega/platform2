...
public BoxReader(String issue_id, int numberOfColumns){
		...
		for ( int a = 0; a < categories.length; a++ ) {
			...
			if ( showCategoryHeadline ) {
				...
				Table headlineTable = new Table(3,1);
					...
					headlineTable.setAlignment(2,1,headlineAlign);
	...
	private Table getSubject(String issue_category_id) throws IOException,SQLException {
		...
		if ( issueName.toLowerCase().equals("fréttir") || issueName.toLowerCase().equals("news") ) {
			...
			News[] news = (News[]) (new News()).findAllByColumnOrdered("news_category_id",String.valueOf(Integer.parseInt(issue_id)+2),"news_date desc");

				more.setURL("/news/newsreader.jsp");
				more.addParameter("category_id",String.valueOf(Integer.parseInt(issue_id)+2));
			for ( int a = 0; a < news.length; a++ ) {
				String headline = news[a].getHeadline();
				if ( isAdmin ) {
					if ( headline.length() > numberOfLettersAdmin ) {
						headline = headline.substring(0,numberOfLettersAdmin)+"...";
					}
				}
				else {
					if ( headline.length() > numberOfLetters ) {
						headline = headline.substring(0,numberOfLetters)+"...";
					}
				}

				Text news_text = new Text(headline);
					news_text.setFontSize("1");

				Link news_link = new Link(news_text,"/news/newsreader.jsp");
					news_link.addParameter("news_id",String.valueOf(news[a].getID()));
...
// spurning með virkni með multiLanguage hér
		if ( issueName.toLowerCase().equals("verkefni") || issueName.toLowerCase().equals("projects") ) {

			Project[] project = (Project[]) (new Project()).findAllByColumnOrdered("issue_id",issue_id,"valid","Y","project_id desc");

			more.setURL("/verkefni/index.jsp");

			for ( int a = 0; a < project.length; a++ ) {
				String headline = project[a].getName();
				if ( isAdmin ) {
					if ( headline.length() > numberOfLettersAdmin ) {
						headline = headline.substring(0,numberOfLettersAdmin)+"...";
					}
				...
				Text project_text = new Text(headline);
					project_text.setFontSize("1");

				Link project_link = new Link(project_text,"/verkefni/index.jsp");
					project_link.addParameter("project_id",String.valueOf(project[a].getID()));
			...
			if ( isAdmin == true ) {
				...
				Link adminLink = new Link(newImage,"/verkefnistemplate/index.jsp");
					adminLink.addParameter("action","Nýtt verk");
