...
//		}
	...
	public void saveProjectFinances() throws SQLException{
		String project_id = (String) modinfo.getSession().getAttribute("project_id");
		if (project_id != null) {
			String finances = getRequest().getParameter("project_goals");
			if (finances != null) {
				Project project = new Project(Integer.parseInt(project_id));
				ProjectExtra[] extra = (ProjectExtra[]) project.findRelated(new ProjectExtra());
				if (extra.length > 0 ) {
...
//		}
	...
	public void saveProjectTasks() throws SQLException{
		String project_id = (String) modinfo.getSession().getAttribute("project_id");
		if (project_id != null) {
			String tasks = getRequest().getParameter("project_tasks");
			if (tasks != null) {
				Project project = new Project(Integer.parseInt(project_id));
				ProjectExtra[] extra = (ProjectExtra[]) project.findRelated(new ProjectExtra());
				if (extra.length > 0 ) {
...
//		}
	...
	public void saveProjectGoals() throws SQLException{
		String project_id = (String) modinfo.getSession().getAttribute("project_id");
		if (project_id != null) {
			String goals = getRequest().getParameter("project_goals");
			if (goals != null) {
				Project project = new Project(Integer.parseInt(project_id));
				ProjectExtra[] extra = (ProjectExtra[]) project.findRelated(new ProjectExtra());
				if (extra.length > 0 ) {
...
//		}
	...
	public void saveProjectDescription() throws SQLException{
		String project_id = (String) modinfo.getSession().getAttribute("project_id");
		if (project_id != null) {
			String description = getRequest().getParameter("project_description");
			if (description != null) {
				Project project = new Project(Integer.parseInt(project_id));
				ProjectExtra[] extra = (ProjectExtra[]) project.findRelated(new ProjectExtra());
				if (extra.length > 0 ) {
...
//	projectDescription();
	...
	public void viewGroup() throws SQLException {
		String[] groups = getRequest().getParameterValues("group_");
		if (groups != null) {
			ProjectGroup group;
			Member[] member;
			SelectionBox listi;
			...
			for (int i=0 ; i < groups.length ; i++) {
				...
				member = (Member[])group.findReverseRelated(new Member());
				listi = new SelectionBox(member);
					listi.setHeight(10);
...
//                myForm.setMethod("get");
	...
	//		myTable.setBorder(1);
		...
		SelectionBox members = new SelectionBox((new Member()).findAllOrdered("first_name"));
	...
	public void saveGroup()throws SQLException {
		String[] members = getRequest().getParameterValues("member");
		String group_name = getRequest().getParameter("group_name");
		ProjectGroup group = new ProjectGroup();
			group.setName(group_name);
			group.insert();

		for (int i = 0 ; i < members.length ; i++ ) {
			Member member = new Member(Integer.parseInt(members[i]));
			member.addTo(group);
		}
        ...
        public void deleteGroup() throws SQLException {
              String[] group_id = (String[]) getRequest().getParameterValues("group_");
              ProjectGroup group;
                  ...
                  try {
                      if (group_id.length > 0) {
                          for (int i = 0 ; i < group_id.length ; i++) {
                              ...
                              member.reverseRemoveFrom(group);
                              group.delete();
                          }
                      }
                      groups();
	...
	public void save() throws SQLException {
		String save_action = getRequest().getParameter("save_action");
		if (save_action != null) {
	...
	public void groupChosen(String what)throws SQLException {
		String[] group = getRequest().getParameterValues("group_");
		String[] members = getRequest().getParameterValues("member");
			...
			if ((group != null) && (members == null) ){
				if (group.length == 1) {
				    ...
				    grouppe.insert();
                                  ...
                                  String sql_query = "Select * from member where";
                                  boolean altered = false;

                                  if (members != null)
                                  if (members.length > 0) {
                                    for (int i = 0 ; i < members.length ; i++) {
                                        member_temp = new Member(Integer.parseInt(members[i]));
                                        altered = true;
                                        sql_query = sql_query + " member_id = "+member_temp.getID();
                                  ...
                                  if (group.length > 0) {
                                      for (int i = 0 ; i < group.length ; i++ ) {
                                          ...
                                          members_temp = (Member[]) groupa.findReverseRelated(new Member());
                                                ...
                                                for (int j = 0 ; j < members_temp.length ; j++) {
                                                  if (altered) {
                                                   ...
                                                   altered = true;
                                                    ...
                                                    sql_query = sql_query + " member_id = "+members_temp[j].getID();
                                                }
                                            }
                                          }
                              ...
                              sql_query = sql_query + " order by first_name";
                              Member[] memberarnir = (Member[]) (new Member()).findAll(sql_query);
                              if (memberarnir != null) {
				...
				newProject2();
					...
					for (int i = 0 ; i < members.length ; i++) {
						member = new Member(Integer.parseInt(members[i]));
						member.addTo(grouppe);
					}
	...
	public void groups() throws SQLException{
		String actionInn = getRequest().getParameter("action");
		String project_id = (String) modinfo.getRequest().getAttribute("project_id");
		String veljaAction = "";
                String member_id = getRequest().getParameter("member_id");
                String group_id = getRequest().getParameter("group_id");
		...
		SelectionBox members = new SelectionBox((new Member()).findAllOrdered("first_name"));
			members.setHeight(10);
                ...
                if (group_id != null) {
                    ...
                    Member[] memm = (Member[])goat.findReverseRelated(new Member());
                    if (memm != null) {
                      if (memm.length > 0) {
...
//			myTable.setBorder(1);
	...
	public void newProject2() throws SQLException {
		...
		String issue_id;
		String importance;
		Issues iss;// = new Issues(Integer.parseInt(issue_id));
		boolean newProject = true;

		if (project_id == null) {
			issue_id = getRequest().getParameter("Issues");
                        // uses tablename as parametername
			importance = getRequest().getParameter("i_pm_importance");
			project = new Project();
//			add(Integer.parseInt(issue_id)+"");
			...
			project.insert();
			modinfo.getRequest().setAttribute("project_id",Integer.toString(project.getID()));
			...
			if (project.getWheelGroupId() != -1) {
				...
				if (group != null) {
					Member[] member = (Member[])group.findReverseRelated(new Member());
                                        select1 = new SelectionBox(member);
                                        select1.addMenuElement(-1,empty_string);
                                          select1.setHeight(select_size);
...
*/
					row +=1;
				myTable.add(new Link("Breyta",URI+"?action=plusStyri&group_id="+group.getID()+"&project_id="+project.getID()+""),1,row);
				}
			}
			else {
					row +=1;
				myTable.add(new Link("Bæta við",URI+"?action=plusStyri&project_id="+project.getID()+""),1,row);
			}
		}
		else {
					row +=1;
		...
		if (!(newProject)) {
			...
			ProjectGroup group = new ProjectGroup(project.getProjectGroupId());
				Member[] member = (Member[])group.findReverseRelated(new Member());
                                select2 = new SelectionBox(member);
                                          select2.setHeight(select_size);
                                          select2.addMenuElement(-1,empty_string);
/*				for (int i = 0 ; i < member.length ; i++ ) {
					myTable.add(member[i].getName(),2,row);
...
*/
			myTable.add(new Link("Breyta",URI+"?action=plusVerkhop&group_id="+group.getID()+"&project_id="+project.getID()+""),3,row);
				row +=1;
			}
			else {
			myTable.add(new Link("Bæta við",URI+"?action=plusVerkhop&project_id="+project.getID()+""),3,row);
				row +=1;
			}
		}
		else {
		...
		if (!(newProject)) {
			if (project.getProjectManagerId() != -1) {
				Member member = new Member(project.getProjectManagerId());
                                if (member != null) {
                                        select4 = new SelectionBox(member.getName());
                                          select4.addMenuElement(member.getID(),member.getName());
                                          select4.setHeight(1);
//					myTable.add(member.getName(),2,row);
        				++row;
                                        theLink = new Link("Breyta",URI+"?action=plusVerkMan&member_id="+member.getID()+"&project_id="+project.getID()+"");
...
//			myTable.add(new Link("Bæta við",URI+"?action=plusVerkMan&project_id="+project.getID()+""),1,row);
              ...
              if (!project.isProjectManagerIdFinal()) {
                ...
                myTable.addBreak(1,row-1);
                myTable.add("Málaflokkur : "+iss.getName(),1,row-1);
                myTable.addBreak(1,row-1);
                myTable.add("Mikilvægi : "+new Importance(project.getImportanceId()).getName(),1,row-1);
		...
		if (!(newProject)) {
			if (project.getGroupId() != -1) {
				...
				if (group != null) {
					Member[] member = (Member[])group.findReverseRelated(new Member());
                                        select3 = new SelectionBox(member);
                                          select3.setHeight(select_size);
                                          select3.addMenuElement(-1,empty_string);
/*
					for (int i = 0 ; i < member.length ; i++ ) {
...
*/
				myTable.add(new Link("Breyta",URI+"?action=plusGroup&group_id="+group.getID()+"&project_id="+project.getID()+""),3,row);
					row +=1;
				}
			}
			else {
				myTable.add(new Link("Bæta við",URI+"?action=plusGroup&project_id="+project.getID()+""),3,row);
					row +=1;
			}
		}
		else {
...
//			myTable.setBorder(1);
		...
		myTable.add("Stýrishópur",1,row);
			...
			if (wheelGroup != null) {
				Member[] member = (Member[])wheelGroup.findReverseRelated(new Member());
				for (int i = 0 ; i < member.length ; i++ ) {
					myTable.add(member[i].getName(),2,row);
					++row;
				}
		...
		myTable.add("Verkefnishópur",1,row);
			...
			ProjectGroup projectGroup = new ProjectGroup(project.getProjectGroupId());
				Member[] member = (Member[])projectGroup.findReverseRelated(new Member());
				for (int i = 0 ; i < member.length ; i++ ) {
					myTable.add(member[i].getName(),2,row);
					++row;
				}
		...
		myTable.add("Verkefnisstóri",1,row);
			...
			if (project.getGroupId() != -1) {
				...
				if (projectManager != null) {
					Member[] member = (Member[])projectManager.findReverseRelated(new Member());
					for (int i = 0 ; i < member.length ; i++ ) {
						myTable.add(member[i].getName(),2,row);
						++row;
					}
...
//		}
	...
	public void newProject3() throws SQLException {
		...
		Project project = new Project(Integer.parseInt(project_id));
                ...
                String wheel_final = getRequest().getParameter("wheel_group");
                String project_final = getRequest().getParameter("project_group");
                String manager_final = getRequest().getParameter("project_manager");
                String other_final = getRequest().getParameter("other_group");
...
//			myTable.setRowColor(4,header_color);
        ...
        public void deleteMembers() throws SQLException {
            String[] member_id = getRequest().getParameterValues("member");
            String veljaAction = getRequest().getParameter("save_action2");
            String group_id = getRequest().getParameter("group_id");
            String member__id = getRequest().getParameter("member_id");

            if (member_id != null)
            for (int p = 0 ; p < member_id.length ; p++ ) {
		Member member= new Member(Integer.parseInt(member_id[p]));
		EmploymentMemberInfo[] extra = (EmploymentMemberInfo[]) member.findReverseRelated(new EmploymentMemberInfo());
		Address[] address= (com.idega.data.genericentity.Address[]) member.findReverseRelated(new Address());
		Phone[] phone= (Phone[]) member.findReverseRelated(new Phone());
		...
		Division[] div  = (Division[]) member.findReverseRelated(new Division());
                ProjectGroup[] group = (ProjectGroup[]) member.findRelated(new ProjectGroup());
...
//			member.removeFrom(extra[i]);
			extra[i].removeFrom(member);
			extra[i].delete();
		}

		if (address != null)
		for (int i = 0 ; i < address.length ; i++) {
			address[i].removeFrom(member);
			address[i].delete();
		}

		if (phone != null)
		for (int i = 0 ; i < phone.length ; i++) {
			phone[i].removeFrom(member);
			phone[i].delete();
		}

		if (div != null)
		for (int i = 0 ; i < div.length ; i++ ) {
			div[i].reverseRemoveFrom(member);
		}

		if (group != null)
		for (int i = 0 ; i < group.length ; i++ ) {
			group[i].removeFrom(member);
		}

		if (logType != null)
		for (int i = 0 ; i < logType.length ; i++ ) {
			logType[i].removeFrom(member);
		}
		...
		add("Starfsmönnum hefur verið eytt<br>");
            ...
            Link linki = new Link(new com.idega.jmodule.object.Image("/pics/bakka.gif","Til baka"),getRequest().getRequestURI());
...
//           if (kalli != null) {
                if (modinfo.getRequest().getParameter("Breyta.y") != null) {
                  action = "Breyta";
                }
                if (modinfo.getRequest().getParameter("Henda.y") != null) {
                  action = "Henda";
                }
                if (modinfo.getRequest().getParameter("delete.x") != null){
		...
		if (action == null) {
			...
			else if  (modinfo.getRequest().getParameter("project_id") != null ) {
					modinfo.getSession().setAttribute("project_id",getRequest().getParameter("project_id"));
			}
			newProject2();
		}
...
//			viewProject();
                      ...
                      String project_id_temp = getRequest().getParameter("project");
			...
			if (getRequest().getParameter("takki").equals("Áfram")) {
				newProject3();
			}
		}
			...
			viewGroup();
			 ...
			 if  (modinfo.getRequest().getParameter("project_id") != null ) {
					modinfo.getSession().setAttribute("project_id",getRequest().getParameter("project_id"));
			}
			projectDescription();
		}
