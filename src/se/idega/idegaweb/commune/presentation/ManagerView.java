...
public class ManagerView extends CommuneBlock {
	...
	private void viewManagerInfo(IWContext iwc) throws Exception {
		...
		if (managerSelected) {
			...
			Collection schUsers = getSchoolUserBusiness(iwc)
					.getSchoolUserHome().findByUser(manager);
			SchoolBusiness sb = (SchoolBusiness) IBOLookup.getServiceInstance(
					iwc, SchoolBusiness.class);
			School school = null;
			String schName = null;
			...
			if (userImageID == -1) {
				...
				fakeImageTable
						.setAlignment(1, 1, Table.HORIZONTAL_ALIGN_CENTER);
				fakeImageTable.setBorder(4);
				fakeImageTable.setHeight(140);
				fakeImageTable.setWidth(100);
				String fakeImageColor = "#CCCCCC";
				Text photoText = new Text(iwrb.getLocalizedString(
						"managerview.photo_text", "Photo"));
				photoText.setBold();
			...
			Name name = new Name(manager.getFirstName(), manager
					.getMiddleName(), manager.getLastName());
			// String sManagerName =
			// name.getName(iwc.getApplicationSettings().getDefaultLocale(),
			// true);
			...
			Text tManagerTelephone = getSmallText(localize("managerview.tel",
					"Tel")
					+ ": " + sManagerTelephone);
			leftTable.add(tManagerTelephone, 1, 8);

		} else {
			add(getSmallText(localize("managerview.no_manager_selected",
					"No manager selected")));
		}
	...
	private Group getWorkGroup(User manager) {
		try {
			...
			throw new RuntimeException("No workgroup found for manager "
					+ manager);
		}
	}
	...
	/*
	 * Commented out since it is never used... private GroupBusiness
	 * getGroupBusiness(IWContext iwc) throws Exception { return (GroupBusiness)
	 * com.idega.business.IBOLookup.getServiceInstance(iwc,
	 * GroupBusiness.class); }
	 */

	protected SchoolUserBusiness getSchoolUserBusiness(IWContext iwc)
			throws RemoteException {
		return (SchoolUserBusiness) IBOLookup.getServiceInstance(iwc,
	...
	public void setManager(User manager) {
		try {
			...
			System.out.println("[" + this.getClass().getName()
					+ "] Exception getting primary key from user: "
					+ e.getMessage());
		}
	}
