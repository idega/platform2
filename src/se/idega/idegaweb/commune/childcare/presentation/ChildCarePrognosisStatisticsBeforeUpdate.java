 ...
 * @author laddi
 */
public class ChildCarePrognosisStatisticsBeforeUpdate extends ChildCareBlock {
     ...
     * 
     */
    public ChildCarePrognosisStatisticsBeforeUpdate() {
	...
	private Table getProviderStatTable(IWContext iwc) throws RemoteException{
        ...
        try {
            stats = getBusiness().getProviderStatsBeforeUpdate(iwc.getCurrentLocale());
        ...
        if (stats != null && !stats.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				int numerInQueueBeforeUpdate = 0;
				...
				numerInQueueBeforeUpdate = getBusiness().getQueueTotalBeforeUpdate(stat.getProviderID().intValue());
				table.add(getSmallText(String.valueOf(numerInQueueBeforeUpdate)), column++, row);
