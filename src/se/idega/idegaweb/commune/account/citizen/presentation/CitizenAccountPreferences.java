...
public class CitizenAccountPreferences extends CommuneBlock {
	...
	private void drawForm(IWContext iwc) throws RemoteException {
		...
		PasswordInput tiNewPasswordRepeated = (PasswordInput) getStyledInterface(new PasswordInput(
		...
		SubmitButton sbUpdate = (SubmitButton) getButton(new SubmitButton(localize(KEY_UPDATE, DEFAULT_UPDATE),
				PARAMETER_FORM_SUBMIT, "true"));
		row++;
	...
	private void updatePreferences(IWContext iwc) throws Exception {
		...
		if (errorMessage != null) {
			...
			if (updateEmail) {
				// ub.updateUserMail(((Integer)user.getPrimaryKey()).intValue(),
				// sEmail);
	...
	private Address getCOAddress(IWContext iwc) {
		...
		try {
			...
			/*
			 * Collection addresses = user.getAddresses(); Iterator iter =
			 * addresses.iterator(); while (iter.hasNext()) { Address address =
			 * (Address) iter.next(); int typeID = address.getAddressTypeID(); if
			 * (typeID == ((Integer)coType.getPrimaryKey()).intValue()) { return
			 * address; } }
			 */
	...
	/**
	 * @param requirePasswordVerification
	 *          The requirePasswordVerification to set.
	 */
	...
	public void setShowPasswordChange(boolean showPassword) {
		iShowPassword = showPassword;
	}
	...
	public void setNoVerificationForBankLogins(boolean noVerificationForBankLogins) {
