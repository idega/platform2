 ...
 *  @version 1.0
 * 
 *  Scale dimension of the logo and the position of it can be set with 
 *  bundle properties in the commune bundle.
 *  (Use BundlePropertySetter)
 * 
 *  The position of the address box can also be set with bundle properties.
 *
 *  The properties concerning these settings are all prefixed with 
 *  "printing.".
 *
 *  To activate the bundle property changes you need to set the bundle 
 *  property "printing.dimension_refresh" to "true";
 *
 *  Every measure is in millimeters(mm).
 *  X coordinates are measured from the left side of the page(paper).
 *   y coordinates are measured from bottom of the page
 * 
 *  The address box is specified with its lower left corner coordinates
 *  and upper right corner coordinates
 */
public class DocumentBusinessBean
	extends com.idega.business.IBOServiceBean
	implements DocumentBusiness {
	...
	private Font getDefaultTagFont() {
		if (defaultTagFont == null) {
			defaultTagFont = new Font(Font.HELVETICA);
			defaultTagFont.setSize(12);
		}
	...
	private Font getDefaultTitleFont() {
		if (defaultTitleFont == null) {
			defaultTitleFont = new Font(Font.HELVETICA);
			defaultTitleFont.setSize(12);
		}
	...
	/* Commented out since it is never used...
	private Font getTagFont(PrintedLetterMessage msg) {
		return getDefaultTagFont();
	}*/
	
	/* Commented out since it is never used...
	private Font getParagraphFont(PrintedLetterMessage msg) {
		return getDefaultParagraphFont();
	}*/

	/* Commented out since it is never used...
	private Font getTitleFont(PrintedLetterMessage msg) {
		return getDefaultTitleFont();
	}*/
	...
	public  String getAddressString(User user) {
		...
		try {
			...
			if(addr == null)
				addr = getUserBusiness().getUsersMainAddress(user);
			if(addr == null)
	...
	public Collection getPrintedDocuments()
		throws FinderException {
		return getPrintDocumentsHome().findAllPrintedLetterDocuments();
	}

	public Collection getPrintedDocuments(String type)
		throws FinderException {
		return getPrintDocumentsHome().findAllDocumentByType(type);
	}
	
	public Collection getPrintedDocuments(String type,IWTimestamp from,IWTimestamp to)
		throws FinderException {
		return getPrintDocumentsHome().findAllDocumentByType(type,from,to);
	}
	
	public Collection getPrintedMessages(String type)
		throws FinderException {
		if(isTypeSystemArchiveMessage(type))
			return getSystemArchivationMessageHome().findPrintedMessages();
	...
	public Collection getPrintedMessagesByPrimaryKeys(String[] primaryKeys,String type)
			...
			if(isTypeSystemArchiveMessage(type))
				msgHome =  getSystemArchivationMessageHome();
	...
	public Collection getPrintedMessages(String type,IWTimestamp from,IWTimestamp to)
		throws FinderException {
		if(isTypeSystemArchiveMessage(type))
			return getSystemArchivationMessageHome().findPrintedMessages(from,to);
		return getPrintedLetterMessageHome().findPrintedLettersByType(type,from,to);
	}
	
	public Collection getUnPrintedMessages(String type)
		throws FinderException {
		if(isTypeSystemArchiveMessage(type))
			return getSystemArchivationMessageHome().findUnPrintedMessages();
		return getPrintedLetterMessageHome().findUnPrintedLettersByType(type);
	}
	
	public Collection getUnPrintedMessages(String type,IWTimestamp from,IWTimestamp to)
		throws FinderException {
		if(isTypeSystemArchiveMessage(type))
			return getSystemArchivationMessageHome().findUnPrintedMessages(from,to);
	...
	public int getUnPrintedDefaultLettersCount() {
		return getPrintedLetterMessageHome()
			.getNumberOfUnPrintedDefaultLetters();
	}

	public int getUnPrintedPasswordLettersCount() {
		return getPrintedLetterMessageHome()
			.getNumberOfUnPrintedPasswordLetters();
	}

	public int getPrintedLettersCountByStatusAndType(
		String caseStatus,
		String type) {
		return getPrintedLetterMessageHome().getNumberOfLettersByStatusAndType(
			caseStatus,
			type);
	}
	...
	public int getUnprintedMessagesCountByType(String type) {
		if(isTypeSystemArchiveMessage(type))
			return getSystemArchivationMessageHome().getNumberOfUnPrintedMessages();
	...
	public int getPrintedLettersCountByType(String type) {
		return getPrintedLetterMessageHome().getNumberOfPrintedLettersByType(
			type);
	}
	...
	public boolean isBulkLetterType(String type) {
		if (type.equals(PrintedLetterMessageBMPBean.LETTER_TYPE_PASSWORD))
			return true;
		if(type.equals(SystemArchivationMessageBMPBean.PRINT_TYPE))
			return true;
	...
	public int[] getUnPrintedLettersIDs(String type) {
		try {
			Collection coll =
				getPrintedLetterMessageHome().findUnPrintedLettersByType(type);
			if (coll != null && coll.size() > 0) {
				int[] theReturn = new int[coll.size()];
				Iterator iter = coll.iterator();
				int counter = 0;
				while (iter.hasNext()) {
					PrintedLetterMessage pMessage =
						(PrintedLetterMessage) iter.next();
					theReturn[counter++] =
						((Integer) pMessage.getPrimaryKey()).intValue();
				}
				return theReturn;
			} else {
	...
	public SystemArchivationMessageHome getSystemArchivationMessageHome() {
		try {
			return (SystemArchivationMessageHome) getIDOHome(
				SystemArchivationMessage.class);
		} catch (Exception e) {
			throw new IBORuntimeException(e);
		}
	}
		...
		try {
			...
			if(registerBulkData){
				...
				while (iter.hasNext()) {
					...
					//contentReturn = createContent(outerDocument, msg, performer,writer,locale);
					//contentReturn = createContent(dpc);
					createHandlerContent(dpc);
					//System.err.println("letter content returns : "+contentReturn+" for msg: "+msg.getPrimaryKey().toString());
					if(contentReturn!=ADDRESS_ERROR){
						outerDocument.newPage();
						try {
							//System.err.println("bulk id = "+bulkFile.getPrimaryKey().toString());
	...
	public int writePDF(PrintMessage msg,User performer,String fileName,Locale locale,boolean flagPrinted)
		...
		//int contentReturn = createContent(document, msg, performer,writer,locale);
	...
	//private int createContent(Document doc,PrintMessage msg,User performer,PdfWriter writer,Locale locale)throws Exception {
	private int createContent(DocumentPrintContext dpc)throws ContentCreationException{
		...
		if(msg instanceof PrintedLetterMessage){
			...
			else if(pmsg.getLetterType().equals(PrintedLetterMessageBMPBean.LETTER_TYPE_DEFAULT))
	...
	private void handleNoAddressUser(){
		/*
		try{
			String status = getMessageBusiness().getCaseStatusError().getStatus();
	...
	public void createAddressContent(String addressString, PdfWriter writer) throws DocumentException {
		...
		*/
		...
		//ct.setSimpleColumn(Ph0,getPointsFromMM((20f+95f)), 655f, getPointsFromMM((193f)), getPointsFromMM(257f), 15, Element.ALIGN_LEFT);
		//ct.setSimpleColumn(Ph0,getPointsFromMM(new Float(addressLowerLeftX).floatValue()), new Float(addressLowerLeftY).floatValue(), new Float(addressUpperRightX).floatValue(), new Float(addressUpperRightY).floatValue(), 15, Element.ALIGN_LEFT);
		ct.setSimpleColumn(Ph0,llx, lly, urx, ury, 15, Element.ALIGN_LEFT);
		ct.go();
	}
	...
	/* Commented out since it is never used...
	private class NoLetterTemplateException extends Exception{
		NoLetterTemplateException(String type){
