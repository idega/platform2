...
/**
 * This is the data bean for the "faktureringsrad", "fakturarad" och
 * "detaljutbetalningspost" (They are all the same dataobject) in the
 * Kravspecifikation Check and Peng.
 * 
 * @author Joakim
 */
public class InvoiceRecordBMPBean extends GenericEntity implements
		InvoiceRecord {
	...
	// For the vux batch
	public static final String COLUMN_VUX_GRADE_PAYMENT = "vux_grade_payment";

	public static final String COLUMN_VUX_PAYMENT = "vux_payment";
	...
	public void initializeAttributes() {
		...
		addAttribute(COLUMN_INVOICE_TEXT, "", true, true,
				java.lang.String.class, 1000);
		addAttribute(COLUMN_INVOICE_TEXT_2, "", true, true,
				java.lang.String.class, 1000);
		addAttribute(COLUMN_RULE_TEXT, "", true, true, java.lang.String.class,
				1000);
		addAttribute(COLUMN_DAYS, "", true, true, java.lang.Integer.class);
		addAttribute(COLUMN_PERIOD_START_CHECK, "", true, true,
				java.sql.Date.class);
		addAttribute(COLUMN_PERIOD_END_CHECK, "", true, true,
				java.sql.Date.class);
		addAttribute(COLUMN_PERIOD_START_PLACEMENT, "", true, true,
				java.sql.Date.class);
		addAttribute(COLUMN_PERIOD_END_PLACEMENT, "", true, true,
				java.sql.Date.class);
		addAttribute(COLUMN_DATE_CREATED, "", true, true, java.sql.Date.class);
		addAttribute(COLUMN_CREATED_BY, "", true, true, java.lang.String.class,
				1000);
		addAttribute(COLUMN_DATE_CHANGED, "", true, true, java.sql.Date.class);
		addAttribute(COLUMN_CHANGED_BY, "", true, true, java.lang.String.class,
				1000);
		...
		addAttribute(COLUMN_OWN_POSTING, "", true, true,
				java.lang.String.class, 1000);
		addAttribute(COLUMN_DOUBLE_POSTING, "", true, true,
				java.lang.String.class, 1000);
		// addAttribute(COLUMN_VAT_TYPE, "", true, true,
		// java.lang.Integer.class);
		addManyToOneRelationship(COLUMN_VAT_RULE_REGULATION_ID,
				Regulation.class);
		...
		addManyToOneRelationship(COLUMN_SCHOOL_CLASS_MEMBER_ID,
		...
		addManyToOneRelationship(COLUMN_REG_SPEC_TYPE_ID,
				RegulationSpecType.class);
		addManyToOneRelationship(COLUMN_CHILDCARE_CONTRACT_ID,
				ChildCareContract.class);

		addAttribute(COLUMN_VUX_PAYMENT, "VUX payment", Boolean.class);
		addAttribute(COLUMN_VUX_GRADE_PAYMENT, "VUX grade payment",
				Boolean.class);
	...
	public boolean getVUXPayment() {
		return getBooleanColumnValue(COLUMN_VUX_PAYMENT, false);
	}

	public boolean getVUXGradePayment() {
		return getBooleanColumnValue(COLUMN_VUX_GRADE_PAYMENT, false);
	}
	...
	/*
	 * public int getVATType() { return getIntColumnValue(COLUMN_VAT_TYPE); }
	 * 
	 * public void setVATType(int i) { setColumn(COLUMN_VAT_TYPE, i); }
	 */
	...
	public void setVUXPayment(boolean payment) {
		setColumn(COLUMN_VUX_PAYMENT, payment);
	}

	public void setVUXGradePayment(boolean payment) {
		setColumn(COLUMN_VUX_GRADE_PAYMENT, payment);
	}
	...
	public Collection ejbFindByInvoiceHeader(InvoiceHeader invoiceHeader)
		...
		final String[] tableNames = { getTableName(), UserBMPBean.TABLE_NAME,
		...
		sql.appendAndEquals(R_ + COLUMN_SCHOOL_CLASS_MEMBER_ID, M_
				+ SchoolClassMemberBMPBean.SCHOOLCLASSMEMBERID);
		sql.appendAndEquals(M_ + SchoolClassMemberBMPBean.MEMBER, U_
				+ User.FIELD_USER_ID);
	...
	public Collection ejbFindByContract(ChildCareContract contract)
			throws FinderException {
		IDOQuery query = idoQuery();
		query.appendSelectAllFrom(this).appendWhereEquals(
				COLUMN_CHILDCARE_CONTRACT_ID, contract);
	...
	public int ejbHomeGetIndividualCountByPaymentRecords(
			PaymentRecord[] paymentRecords) throws IDOException {
		if (0 >= paymentRecords.length)
			return 0;
		final String R_ = "r."; // sql alias for invoice record
		final String M_ = "m."; // sql alias for schoolclassmember
		final String[] tableNames = { getTableName(),
				SchoolClassMemberBMPBean.SCHOOLCLASSMEMBER };
		final String[] tableAliases = { "r", "m" };
		final IDOQuery sql = idoQuery();
		sql.appendSelect().append(
				" count (distinct " + M_ + SchoolClassMemberBMPBean.MEMBER
						+ ") ");
		sql.appendFrom(tableNames, tableAliases);
	...
	public Collection ejbFindByPaymentRecordOrderedByStudentName(
		...
		sql.appendAndEquals(M_ + SchoolClassMemberBMPBean.MEMBER, U_
				+ User.FIELD_USER_ID);
		sql.appendOrderBy(new String[] { U_ + User.FIELD_LAST_NAME,
				U_ + User.FIELD_FIRST_NAME });
		return idoFindPKsByQuery(sql);
	}
	 ...
	 * @throws IDOException
	 */
	public int ejbHomeGetPlacementCountForSchoolCategoryAndPeriod(
		...
		sql.append("select count(r.cacc_invoice_record_id) from "
				+ getEntityName());
		sql.append(" r, cacc_invoice_header h ");
		sql.appendWhereEqualsQuoted("h.school_category_id", schoolCategoryID);
		sql.appendAnd().append("h.period").appendGreaterThanOrEqualsSign()
				.append(start.getDate());
		sql.appendAnd().append("h.period").appendLessThanSign().append(
				end.getDate());
		sql.appendAnd().append(
				"r." + COLUMN_INVOICE_HEADER + " = h.cacc_invoice_header_id");
		return idoGetNumberOfRecords(sql);
	}

	/**
	 * Gets the number of handled children for the specified school types and
	 * period
	 * 
	 * @param schoolTypes
	 * @param month
	 ...
	 */
	public int ejbHomeGetNumberOfHandledChildrenForSchoolTypesAndMonth(
		...
		sql.append("select count (distinct mem.").append(
				SchoolClassMemberBMPBean.MEMBER).append(" ) ").appendFrom();
		// sch_class_member mem, cacc_invoice_record rec
		sql.append(SchoolClassMemberBMPBean.SCHOOLCLASSMEMBER).append(" mem, ")
				.append(getEntityName()).append(" rec ");
		// where mem.sch_class_member_id = rec.sch_class_member_id
		sql.appendWhere();
		sql.append("mem.").append(COLUMN_SCHOOL_CLASS_MEMBER_ID)
				.appendEqualSign().append("rec.").append(
						COLUMN_SCHOOL_CLASS_MEMBER_ID);
		// and
		sql.appendAnd();
		// rec.period_start_check <= '2003-11-30' and rec.period_end_check >=
		// '2003-11-01'
		sql.append("rec.").append(COLUMN_PERIOD_START_CHECK)
				.appendLessThanOrEqualsSign().append(lastDate);
		sql.appendAnd();
		sql.append("rec.").append(COLUMN_PERIOD_END_CHECK)
				.appendGreaterThanOrEqualsSign().append(firstDate);
		// and
		sql.appendAnd();
		// rec.sch_school_type_id in (4,2,3)
		sql.append(" rec.").append(COLUMN_SCHOOL_TYPE_ID).appendInCollection(
				schoolTypes);
		return idoGetNumberOfRecords(sql);
	}

	public int ejbHomeGetNumberOfInvoicesForStudent(SchoolClassMember student)
			throws IDOException {
		IDOQuery sql = idoQuery();
	...
	public int ejbHomeGetNumberOfInvoicesForStudentWithVUXGradePayment(
			SchoolClassMember student) throws IDOException {
		IDOQuery sql = idoQuery();
		sql.appendSelectCountFrom(this);
		sql.appendWhereEquals(COLUMN_SCHOOL_CLASS_MEMBER_ID, student);
		sql.appendAndEquals(COLUMN_VUX_GRADE_PAYMENT, true);

		return idoGetNumberOfRecords(sql);
	}

	public int ejbHomeGetNumberOfInvoicesForStudentWithVUXPayment(
			SchoolClassMember student) throws IDOException {
		IDOQuery sql = idoQuery();
		sql.appendSelectCountFrom(this);
		sql.appendWhereEquals(COLUMN_SCHOOL_CLASS_MEMBER_ID, student);
		sql.appendAndEquals(COLUMN_VUX_PAYMENT, true);

		return idoGetNumberOfRecords(sql);
	}
	
	public Collection ejbFindBySchoolClassMember(SchoolClassMember student) throws FinderException {
		IDOQuery sql = idoQuery();
		sql.appendSelectAllFrom(this);
		sql.appendWhereEquals(COLUMN_SCHOOL_CLASS_MEMBER_ID, student);
	...
	public double ejbHomeGetTotalAmountForSchoolTypesAndMonth(
			Collection schoolTypes, CalendarMonth month) throws IDOException {
		// get the first and the last date of the month
		Date firstDate = month.getFirstDateOfMonth();
		Date lastDate = month.getLastDateOfMonth();
		...
		// where
		sql.appendWhere();
		// period_start_check <= '2003-11-30' and period_end_check >=
		// '2003-11-01'
		sql.append(COLUMN_PERIOD_START_CHECK).appendLessThanOrEqualsSign()
				.append(lastDate);
		sql.appendAnd();
		sql.append(COLUMN_PERIOD_END_CHECK).appendGreaterThanOrEqualsSign()
				.append(firstDate);
		// and
		sql.appendAnd();
	 ...
	 * @throws FinderException
	 */
	public Collection ejbFindByMonthAndCategory(CalendarMonth month,
			String categoryId) throws FinderException {
		/*
		 * IWTimestamp start = new IWTimestamp(month); start.setAsDate();
		 * start.setDay(1); IWTimestamp end = new IWTimestamp(start);
		 * end.addMonths(1);
		 */
		...
		sql.appendWhere("r." + COLUMN_PERIOD_START_CHECK)
				.appendGreaterThanOrEqualsSign().append(start);
		sql.appendAnd().append("r." + COLUMN_PERIOD_END_CHECK)
				.appendLessThanOrEqualsSign().append(end);
		sql.appendAndEquals("r." + COLUMN_SCHOOL_TYPE_ID,
				"st.sch_school_type_id");
