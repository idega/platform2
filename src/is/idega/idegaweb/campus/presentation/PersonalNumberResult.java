...
public class PersonalNumberResult extends CampusBlock implements Campus {
  ...
  private PresentationObject getSSNResult(IWContext iwc){
    ...
    if(SSN!=null){
      try{
        //Collection applicants = ContractFinder.findAllNonContractApplicationsBySSN(SSN);
      	Collection applicants = getContractService(iwc).getContractHome().getUnsignedApplicants(SSN);
        if(applicants!=null  && !applicants.isEmpty()){
          T.add(getNonContractApplicantInfo(applicants),col,row++);
        }
        Collection contracts = getContractService(iwc).getContractHome().findByPersonalID(SSN);
  ...
  private PresentationObject getContractInfo(IWContext iwc,Collection contracts)throws FinderException,RemoteException{
    DataTable T = new DataTable();
     T.setUseBottom(false);
    T.setUseTop(false);
  ...
  public PresentationObject getApplicantInfo(Applicant applicant){
	DataTable T = new DataTable();
		...
		if(applicant!=null){
		  ...
		  T.add(getText(String.valueOf(applicant.getPrimaryKey().toString())),col,row++);
