...
public class TextControl3 extends TextControl {
	...
	public void doMenu() {
		...
		try {
			for (int a = 5; a < 13; a++) {
				TxText text = ((com.idega.block.text.data.TxTextHome) com.idega.data.IDOLookup
						.getHomeLegacy(TxText.class)).findByPrimaryKeyLegacy(a);
				if (text == null) {
					try {
						text = ((com.idega.block.text.data.TxTextHome) com.idega.data.IDOLookup
								.getHomeLegacy(TxText.class)).create();
