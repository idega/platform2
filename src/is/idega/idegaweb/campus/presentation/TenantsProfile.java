...
public class TenantsProfile extends CampusBlock {
	...
	private final static String NAME = "name", SSN = "ssn", MOBILE = "mobile", EMAIL = "email", FACULTY = "faculty",
			STUDYTRACK = "studytrack", STUDYBEGIN = "studybegin", STUDYEND = "studyend", SPOUSENAME = "spousename",
			SPOUSESSN = "spousessn", CHILDNAME = "childname", CHILDSSN = "childssn", CHILDID = "childid",
	...
	public void main(IWContext iwc) throws java.rmi.RemoteException {
		...
		if (isAdmin || isLoggedOn) {
			...
			try {
				java.util.Collection userContracts = ((ContractHome) IDOLookup.getHome(Contract.class)).findByUserAndRented(
				...
				if (userContracts != null && !userContracts.isEmpty()) {
					...
					apartmentView = getBuildingService(iwc).getApartmentViewHome().findByPrimaryKey(
	...
	private PresentationObject getProfile(IWContext iwc) {
		...
		// addToTable(table,row++,localize("name","Name"),_applicant.getFullName(),new
		// TextInput(NAME),35);
		// addToTable(table,row++,localize("ssn","SSN"),_applicant.getSSN(),new
		// TextInput(SSN),10);
		// addToTable(table,row++,localize("email","email"),campusApplication.getEmail(),new
		// TextInput(EMAIL),20);
		...
		addToTable(table, row++, localize("mobile", "Mobile phone"), _applicant.getMobilePhone(),
				new TextInput(MOBILE), 7);
		addToTable(table, row++, localize("faculty", "Faculty"), campusApplication.getFaculty(),
				new TextInput(FACULTY), 20);
		addToTable(table, row++, localize("studyTrack", "Study track"), campusApplication.getStudyTrack(),
				new TextInput(STUDYTRACK), 20);
		addToTable(table, row++, localize("studyBegin", "Study began"), campusApplication.getStudyBeginMonth() + "."
				+ campusApplication.getStudyBeginYear(), new TextInput(STUDYBEGIN), 7);
		addToTable(table, row++, localize("studyEnd", "Study ends"), campusApplication.getStudyEndMonth() + "."
				+ campusApplication.getStudyEndYear(), new TextInput(STUDYEND), 7);
		...
		if (update) {
			...
			if (children != null && children.size() > 0) {
				...
				for (int i = 0; i < size; i++) {
					...
					addToTable(table, row++, localize("childName", "Child name"), child.getName(), new TextInput(
							CHILDNAME + i), 35);
					addToTable(table, row++, localize("childSSN", "Child ssn"), child.getSSN(), new TextInput(CHILDSSN
							+ i), 10);
		...
		if (update) {
			...
			table.add(new SubmitButton(getResourceBundle().getImage("save.gif"), PARAMETER_MODE, PARAMETER_SAVE), 1,
					row);
			return myForm;
		}
		else {
	...
	private Table getApartment(IWContext iwc) {
		...
		String[] values = { apartmentView.getApartmentName(), apartmentView.getFloorName(),
				apartmentView.getComplexName(), apartmentView.getBuildingName() };
		row = 2;
	...
	private Table getAccount(IWContext iwc) throws java.rmi.RemoteException {
		...
		if (accounts != null && !accounts.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				double balance = getCampusService(iwc).getFinanceService().getAccountBalancePublished(
	...
	private Table getRequests(IWContext iwc) {
		...
		if (requests != null) {
			for (int a = 0; a < requests.size(); a++) {
				...
				try {
					table.add(formatText(new IWTimestamp(request.getDateSent()).getLocaleDate(iwc.getCurrentLocale())),
							2, row);
				}
				catch (Exception e) {
					table.add("", 2, row);
	...
	private void save(IWContext iwc) throws RemoteException {
		...
		if (ssn != null) {
			...
			if (!ssn.equals(user.getPersonalID())
					&& SocialSecurityNumber.isValidSocialSecurityNumber(ssn, getCampusSettings(iwc).getSystemLocale())) {
			...
			if (childcount != null) {
				...
				if (count > 0) {
					...
					for (int i = 0; i < count; i++) {
						...
						int childId = iwc.isParameterSet(CHILDID + i) ? Integer.parseInt(iwc.getParameter(CHILDID + i))
								: -1;
