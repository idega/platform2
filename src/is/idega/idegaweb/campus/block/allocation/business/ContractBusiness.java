/*
 * $Id: ContractBusiness.java,v 1.28 2004/05/24 14:21:41 palli Exp $
 *
...
public class ContractBusiness {
    ...
    public static String signCampusContract(IWApplicationContext iwac,
            int iContractId, int iGroupId, int iCashierId, String sEmail,
        ...
        try {
            ...
            eContract = ((is.idega.idegaweb.campus.block.allocation.data.ContractHome) com.idega.data.IDOLookup
            ...
            if (eContract != null) {
                ...
                if (newAccount) {
                    ...
                    AccountManager.makeNewFinanceAccount(iUserId, prefix
                            + " - " + String.valueOf(iUserId), "", iCashierId,
                            1);
                }
                if (newPhoneAccount) {
                    //System.err.println("adding phone account ");
                    String prefix = iwrb.getLocalizedString("phone", "Phone");
                    AccountManager.makeNewPhoneAccount(iUserId, prefix + " - "
                            + String.valueOf(iUserId), "", iCashierId, 1);
                }
                ...
                /*
                 * if(sendMail){ sendMail(iUserId,login,pass,iwrb); }
                 */
                ...
                MailingListBusiness.processMailEvent(iwac, iContractId,
    ...
    public static void createLogin(int iUserId, int iGroupId, String login,
            String pass, boolean generatePasswd) throws Exception {

        User eUser = ((com.idega.core.user.data.UserHome) com.idega.data.IDOLookup
                .getHomeLegacy(User.class)).findByPrimaryKeyLegacy(iUserId);
        //GenericGroup gg =
        // ((com.idega.core.data.GenericGroupHome)com.idega.data.IDOLookup.getHomeLegacy(GenericGroup.class)).findByPrimaryKeyLegacy(iGroupId);
        PermissionGroup pg = ((com.idega.core.accesscontrol.data.PermissionGroupHome) com.idega.data.IDOLookup
                .getHomeLegacy(PermissionGroup.class))
                .findByPrimaryKeyLegacy(iGroupId);
        ...
        //int validDays = today.getDaysBetween(today,new
        // IWTimestamp(eContract.getValidTo()));
        LoginDBHandler.createLogin(iUserId, login, pass);
        //LoginDBHandler.createLogin(iUserId,login,passwd,new
        // Boolean(true),today,validDays,new Boolean(false),new
        // Boolean(true),new Boolean(false),"");
    }

    public static void changeApplicationStatus(Contract eContract)
            throws Exception {
        String status = com.idega.block.application.data.ApplicationBMPBean.STATUS_SIGNED;
        List L = null;
        L = EntityFinder.getInstance().findAllByColumn(Application.class,
    ...
    public static void deleteFromWaitingList(Integer applicantID) {
        List L = WaitingListFinder.listOfWaitingList(
                WaitingListFinder.APPLICANT, applicantID.intValue(), 0, 0);
        if (L != null) {
            Iterator I = L.iterator();
            while (I.hasNext()) {
                try {
    ...
    public static void sendMail(int iUserId, String login, String pass,
        ...
        if (lEmails != null) {
            ...
            try {
                String body = iwrb.getLocalizedString("signed_contract_body",
                        "You have a signed contract to a apartment");
                ...
                sbody.append("\n");
                String header = iwrb.getLocalizedString("signed_contract",
                        "Signed Contract");
                String from = sp != null ? sp.getAdminEmail()
                        : "admin@campus.is";
                if (from == null || "".equals(from)) from = "admin@campus.is";
                String host = sp != null ? sp.getEmailHost() : "mail.idega.is";
                if (host == null || "".equals(host)) host = "mail.idega.is";
                if (address == null || "".equals(address))
                        address = "aron@idega.is";
                SendMail.send(from, address, "", "aron@idega.is", host, header,
                        sbody.toString());
    ...
    public static Contract endContract(int iContractId, IWTimestamp movingDate,
            String info, boolean datesync) {
        try {
            Contract C = ((is.idega.idegaweb.campus.block.allocation.data.ContractHome) com.idega.data.IDOLookup
                    .getHomeLegacy(Contract.class))
                    .findByPrimaryKeyLegacy(iContractId);
            C.setMovingDate(movingDate.getSQLDate());
            if (datesync) C.setValidTo(movingDate.getSQLDate());
    ...
    public static void resignContract(IWApplicationContext iwac,
            int iContractId, IWTimestamp movingDate, String info,
            boolean datesync) {
        try {
            Contract C = ((is.idega.idegaweb.campus.block.allocation.data.ContractHome) com.idega.data.IDOLookup
                    .getHomeLegacy(Contract.class))
    ...
    public static boolean makeNewContract(IWApplicationContext iwc, User eUser,
            Applicant eApplicant, int iApartmentId, IWTimestamp from,
            IWTimestamp to) {

        Contract eContract = ((ContractHome) IDOLookup
                .getHomeLegacy(Contract.class)).createLegacy();
        ...
        try {
            eContract.insert();
            MailingListBusiness.processMailEvent(iwc, eContract.getID(),
    ...
    public static User makeNewUser(Applicant A, String[] emails) {
        ...
        try {
            ...
            if (ssn != null) {
                try {
                    user = ((UserHome) IDOLookup.getHome(User.class))
                            .findByPersonalID(ssn);
                } catch (IDOLookupException e) {
                    e.printStackTrace();
                } catch (RemoteException e) {
                    e.printStackTrace();
                } catch (FinderException e) {
                    user = ub.insertUser(A.getFirstName(), A.getMiddleName(), A
                            .getLastName(), A.getFirstName(), "", null, null,
                            null, A.getSSN());
                }
            } else {
                user = ub.insertUser(A.getFirstName(), A.getMiddleName(), A
                        .getLastName(), A.getFirstName(), "", null, null, null,
                        null);
            }
    ...
    public static boolean deleteAllocation(int iContractId) {
        ...
        /*
         * try { Contract eContract =
         * ((is.idega.idegaweb.campus.block.allocation.data.ContractHome)com.idega.data.IDOLookup.getHomeLegacy(Contract.class)).findByPrimaryKeyLegacy(iContractId);
         * User eUser =
         * ((com.idega.core.user.data.UserHome)com.idega.data.IDOLookup.getHomeLegacy(User.class)).findByPrimaryKeyLegacy(eContract.getUserId().intValue());
         * eContract.delete(); eUser.delete();
         * 
         * return true; } catch (SQLException ex) { ex.printStackTrace();
         * return false; }
         */
    }

    public static IWTimestamp[] getContractStampsForApartment(int apartmentId) {
        Apartment ap = com.idega.block.building.business.BuildingCacher
                .getApartment(apartmentId);
        return getContractStampsForApartment(ap);
    }

    public static IWTimestamp[] getContractStampsForApartment(
            Apartment apartment) {
        ApartmentTypePeriods ATP = ContractFinder.getPeriod(apartment
                .getApartmentTypeId());
        return getContractStampsFromPeriod(ATP, 1);

    }

    public static IWTimestamp[] getContractStampsFromPeriod(
            ApartmentTypePeriods ATP, int monthOverlap) {
    ...
    public static String getLocalizedStatus(
            com.idega.idegaweb.IWResourceBundle iwrb, String status) {
        String r = "";
    ...
    public static boolean doGarbageContract(int iContract) {
        ...
        try {
            Contract eContract = ((is.idega.idegaweb.campus.block.allocation.data.ContractHome) com.idega.data.IDOLookup
                    .getHomeLegacy(Contract.class)).findByPrimaryKeyLegacy(id);
