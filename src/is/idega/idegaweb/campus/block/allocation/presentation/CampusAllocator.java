...
public class CampusAllocator extends CampusBlock implements Campus {
	...
	protected void control(IWContext iwc) throws RemoteException,
		...
		if (iwc.getParameter(SUBCATEGORY_ID) != null) {
			pSubcatId = new Parameter(SUBCATEGORY_ID, iwc
					.getParameter(SUBCATEGORY_ID));
			subcatID = Integer.valueOf(iwc.getParameter(SUBCATEGORY_ID));
			try {
				subcategory = campusService.getBuildingService()
						.getApartmentSubcategoryHome().findByPrimaryKey(
								subcatID);
		...
		/*
		 * if (iwc.getParameter(COMPLEX_ID) != null) { pComplexId = new
		 * Parameter(COMPLEX_ID, iwc.getParameter(COMPLEX_ID)); complexID =
		 * Integer.valueOf(iwc.getParameter(COMPLEX_ID)); try { complex =
		 * campusService.getBuildingService().getComplexHome()
		 * .findByPrimaryKey(complexID); } catch (RemoteException e) {
		 * e.printStackTrace(); } catch (FinderException e) {
		 * e.printStackTrace(); } }
		 */
		...
		if (isAdmin) {
			if (subcategory != null) {// && complex != null) {
				...
				// Change allocation
					...
					Frame.add(getApartmentsForm(iwc, applicantID, contractID,
							waitinglistID), 3, row);
					row++;
				...
				// delete allocation
					...
					Integer wID = new Integer(iwc
							.getParameter("remove_waitinglist"));
					...
					Integer wID = new Integer(iwc
							.getParameter("reactivate_waitinglist"));
	...
	private PresentationObject getApartmentsForm(IWContext iwc,
		...
		Table Frame = new Table();
		/*
		 * Table Header = new Table(); Header.add(headerText(AT.getName()+"
		 * "),1,1); Header.add(headerText(CX.getName()),1,1);
		 * Header.mergeCells(1,1,4,1); Header.setRowColor(1,blueColor);
		 * Header.setWidth("100%"); Frame.add(Header,1,1);
		 */
		if (contract != null)
	...
	public PresentationObject getApplicantInfo(IWContext iwc,
		...
		CampusApplicationHolder AH = getApplicationService(iwc)
				.getApplicantInfo(applicantID.intValue());
		if (AH != null) {
...
//			}
	...
	private PresentationObject getCategoryLists(IWContext iwc)
		...
		if (showStatistics) {
				...
				for (Iterator iter = Categories.iterator(); iter.hasNext();) {
					...
					if (L != null) {
						...
						for (int j = 0; j < lLen; j++) {
							...
							/*
							 * if (view.getComplex() != null) { if
							 * (view.getComplex().getLocked()) { showEntry =
							 * false; } }
							 */

							/*
							 * if (view.getApartmentType() != null) { if
							 * (view.getApartmentType().getLocked()) { showEntry =
							 * false; } }
							 */
			...
			int row = 2;
			// ComplexSubcategoryViewHome csHome =
			// campusService.getBuildingService().getComplexSubcategoryViewHome();
			...
			if (Categories != null && !Categories.isEmpty()) {
				for (Iterator iter = Categories.iterator(); iter.hasNext();) {
					...
					for (Iterator iterator = subcategories.iterator(); iterator
						...
						/*
						 * if (complexSubcategory.getComplex() != null) { if
						 * (complexSubcategory.getComplex().getLocked()) {
						 * showEntry = false; } }
						 */

						/*
						 * if (complexSubcategory.getSubcategory() != null) { if
						 * (complexSubcategory.getSubcategory().getl) {
						 * showEntry = false; } }
						 */
	...
	private Link getOffWaitingListLink(WaitingList waitingList) {
		...
		L
				.addParameter("offwaitinglist", waitingList.getPrimaryKey()
						.toString());
	...
	private Link getDenialChangeLink(Integer contractID,
		...
		if (rejections < allowedRejections) {
			...
			L
					.addParameter("applicant", waitingList.getApplicantId()
							.toString());
			L.addParameter("wl_id", waitingList.getPrimaryKey().toString());
	...
	private PresentationObject getContractMakingTable(IWContext iwc,
			...
			Integer apartmentID, WaitingList waitingList)
			throws RemoteException, FinderException {
		...
		if (applicantID.intValue() != -1) {
			HiddenInput Hid = new HiddenInput("applicantid", applicantID
					.toString());
			T.add(Hid);
		}
		if (apartmentID.intValue() != -1) {
			HiddenInput Hid = new HiddenInput("apartmentid", apartmentID
					.toString());
			T.add(Hid);
		}
	...
	private PresentationObject getContractsForm(IWContext iwc,
			...
			IWTimestamp from, WaitingList waitingList) throws RemoteException,
			FinderException {
			...
			for (Iterator iter = apartments.iterator(); iter.hasNext();) {
				...
				if (apartment.getApartment().getRentable()) {
					...
					nextAvailable = new IWTimestamp(campusService
							.getContractService().getNextAvailableDate(
					...
					if (nextAvailable != null) {
						...
						T
								.add(getApartmentContractsLink(text,
	...
	public Link getPDFLink(IWContext iwc, PresentationObject MO,
		...
		link
				.setURL(iwc.getIWMainApplication().getMediaServletURI()
						+ "apps_type_" + aprt_type_id + "_complex_" + cmplx_id
						+ ".pdf");
		link.addParameter(CampusApplicationWriter.PRM_WRITABLE_CLASS,
				IWMainApplication
						.getEncryptedClassName(CampusApplicationWriter.class));
				...
				while (it.hasNext()) {
					...
					try {
						...
						Frame.add(getWaitingListOrderLink(WL.getPrimaryKey()
								.toString(), numberOnList, subcatID.intValue(),
								listSize), col++, row);
						numberOnList++;
						Frame.add(getText(WL.getPriorityLevel()), col++, row);
						if (priorityColorMap.containsKey(WL.getPriorityLevel())) {
							colorMap.put(new Integer(row), priorityColorMap
									.get(WL.getPriorityLevel()));
						}
						...
						if (bcontracts
							...
							if (WL.getNumberOfRejections() > 0) {
								Frame.add(getDenialChangeLink(contractID, WL,
										isOnTime), col++, row);
							}
						...
						Frame
								.add(getText(applicant.getResidence()), col++,
										row);
						if (ifLong)
	...
	private String saveAllocation(IWContext iwc) {
		...
		IWTimestamp validFrom = new IWTimestamp(iwc
				.getParameter("contract_date_from"));
		IWTimestamp validTo = new IWTimestamp(iwc
				.getParameter("contract_date_to"));
