...
public class ContractFiler extends CampusBlock {
	...
	public void main(IWContext iwc) {
		...
		if (iwc.getParameter(prmOneId) != null) {
			...
			//CampusContractWriter.writePDF(id,iwrb,path+filename, nameFont,titleFont, paraFont, tagFont, textFont);
			if (filewritten)
			...
			boolean filewritten = false;
			//CampusContractWriter.writeTestPDF(iwrb,path+filetest,  nameFont,titleFont, paraFont, tagFont, textFont);
			if (filewritten)
			...
			*/
			boolean filewritten = false;
			//CampusContractWriter.writePDF(ids,iwrb,path+filetest,  nameFont,titleFont, paraFont, tagFont, textFont);
			if (filewritten)
				p.setToRedirect("/servlet/pdf?&dir=" + path + filetest, 1);
			else
				add("failed");
