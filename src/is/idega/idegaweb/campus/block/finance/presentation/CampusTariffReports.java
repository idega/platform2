...
public class CampusTariffReports extends Finance {
	...
	private void parse(IWContext iwc) {
		...
		try {
			Collection keys = ((AccountKeyHome) IDOLookup.getHome(AccountKey.class)).findByPrimaryKeys(acckeys);
			...
			for (Iterator iter = keys.iterator(); iter.hasNext();) {
				...
				keyMap.put((Integer) element.getPrimaryKey(), element.getInfo());
			}
		}
		catch (Exception e) {
			e.printStackTrace();
		...
		if (!iwc.isParameterSet("bycontract")) {
			...
			if (iwc.isParameterSet("byjoins")) {
				try {
					...
					accountEntriesReports = AccountEntriesReportBMPBean.findAllBySearch(buildings, acckeys,
							from.getDate(), to.getDate(), iwc.isParameterSet("bytariffkey"));
				}
				...
				try {
					accountReports = AccountEntryReportBMPBean.findAllBySearch(buildings, acckeys, from.getTimestamp(),
							to.getTimestamp(), iwc.isParameterSet("bytariffkey"));
				}
				catch (java.sql.SQLException ex) {
					ex.printStackTrace();
				}
	...
	public PresentationObject getSearchForm() throws java.rmi.RemoteException {
		...
		T.add(chkShowAllKeys, 1, 3);
		T.add(getSmallHeader(localize("show_all_keys", "Show all account keys")), 1, 3);
		// T.add(chkByJoinSQL,1,3);
		// T.add(getSmallHeader(localize("show_by_join_sql", "Show by join
		// sql")),1, 3);
	...
	private Collection groupReportsByContracts() {
		...
		for (Iterator iter = accountReports.iterator(); iter.hasNext();) {
			...
			if (mapOfIndexByContract.containsKey(accountID)) {
				...
				AccountEntryReport previous = (AccountEntryReport) m.get(element.getKeyID());
				if (previous != null) {
					element.setTotal(new Float(element.getTotal().floatValue() + previous.getTotal().floatValue()));
				}
	...
	public GenericSelect getBuildingsSelection(String name) throws java.rmi.RemoteException {
		GenericSelect drp = useBoxSelection ? (GenericSelect) new SelectPanel(name)
				: (GenericSelect) new SelectDropdown(name);
		try {
