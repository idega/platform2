...
public class CampusFinanceHandler implements FinanceHandler {
	...
	public boolean executeAssessment(IWApplicationContext iwac, Integer categoryId, Integer tariffGroupId,
			String roundName, Integer cashierId, Integer accountKeyId, IWTimestamp paymentdate, IWTimestamp start,
		...
		String[] statuses = { ContractBMPBean.STATUS_SIGNED, ContractBMPBean.STATUS_ENDED,
				ContractBMPBean.STATUS_RESIGNED, ContractBMPBean.STATUS_TERMINATED };
		try {
			...
			if (excessRoundID != null && excessRoundID.intValue() > 0) {
				listOfUsers = caah.findByTypeAndStatusAndOverlapPeriodAndNotInRound(getAccountType(), statuses,
						start.getDate(), end.getDate(), excessRoundID);
			}
			else {
				listOfUsers = caah.findByTypeAndStatusAndOverlapPeriod(getAccountType(), statuses, start.getDate(),
						end.getDate());
			}
	...
	public double getFactor(ContractAccountApartment con, IWTimestamp start, IWTimestamp end, int precision) {
		...
		// if(ret<1)System.out.println("factor for contract
		// "+con.getContractId() +" start:"+(new
		// java.util.Date(begin)).toString()+" end: "+(new
		// java.util.Date(endin)).toString()+" from"+(new
		// java.util.Date(valfr)).toString()+" to"+(new
		// java.util.Date(valto)).toString()+" factor: "+ret);
	...
	private double getFactor(long begin, long endin, long valfr, long valto, int precision) {
		 ...
		 * theBegin.setTime(0,0,0,0); IWTimestamp theEnd = new
		 * IWTimestamp(endin); theEnd.setTime(23,59,59); begin =
		 * theBegin.getDate().getTime(); endin = theEnd.getDate().getTime();
		 * BigDecimal ret = new BigDecimal(endin - begin);
		 * if(IWTimestamp.getDaysBetween(theBegin,theEnd)>0){ ret =
	...
	public static void main(String[] args) {
		...
		for (int day = 1; day <= 30; day++) {
			...
			double ret = handler.getFactor(begin.getDate().getTime(), end.getDate().getTime(),
					from.getDate().getTime(), to.getDate().getTime(), 4);
			// System.out.print(" Begin: "+begin.toString()+" End
			// "+end.toString());
	...
	public Collection listOfAssessmentTariffPreviews(IWApplicationContext iwac, Integer tariffGroupId,
			IWTimestamp start, IWTimestamp end) throws java.rmi.RemoteException {
		Collection tariffs = null;
		try {
			tariffs = ((TariffHome) IDOLookup.getHome(Tariff.class)).findByTariffGroup((tariffGroupId));
		}
	...
	private void createApartmentAccountEntry(ContractAccountApartment caa, AccountEntry AE) throws CreateException,
			IDOLookupException {
		ApartmentAccountEntry aprtEntry = ((ApartmentAccountEntryHome) IDOLookup.getHome(ApartmentAccountEntry.class)).create();
		aprtEntry.setAccountEntryID((Integer) AE.getPrimaryKey());
		aprtEntry.setApartmentID(new Integer(caa.getApartmentId()));
		aprtEntry.store();
	}

	private void createBatchContract(ContractAccountApartment caa, AssessmentRound round) throws CreateException,
			IDOLookupException {
	...
	public Collection getTariffsForAccountInGroup(Integer accountID, Integer tariffGroupID) {
		...
		try {
			...
			ContractAccountApartment account = ((ContractAccountApartmentHome) IDOLookup.getHome(ContractAccountApartment.class)).findByAccountAndRented(
					accountID, true);
			for (Iterator iter = tariffs.iterator(); iter.hasNext();) {
	...
	public void publishAssessment(IWApplicationContext iwac, Integer roundId) {
		try {
			CampusAssessmentBusiness assBuiz = (CampusAssessmentBusiness) IBOLookup.getServiceInstance(iwac,
	...
	private int getPrecision(IWApplicationContext iwac) {
		String precision = iwac.getIWMainApplication().getBundle(CampusSettings.IW_BUNDLE_IDENTIFIER).getProperty(
