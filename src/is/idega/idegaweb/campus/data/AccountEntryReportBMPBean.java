...
public class AccountEntryReportBMPBean implements AccountEntryReport {
	...
	public static Collection findAllBySearch(String[] buildingIds,
		...
		if (buildingIds != null) {
			try {
				...
				while (RS.next()) {
					...
					// ACC_ID BUILD_ID BUILDING FIRST_NAME MIDDLE_NAME LAST_NAME
					// PERSONAL_ID KEYID KEYCODE KEYINFO TOTAL
					if (tempObj != null) {
	...
	public static Collection findAllBySearchAllEntries(String[] buildingIds,
			String[] accountKeys, Timestamp from, Timestamp to,
			boolean byAccountCode) throws SQLException {
		Connection conn = null;
		Statement Stmt = null;
		ResultSetMetaData metaData;
		Vector vector = null;
		String sql = getFindSql2(buildingIds, accountKeys, from, to,
				byAccountCode);
		if (buildingIds != null) {
			try {
				conn = ConnectionBroker.getConnection();
				...
				while (RS.next()) {
					...
					if (tempObj != null) {
						...
						tempObj.setTotal(new Float(RS.getFloat(11)));// RS.
																		...
																		// COLUMN_TOTAL
																		// )));
						tempObj.setInfo(RS.getString(12));
						tempObj.setPaymentDate(RS.getDate(13));
	...
	private static String getFindSql(String[] buildingIds, String[] keyIds,
			java.sql.Timestamp from, java.sql.Timestamp to,
			boolean byAccountKeyCode) {

		StringBuffer sql = new StringBuffer();

		sql
				.append("  select a.FIN_ACCOUNT_ID ACC_ID,b.BU_BUILDING_ID BUILD_ID,b.NAME BUILDING, ");
		sql.append("  ");
		sql.append(" u.FIRST_NAME,u.MIDDLE_NAME,u.LAST_NAME,u.PERSONAL_ID, ");
		sql.append(" ");
		if (!byAccountKeyCode)
			sql
					.append(" k.FIN_ACC_KEY_ID KEYID,k.NAME KEYCODE,k.INFO KEYINFO ");
		else
			sql
					.append(" tk.FIN_TARIFF_KEY_ID  KEYID,tk.NAME KEYCODE,tk.INFO KEYINFO ");
		sql.append(",sum(e.TOTAL) TOTAL ");
		sql
				.append(" from FIN_ACC_ENTRY e, FIN_ACCOUNT a,IC_USER u, FIN_ACC_KEY k,FIN_TARIFF_KEY tk , ");
		sql
				.append(" CAM_APRT_ACC_ENTRY ce,BU_APARTMENT ap, BU_FLOOR f,BU_BUILDING b ");
		...
		sql
				.append(" group by a.FIN_ACCOUNT_ID,b.BU_BUILDING_ID,b.name,ap.bu_apartment_id,ap.NAME, ");
		sql.append(" u.FIRST_NAME,u.MIDDLE_NAME,u.LAST_NAME,u.PERSONAL_ID, ");

		sql.append(!byAccountKeyCode ? " k.fin_acc_key_id,k.NAME,k.INFO "
				: "tk.fin_tariff_key_id,tk.NAME,tk.INFO");
		sql.append(" order by u.FIRST_NAME,u.LAST_NAME,a.FIN_ACCOUNT_ID ");
		// System.out.println(sql.toString());
		return sql.toString();
	}

	private static String getFindSql2(String[] buildingIds, String[] keyIds,
			java.sql.Timestamp from, java.sql.Timestamp to,
			boolean byAccountKeyCode) {
		...
		if (!byAccountKeyCode)
			...
			sql
					.append(" tk.FIN_TARIFF_KEY_ID  KEYID,tk.NAME KEYCODE,tk.INFO KEYINFO ");
		sql.append(",e.TOTAL TOTAL, e.INFO INFO, e.PAYMENT_DATE PAYMENT_DATE ");
		sql
				.append(" from FIN_ACC_ENTRY e, FIN_ACCOUNT a,IC_USER u, FIN_ACC_KEY k,FIN_TARIFF_KEY tk , ");
		sql
				.append(" CAM_APRT_ACC_ENTRY ce,BU_APARTMENT ap, BU_FLOOR f,BU_BUILDING b ");
