...
public class NortekBusinessBean extends IBOServiceBean implements
	...
	public boolean isCardValid(String serialNumber) {
		...
		try {
			...
			addLogEntry(card, IWTimestamp.RightNow(), null, ACTION_VERIFY,
					Boolean.toString(card.getIsValid()), null, false, null,
					serialNumber);
		} catch (FinderException e) {
			e.printStackTrace();
			addLogEntry(card, IWTimestamp.RightNow(), null, ACTION_VERIFY,
					Boolean.toString(false), null, true, e.getMessage(),
					serialNumber);
			return false;
		}
	...
	public boolean banCard(String serialNumber, boolean ban) {
		...
		try {
			card = getCardHome().findByPrimaryKey(serialNumber);
			addLogEntry(card, IWTimestamp.RightNow(), null, ACTION_BAN, Boolean
					.toString(ban), null, false, null, serialNumber);
		} catch (FinderException e) {
			e.printStackTrace();
			try {
				...
				card
						.setDecodedCardSerialNumber(decodeSerialNumber(serialNumber));
				card.store();
				addLogEntry(card, IWTimestamp.RightNow(), null, ACTION_BAN,
						Boolean.toString(ban), null, false, null, serialNumber);
			} catch (CreateException e1) {
				e1.printStackTrace();
				addLogEntry(card, IWTimestamp.RightNow(), null, ACTION_BAN,
						Boolean.toString(ban), null, true, e1.getMessage(),
						serialNumber);
				return false;
			}
		}
	...
	public boolean addAmountToCardUser(String serialNumber, Date timestamp,
			double amount, String terminalNumber) {
		Card card = null;
		...
		try {
			...
			Contract contract = getCampusAssessmentBusiness()
					.findContractForUser(user);
			Integer division = new Integer(contract.getApartment().getFloor()
					.getBuilding().getDivision());
			...
			getCampusAssessmentBusiness().assessTariffsToAccount(
					(float) amount, info, info,
					(Integer) account.getPrimaryKey(),
					(Integer) setup.getAccountKey().getPrimaryKey(),
					today.getDate(),
					(Integer) setup.getTariffGroup().getPrimaryKey(),
					(Integer) setup.getFinanceCategory().getPrimaryKey(),
					contract.getApartmentId(), false, null);

			addLogEntry(card, IWTimestamp.RightNow(),
					new IWTimestamp(timestamp), ACTION_ADD, Double
							.toString(amount), terminalNumber, false, null,
					serialNumber);
		} catch (Exception e) {
			e.printStackTrace();
			Throwable t = e.getCause();
			if (t != null) {
				addLogEntry(card, IWTimestamp.RightNow(), new IWTimestamp(
						timestamp), ACTION_ADD, Double.toString(amount),
						terminalNumber, true, t.getMessage(), serialNumber);
			} else {
				addLogEntry(card, IWTimestamp.RightNow(), new IWTimestamp(
						timestamp), ACTION_ADD, Double.toString(amount),
						terminalNumber, true, e.getMessage(), serialNumber);
			}
			return false;
		}
	...
	private void addLogEntry(Card card, IWTimestamp stamp,
			IWTimestamp externalStamp, String action, String value,
			String terminal, boolean isError, String errorMessage,
			String serialNumber) {
	...
	public Collection getCardsByValdi(boolean valid) {
		Collection col = null;
		try {
			col = getCardHome().findAllByValid(valid);
		} catch (FinderException e) {
		}
	...
	public String encodeDecodedSerialNumber(String encodedSerialNumber) {
		String ret = CARD_PREFIX
				+ Integer.toHexString(
						0x10000 | Integer.parseInt(encodedSerialNumber))
	...
	private CardTransactionLogHome getCardTransactionLogHome() {
		try {
			return (CardTransactionLogHome) IDOLookup
					.getHome(CardTransactionLog.class);
		} catch (IDOLookupException e) {
			e.printStackTrace();
		}
