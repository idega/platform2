...
public class FinanceEntryBMPBean extends GenericEntity implements FinanceEntry, BasketItem {
	...
	protected final static String COLUMN_DUE_DATE = "due_date";
...
/*	public void setStatusReady() {
	...
	public Date getDueDate() {
		return getDateColumnValue(COLUMN_DUE_DATE);
	}

	public void setDueDate(Date dueDate) {
		setColumn(COLUMN_DUE_DATE, dueDate);
	}

	public Date getFinalDueDate() {
		return getDateColumnValue(COLUMN_FINAL_DUE_DATE);
	}

	public void setFinalDueDate(Date finalDueDate) {
		setColumn(COLUMN_FINAL_DUE_DATE, finalDueDate);
	}

	public String getPayedBySSN() {
		return getStringColumnValue(COLUMN_PAYED_BY_SSN);
	}
	...
	public Collection ejbFindAllOpenAssessmentByUser(Group club, Group division, User user) throws FinderException {
		IDOQuery sql = idoQuery();
		sql.appendSelectAllFrom(this);
		sql.appendWhereEquals(COLUMN_CLUB_ID, club);
		if (division != null) {
	...
	public Collection ejbFindAllAssessmentByUser(Group club, Group division, User user, IWTimestamp entriesAfter)
	 ...
	 * @throws FinderException
	 */
	public Collection ejbFindAllFinanceEntriesByDateIntervalDivisionsAndGroupsOrderedByDivisionGroupAndDate(Group club,
		...
		sql.appendWhereEquals(F_ + COLUMN_USER_ID, U_ + User.FIELD_USER_ID);
		...
		if (personalID != null && !personalID.equals("")) {
			sql.appendAnd().append(U_ + User.FIELD_PERSONAL_ID).appendLike().append("\'" + personalID + "%'");
		}
	 ...
	 * @throws FinderException
	 */
	public Collection ejbFindAllFinanceEntriesByPaymentDateDivisionsAndGroupsOrderedByDivisionGroupAndDate(Group club,
		...
		sql.appendAndEquals(F_ + COLUMN_USER_ID, U_ + User.FIELD_USER_ID);
		sql.appendAnd().append(A_ + AssessmentRoundBMPBean.COLUMN_PAYMENT_DATE).appendLessThanSign().append(
				now.getDate());
		sql.appendAndEqualsQuoted(COLUMN_OPEN, ENTRY_OPEN_YES);
		if (types != null && types.length > 0) {
	...
	public Collection ejbFindAllByGroupAndPaymentTypeNotInBatch(Group group, PaymentType type, IWTimestamp dateFrom,
