...
public class WorkReportSender extends WorkReportSelector {
	...
	private Form checkWorkReport(IWContext iwc) throws RemoteException{
		...
		boolean accountForEmpty = getWorkReportBusiness(iwc).isThereAYearlyAccountForAnEmptyDivision(workReportId);
	...
	private void sendWorkReport(IWContext iwc) throws RemoteException{
		if(getWorkReportBusiness(iwc).sendWorkReport(getWorkReportId(),iwc.getParameter(PARAM_TEXT),iwrb)){
			...
			add(iwrb.getLocalizedString("workreportsender.sending_failed","Sending failed! Please fix issues first!"));
		}
	}
