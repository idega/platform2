...
private double grunn;
  ...
  private void handicapIncrease(double breyting) {
    ...
    Tournament tournament = ((is.idega.idegaweb.golf.entity.TournamentHome)com.idega.data.IDOLookup.getHomeLegacy(Tournament.class)).findByPrimaryKeyLegacy(tournament_id);
    ...
    Tee[] tee = (Tee[]) (((is.idega.idegaweb.golf.entity.TeeHome)com.idega.data.IDOLookup.getHomeLegacy(Tee.class)).createLegacy()).findAllByColumn("field_id",field.getID()+"","tee_color_id",tee_color_id+"");
  ...
  public static int getTotalPoints(int scorecard_id, float grunnHandicap) {
    ...
    try {
      ...
      Scorecard scorecard = ((is.idega.idegaweb.golf.entity.ScorecardHome)com.idega.data.IDOLookup.getHomeLegacy(Scorecard.class)).findByPrimaryKeyLegacy(scorecard_id);

      Handicap handicap = new Handicap((double) grunnHandicap);

      Field field = ((is.idega.idegaweb.golf.entity.FieldHome)com.idega.data.IDOLookup.getHomeLegacy(Field.class)).findByPrimaryKeyLegacy(scorecard.getFieldID());
  ...
  public static int getTotalPoints(int scorecard_id, int leikHandicap) {
    ...
    try {
      ...
      Stroke[] stroke = (Stroke[]) (((is.idega.idegaweb.golf.entity.StrokeHome)com.idega.data.IDOLookup.getHomeLegacy(Stroke.class)).createLegacy()).findAll("select stroke.* from stroke s,tee t where s.tee_id = t.tee_id and scorecard_id = "+scorecard_id+" order by hole_number");
