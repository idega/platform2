...
import is.idega.idegaweb.golf.block.news.data.NewsCategory;
...
public class NewsReader extends Block {
	...
	public NewsReader(IWTimestamp timestamp) {
		this.showAll = true;
	...
	public NewsReader(int categoryId, IWTimestamp timestamp) {
		this.showAll = false;
		this.categoryId = categoryId;
		this.date = timestamp.toSQLString();
	}
	...
	public void main(IWContext modinfo) throws Exception {
		...
		if (newsReaderURL == null) {
			newsReaderURL = iwb.getProperty("news_reader_url", modinfo.getRequestURI());//link
																																									// with
																																									// ""
																																									// constructs
																																									// a
		...
		try {
			...
			if (news_category_id != null) {
				categoryId = Integer.parseInt(news_category_id);//overrides the preset
																												// category
				showSingleNews = false;//nope we are showing a collection!
				showAll = false;
				//System.out.println("inni í category");
			}
			else if ((news_category_id == null) && (news_id != null)) { //yup only
																																	// one to see
																																	// if this.
																																	// owns the
																																	// newscategory!
				// System.out.println("(news_category_id==null) && (news_id
				// =="+news_id);
				/*
				 * if( categoryId != 0 ){ System.out.println("categoryId != 0 "); if(
				 * news[0].getNewsCategoryId() == categoryId ) { System.out.println("My
				 * category! "); showSingleNews = true; } } else{
				 * System.out.println("categoryId is 0 "); showSingleNews = true; }
				 */
				showSingleNews = true;
			}
			...
			if (showSingleNews) {//single news
				try {
					news[0] = ((NewsHome) IDOLookup.getHomeLegacy(News.class)).findByPrimaryKey(Integer.parseInt(news_id));
				}
				...
				if (categoryId == 0) {//show the whole collection
					...
					showAll = true;
					attribs = (NewsCategoryAttributes[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.NewsCategoryAttributes")).findAllByColumn("attribute_name", attributeName, "attribute_id", Integer.toString(attributeId));
					...
					showAll = false;
					categoryString = "news_category_id = '" + categoryId + "' ";
					//  System.err.println("Show all false categoryId="+categoryId);
				}
				...
				if (date == null) {//not by Date
					if (showAll) {
						...
						news = (News[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News")).findAll("select * from news " + categoryString + orderBy);
					}
					else
						news = (News[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News")).findAllByColumn("news_category_id", categoryId);
				}
				else {// by date
					//debug
					// System.err.println(date);
					String DatastoreType = getDatastoreType(GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News"));
					byDate = true;
					if ((categoryString != null) && !(categoryString.equals("")))
						categoryString = " OR " + categoryString;
					if ((showAll) && !(DatastoreType.equals("oracle"))) {
						...
						news = (News[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News")).findAll(statementstring);
						// if (news != null) System.err.println("fann "+ news.length
						// +"fréttir");

					}
					else if ((showAll) && (DatastoreType.equals("oracle")))
						news = (News[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News")).findAll(selectFrom + "news_date >= " + date + " " + categoryString + orderBy);
					else {
						if (!(DatastoreType.equals("oracle"))) {
							...
							news = (News[]) (GenericEntity.getStaticInstance("is.idega.idegaweb.golf.block.news.data.News")).findAll(selectFrom + sNewsCategoryId + categoryId + "' and news_date >= " + date + orderBy);
					}
				}
				...
				if (news.length > 0) {
					...
					if (byDate) {
						...
						if (news.length != 0) {
							...
							add(new Text("<b>No news in database</b><br>"));
					}
					else
						add(new Text("<b>No news in database</b><br>"));
	...
	private Table drawNewsTable(News[] news) throws IOException, SQLException {
		...
		//System.out.println("limitNumberOfNews: "+ limitNumberOfNews+"
		// numberOfDisplayedNews: "+numberOfDisplayedNews+" newsLength:
		// "+newsLength);
		...
		for (int i = 0; i < numberOfDisplayedNews; i++) {
			...
			try {
				newscat = ((NewsCategoryHome) IDOLookup.getHomeLegacy(NewsCategory.class)).findByPrimaryKey(news_category_id);
			}
			catch (FinderException fe) {
				throw new SQLException(fe.getMessage());
			}
	...
	private Table insertTable(String TimeStamp, String Headline, String NewsText, Text information, int newsId, int image_id) throws SQLException {
		...
		if (headline.getMarkupAttribute("size") == null)
			headline.setFontSize(2);
		else if (headline.getMarkupAttribute("size").equals(""))
			headline.setFontSize(2);

		Text newstext = new Text(NewsText);
		newstext = setTextAttributes(newstext);
		if (newstext.getMarkupAttribute("size") == null)
			newstext.setFontSize(2);
		else if (newstext.getMarkupAttribute("size").equals(""))
			newstext.setFontSize(2);

		getInformationProxy().setFontColor("#666666");
		information = setInformationAttributes(information);
		if (information.getMarkupAttribute("size") == null)
			information.setFontSize(1);
		else if (information.getMarkupAttribute("size").equals(""))
			information.setFontSize(1);
		...
		if (headlineAsLink) {
			...
			headlineImage.setMarkupAttribute("align", "absmiddle");
	...
	private Text getInfoText(String Author, String Source, String Category, String TimeStamp) {
		...
		if (showOnlyDates) {
			IWTimestamp timeStamp = new IWTimestamp(TimeStamp.substring(6, 10) + "-" + TimeStamp.substring(3, 5) + "-" + TimeStamp.substring(0, 2));
	...
	private void addNext(Table table) {
		switch (LAYOUT) {
			...
			case NEWS_SITE_LAYOUT :
				//debug NEWS_SITE_LAYOUT shows only one image for now...code in
				// insertTable
	...
	public void setNumberOfDisplayedNews(int numberOfDisplayedNews) {
		...
		if (numberOfDisplayedNews < 0)
			numberOfDisplayedNews = (-1) * numberOfDisplayedNews;
	...
	public void setNewsReaderURLAsSamePage(IWContext modinfo) {
		this.newsReaderURL = modinfo.getRequestURI();
	}

	public void setNewsCollectionURLAsSamePage(IWContext modinfo) {
		this.newsCollectionURL = modinfo.getRequestURI();
	}

	public void setNumberOfExpandedNews(int numberOfExpandedNews) {
		if (numberOfExpandedNews < 0)
			numberOfExpandedNews = (-1) * numberOfExpandedNews;
	...
	public void addNewsEditorURLParameter(String pameterName, String parameterValue) {
		//TODO: Reimplement lost method addNewsEditorURLParameter
	}

	public void addNewsEditorURLParameter(String pameterName, int parameterValue) {
		addNewsEditorURLParameter(pameterName, Integer.toString(parameterValue));
	}
