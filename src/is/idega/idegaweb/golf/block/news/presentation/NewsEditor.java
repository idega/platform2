...
public class NewsEditor extends Block {
	...
	private String PARAMETER_URL = "prm_url";
	...
	private String[] Lang = {"FrŽttastj—ri", "Fyrirsšgn", "FrŽtt", "Flokkur", "Hšfundur", "Heimild", "Fjšldi s?nilegra daga", "Mynd"};
	//private String[] Lang = {"Fréttastjóri","Fyrirsögn", "Frétt",
	// "Flokkur","Höfundur", "Heimild", "Fjöldi sýnilegra daga", "Mynd"};
	private String[] IS = {"FrŽttastj—ri", "Fyrirsšgn", "FrŽtt", "Flokkur", "Hšfundur", "Heimild", "Fjšldi s?nilegra daga", "Mynd"};
	//private String[] IS = {"Fréttastjóri","Fyrirsögn", "Frétt",
	// "Flokkur","Höfundur", "Heimild", "Fjöldi sýnilegra daga", "Mynd"};
	...
	private void setSpokenLanguage(IWContext modinfo) {
		...
		if (language2 == null)
			language2 = (String) modinfo.getSession().getAttribute("language");
		if (language2 != null)
			language = language2;
	...
	public void main(IWContext modinfo) throws Exception {
		...
		String tUrl = modinfo.getParameter(PARAMETER_URL);
		if (tUrl != null) {
			this.urlToReload = tUrl;
		}
		...
		try {
			...
			//else we are either writing something new or we are updating something
			// we have selected
			else {
				if (isAdmin) {
					...
					add(new Text("<center><b></b>Login First!</b></center>"));
				//add(editorTable(modinfo));
			}
		}
		catch (Exception e) {
	...
	public Text deleteNews(String news_id) throws SQLException {
		try {
			News news = ((NewsHome) IDOLookup.getHomeLegacy(News.class)).findByPrimaryKey(Integer.parseInt(news_id));
			news.delete();
	...
	public Table editorTable(IWContext modinfo) throws SQLException, IOException {
		...
		/*
		 * String mode = modinfo.getParameter("mode"); String mode =
		 * modinfo.getParameter("mode");
		 */
		...
		if (urlToReload != null) {
			mainPanel.add(new HiddenInput(this.PARAMETER_URL, this.urlToReload), 1, 4);
		}

		if (news_id != null) {
			try {
				news = ((NewsHome) IDOLookup.getHomeLegacy(News.class)).findByPrimaryKey(Integer.parseInt(news_id));
			}
			catch (FinderException fe) {
				throw new SQLException(fe.getMessage());
			}
		...
		NewsCategoryAttributes[] attribs = (NewsCategoryAttributes[]) ((NewsCategoryAttributes) IDOLookup.instanciateEntity(NewsCategoryAttributes.class)).findAllByColumn("attribute_name", attributeName, "attribute_id", "" + attributeId);
		...
		NewsCategory newscat = (NewsCategory) IDOLookup.instanciateEntity(NewsCategory.class);
		...
		/*
		 * myDropdown.setSelectedElement("Y"); myDropdown.setToSubmit();
		 * myDropdown.keepStatusOnAction(); myDropdown.addMenuElement("-1", "Veldu
		 * fréttaflokk" );
		 */
		...
		counterMenu.addMenuElement("-1", "î‡kve?i?");
		...
		/*
		 * Table returnTable = new Table(1,1); returnTable.add(myForm,1,1);
		 */
	...
	public boolean storeNews(IWContext modinfo) throws SQLException, IOException {
		...
		if (update) {
			try {
				...
				update = false;
				news = (News) IDOLookup.createLegacy(News.class);
			}
		}
		else {
			news = (News) IDOLookup.createLegacy(News.class);
	...
	public Table feedBack(boolean isOk) {
		...
		actionForm.add(new SubmitButton("Sko?a frŽttir"));
		if (urlToReload != null) {
			actionForm.setAction(urlToReload);
		}
		else {
			actionForm.setAction("/index.jsp");
		}
		if (this.attributeName != null) {
			actionForm.addParameter(this.attributeName, Integer.toString(this.attributeId));
		}
		...
		if (isOk) {
			myTable.addText("FrŽttin hefur veri? skr‡?", 1, 1);
			myTable.add(backForm, 1, 2);
			myTable.add(actionForm, 2, 2);

		}
		else {
			myTable.addText("Fylla ver?ur ’ alla reiti auk ?ess a? velja flokk", 1, 1);
	 ...
	 * try{ Class.forName("org.gjt.mm.mysql.Driver").newInstance(); //loads the
	 * driver String url =
	 * "jdbc:mysql://einstein.idega.is:3306/idegaweb?user=root&password=crazy0nes";
	 * Connection Conn2 = DriverManager.getConnection(url); Statement MysqlStmt =
	 * Conn2.createStatement(); MysqlStmt.executeUpdate("insert into
	 * headlines(group_id,site,headline,theURL)
	 * values(3,'"+site+"','"+Header+"','"+news_url+"')"); MysqlStmt.close();
	 * Conn2.close(); } catch (SQLException E) { System.err.print("SQLException: " +
	 * E.getMessage()); System.err.print("SQLState: " + E.getSQLState()); } catch
	...
	public DropdownMenu insertDropdown(String dropdownName) throws IOException, SQLException {
		...
		myDropdown.addMenuElement("Y", "FrŽtt");
	...
	public DropdownMenu newsDropdown(String newsDropdownName, String categoryDropdownName, Connection Conn, IWContext modinfo) throws IOException, SQLException {
		//PrintWriter out = modinfo.getResponse().getWriter();
		...
		myDropdown.addMenuElement("-1", "Veldu frŽtt");
		while (RS.next()) {
	...
	public DropdownMenu categoryDropdown(String categoryDropdownName, Connection Conn) throws IOException, SQLException {
		...
		try {
			...
			myDropdown.addMenuElement("-1", "Veldu frŽttaflokk");
			while (RS.next()) {
	...
	public void setNewsPageUrl(String url) {
