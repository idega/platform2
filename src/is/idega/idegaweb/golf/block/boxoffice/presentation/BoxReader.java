...
public BoxReader(String issue_id, int numberOfColumns){
		...
		for ( int a = 0; a < subject.length; a++ ) {
			...
			Text content_text = new Text(subject[a].getSubjectName());
				...
				content_text.setFontColor(categoryTextColor);
                        ...
                        IWTimestamp stampurinn = new IWTimestamp(subject[a].getSubjectDate());
	...
	private Table getSubject(String issue_category_id) throws IOException,SQLException {
		...
		if ( issueName.toLowerCase().equals("fréttir") || issueName.toLowerCase().equals("news") ) {
			...
			NewsBMPBean[] news = (NewsBMPBean[]) (new NewsBMPBean()).findAllByColumnOrdered("news_category_id",String.valueOf(Integer.parseInt(issue_id)+2),"news_date desc");
