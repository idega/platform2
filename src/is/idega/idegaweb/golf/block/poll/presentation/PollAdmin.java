...
import is.idega.idegaweb.golf.block.poll.data.Poll;
import is.idega.idegaweb.golf.block.poll.data.PollAttributes;
import is.idega.idegaweb.golf.block.poll.data.PollHome;
import is.idega.idegaweb.golf.block.poll.data.PollOption;
import is.idega.idegaweb.golf.block.poll.data.PollOptionHome;
...
boolean isAdmin = false;
    ...
    private void setAttributes(IWContext modinfo,HttpServletRequest request) {
    ...
    private void getAction(IWContext modinfo,HttpServletRequest request) throws IOException, SQLException {
...
public void start(IWContext modinfo) throws SQLException, IOException {
...
//			myForm.setMethod("get");
		...
		myTable.addBreak();
                ...
                PollAttributes[] poll_attrib = (PollAttributes[]) ((PollAttributes) IDOLookup.instanciateEntity(PollAttributes.class)).findAllByColumn("attribute_name",attribute_name,"attribute_id",""+attribute_id);
                ...
                if (poll_attrib != null) {
                  ...
                  if (!(sql_string.equals("Select * from poll where"))) {
                    sql_string = sql_string + " order by poll_id desc";
                    Poll[] polls = (Poll[]) ((Poll) IDOLookup.instanciateEntity(Poll.class)).findAll(sql_string);
		    DropdownMenu pollin= new DropdownMenu(polls);
                      pollin.setMarkupAttribute("size","10");
...
//                Link loka_glugga = new Link("Loka glugga",getRequest().getRequestURI());
//                  loka_glugga.addParameter("submit","close_window");
...
//			myForm.setMethod("get");
		myForm.add(myTable);
              ...
              if (poll_id != null) {
		Poll thePoll = ((PollHome) IDOLookup.getHomeLegacy(Poll.class)).findByPrimaryKeyLegacy(Integer.parseInt(poll_id));
		...
		myTable.add(new SelectionBox(((PollOption) IDOLookup.instanciateEntity(PollOption.class)).findAllByColumn("poll_id",poll_id)),1,2);
...
//          add("update"+update);
         if (update) {
                ...
                if (name.equals("")) {
                    ...
                    if (useDate) {
                      IWTimestamp stamp_from = new IWTimestamp(from_year+"-"+from_month+"-"+from_day+" 00:00:00.0");
                      IWTimestamp stamp_to = new IWTimestamp(from_year+"-"+from_month+"-"+from_day+" 00:00:00.0");
...
*/
                ...
                if (name.equals("")) {
                    ...
                    Poll poll = (Poll) IDOLookup.createLegacy(Poll.class);
                    ...
                    PollAttributes poll_attrib = (PollAttributes) IDOLookup.createLegacy(PollAttributes.class);
                      poll_attrib.setPollId(poll.getID());
                      poll_attrib.setAttributeName(attribute_name);
                      poll_attrib.setAttributeId(attribute_id);
                    poll_attrib.insert();
...
//		Poll poll = new Poll();
//			poll.setUnionID(union_id);
//			poll.setDefaultValues();
//		EntityInsert entIns = new EntityInsert(poll);
...
//		add(entIns);
	...
	public void newAnswer() throws SQLException, IOException  {
		...
		Table table = new Table(2,4);
          ...
          if (question_id != null) {
		PollOption option = (PollOption) IDOLookup.createLegacy(PollOption.class);
...
/*
		option.setVisible("poll_id",false);
//		option.set
		EntityInsert entIns = new EntityInsert(option);
//			entInt
		add(entIns );
//		add(myForm);
*/
	...
	public void saveAnswer() throws SQLException,IOException {
                ...
                if (poll_option != null) {
                  PollOption poll_opt = ((PollOptionHome) IDOLookup.getHomeLegacy(PollOption.class)).findByPrimaryKeyLegacy(Integer.parseInt(poll_option));
                  if (poll_answer != null) {
	...
	public void changeAnswer(String answer_id) throws SQLException, IOException {
		...
		if (answer_id != null ) {
			PollOption option = ((PollOptionHome) IDOLookup.getHomeLegacy(PollOption.class)).findByPrimaryKeyLegacy(Integer.parseInt(answer_id));
	...
	public void deleteQuestion() throws SQLException, IOException {
                String question_id = getRequest().getParameter("poll");
		Poll poll = ((PollHome) IDOLookup.getHomeLegacy(Poll.class)).findByPrimaryKeyLegacy(Integer.parseInt(question_id));

                PollAttributes[] pollAtt = (PollAttributes[])((PollAttributes) IDOLookup.instanciateEntity(PollAttributes.class)).findAllByColumn("poll_id",""+poll.getID());
                if (pollAtt != null) {
                  if (pollAtt.length > 0) {
                    for (int i = 0 ; i < pollAtt.length ; i++) {
                      pollAtt[i].delete();
                    }
                  }
                }
...
//                getRequest().setParameter("submit","");
//               main();
