      ...
      if ( memberList != null ) {
        for ( int a = 0; a < memberList.size(); a++ ) {
          try {
            ...
            Scorecard[] scorecardBefore = (Scorecard[]) Scorecard.getStaticInstance("is.idega.idegaweb.golf.entity.Scorecard").findAll("select * from scorecard where scorecard_date < '"+dateBefore.toSQLDateString()+"' and scorecard_date is not null and member_id = "+Integer.toString(member.getID())+" order by scorecard_date desc");
            Scorecard[] scorecardAfter = (Scorecard[]) Scorecard.getStaticInstance("is.idega.idegaweb.golf.entity.Scorecard").findAll("select * from scorecard where scorecard_date < '"+dateAfter.toSQLDateString()+"' and scorecard_date > '"+dateBefore.toSQLDateString()+"' and scorecard_date is not null and member_id = "+Integer.toString(member.getID())+" order by scorecard_date desc");
