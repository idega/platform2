...
public class UserIdEditor extends Editor {
  ...
  private void doMakeIds(IWContext iwc) throws Exception{
   ...
   private void doTrulyMakeIds(IWContext iwc) throws Exception{
    ...
    UserIds U = ((UserIds[])new UserIds().findAllByColumn("union_id",id))[0];
    UserId userid = null;
  ...
  private UserIds[] getUserIds()throws SQLException{
    UserIds[] U = (UserIds[])new UserIds().findAll();
    return U;
  }

  private Union[] getUnions() throws SQLException{
    Union[] u = (Union[])new Union().findAllOrdered("abbrevation");
    return u;
  }
  private Union[] getUnion(int unionid)throws SQLException{
    Union[] u = new Union[1];
    u[0] = new Union(unionid);
    return u;
  }
  ...
  private UnionMemberInfo[] getUMIs(int iUnionId)throws SQLException{
    UnionMemberInfo[] umis = (UnionMemberInfo[])new UnionMemberInfo().findAllByColumn("union_id",String.valueOf(iUnionId),"member_status","A");
    return umis;
  }
