...
public class TournamentController
{
	...
	public static Tournament[] getNextTwoTournaments() throws Exception
	{
		...
		Tournament[] tourns =
			(Tournament[]) tournament.findAll(
				"select * from tournament where start_time>'" + stamp.toSQLDateString() + "' order by start_time",
				2);
		return tourns;
	}
	...
	public static Tournament[] getLastTwoTournaments() throws Exception
	{
		...
		Tournament[] tourns =
			(Tournament[]) tournament.findAll(
				"select * from tournament where start_time<'" + stamp.toSQLDateString() + "' order by start_time desc",
				2);
		return tourns;
	}
	...
	public static Tournament[] getTournaments(IWTimestamp stamp) throws Exception
	{
		...
		Tournament[] tourns =
			(Tournament[]) tournament.findAll(
				"select * from tournament where start_time>= '"
					+ stamp.toSQLDateString()
					+ "' AND start_time<'"
					+ next.toSQLDateString()
					+ "' order by start_time");
		return tourns;
	}
	...
	public static int getTotalStrokes(Tournament tournament, TournamentRound round, is.idega.idegaweb.golf.entity.Member member)
		throws Exception
	{
		int totalStrokes = -1;
	...
	public static Tournament[] getLastClosedTournaments(int number) throws Exception
	{
		...
		Tournament[] tourns =
			(Tournament[]) tournament.findAll(
				"select * from tournament where closed = 'Y' and closed_date<'" + stamp.toSQLString() + "' order by closed_date desc",
				number);
		return tourns;
	}
	...
	public static void removeTournamentTableApplicationAttribute(IWContext modinfo)
	{
	...
	public static void removeTournamentBoxApplication(IWContext modinfo)
	{
		modinfo.removeApplicationAttribute("tournament_box");
	}

	public static TournamentBox getTournamentBox(IWContext modinfo)
	{
	...
	public static DropdownMenu getDropdownOrderedByUnion(DropdownMenu menu, IWContext modinfo)
	{
		return getDropdownOrderedByUnion(menu, modinfo, 2003);
	}

	public static DropdownMenu getDropdownOrderedByUnion(DropdownMenu menu, IWContext modinfo, int year)
	{
		try
		{
			...
			DropdownMenu applicationMenu =
				(DropdownMenu) modinfo.getApplicationAttribute(
					"tournament_dropdownmenu_ordered_by_union_clubadmin_"
						+ clubAdmin
						+ "_union_id_"
						+ union_id
						+ "_menuName_"
			...
			if (applicationMenu == null)
			{
				if (clubAdmin)
				{
					...
					if (union_id != 1)
					{
						...
						{
							tours =
								(Tournament[]) ((Tournament) IDOLookup.instanciateEntity(Tournament.class)).findAll(
									"select * from tournament where union_id = "
										+ union_id
										+ " and START_TIME < '"
										+ year
										+ "-12-31' and START_TIME > '"
										+ year
										+ "-01-01' order by START_TIME");
						}
				...
				modinfo.setApplicationAttribute(
					"tournament_dropdownmenu_ordered_by_union_clubadmin_"
						+ clubAdmin
						+ "_union_id_"
						+ union_id
	...
	public static List getMembersInTournamentGroup(Tournament tournament, TournamentGroup tourGroup, Vector members_to_check)
		throws SQLException
	{
	...
	public static List getMembersInTournamentGroup(
		Tournament tournament,
		TournamentGroup tourGroup,
		is.idega.idegaweb.golf.entity.Member[] members_to_check)
		throws SQLException
	{
			...
			try
			{
				tournamentGroupMembers =
					EntityFinder.findAll(
							(Member) IDOLookup.instanciateEntity(Member.class),
						"SELECT member.* from member, tournament_member where member.member_id = tournament_member.member_id and tournament_member.tournament_id = "
							+ tournament.getID()
							+ " AND tournament_member.tournament_group_id = "
							+ tourGroup.getID());
	...
	public static List getTournamentRoundMembersList(int tournament_round_id) throws SQLException
	{
		...
		try
		{
			members =
				EntityFinder.findAll(
						(TournamentParticipants) IDOLookup.instanciateEntity(TournamentParticipants.class),
					"SELECT * from tournament_round_participants where tournament_round_id = "
						+ tournament_round_id
						+ " ORDER by grup_num");
		}
		catch (Exception ex)
	...
	public static is.idega.idegaweb.golf.entity.TournamentRoundParticipants[] getTournamentRoundMembers(int tournament_round_id)
		throws SQLException
	{
		is.idega.idegaweb.golf.entity.TournamentRoundParticipants[] members =
			(is.idega.idegaweb.golf.entity.TournamentRoundParticipants[])
				((TournamentRoundParticipants) IDOLookup.instanciateEntity(TournamentRoundParticipants.class)).findAll(
				"SELECT * from tournament_round_participants where tournament_round_id = " + tournament_round_id + " ORDER by grup_num");
		return members;
	}
	...
	public static boolean isMemberInTournamentGroup(is.idega.idegaweb.golf.entity.Member member, TournamentGroup tourGroup)
		throws SQLException
	{
		boolean young = false;
		boolean old = false;
		...
		if ((tourGroup.getGender() == member.getGender().charAt(0)) || (tourGroup.getGender() == 'B'))
			sex = true;

		if (sex)
		{
			double memberHandicap = member.getHandicap();
			if (memberHandicap == 100)
				memberHandicap = 36;
			if (tourGroup.getMinHandicap() <= memberHandicap)
				minHand = true;
			if (minHand)
			{
				if (tourGroup.getMaxHandicap() >= memberHandicap)
					maxHand = true;
				if (maxHand)
				{
					int memberAge = member.getAge(); //getAge(member);
					if (tourGroup.getMinAge() <= memberAge)
						young = true;
					if (young)
					{
						if (tourGroup.getMaxAge() >= memberAge)
							old = true;
					}
				}
			}
	...
	public static int[] isMemberAllowedToRegister(is.idega.idegaweb.golf.entity.Member member, Tournament tournament) throws SQLException
	{
		...
		try
		{
			String[] socials =
				SimpleQuerier.executeStringQuery(
					"SELECT MEMBER_ID FROM MEMBER WHERE SOCIAL_SECURITY_NUMBER = '" + member.getSocialSecurityNumber() + "' ");
			if (socials.length > 1)
			{
				errors[0] = 1;
			}
		...
		try
		{
			String[] umi =
				SimpleQuerier.executeStringQuery(
					"Select union_member_info_id from union_member_info where member_id = "
						+ member.getID()
						+ " and MEMBER_STATUS = 'A' and MEMBERSHIP_TYPE = 'main'");
			if (umi.length != 1)
			{
				errors[1] = 1;
			}
	...
	public static boolean setupStartingtime(
		IWContext modinfo,
	...
	public static boolean blockStartingtime(IWContext modinfo, String name,int tournament_round_id,
		int startingGroup, int finishingGroup) throws SQLException 
	{
		...
		int howManyEachGroup = tournament.getNumberInGroup();
		//System.out.println(" ... startingGroup ="+startingGroup+", finishingGroup ="+finishingGroup+", howManyEachGroup = "+howManyEachGroup);

		for (int a = startingGroup; a <= finishingGroup; a++)
		{
			for (int b = 1; b <= howManyEachGroup; b++)
			{
				//System.out.println(" ... blocking groupNumber ="+a+", currentTimeWithingGroup +"+b);
				Startingtime startingtime = (Startingtime) IDOLookup.createLegacy(Startingtime.class);
				startingtime.setFieldID(tournament.getFieldId());
				//System.out.println(" ... fieldId = "+tournament.getFieldId());
				startingtime.setStartingtimeDate(new IWTimestamp(tourRound.getRoundDate()).getSQLDate());
				//System.out.println(" ... date = "+(new idegaTimestamp(tourRound.getRoundDate()).getSQLDate()).toString());
	...
	public static boolean unblockStartingtime(IWContext modinfo, int tournament_round_id, int startingGroup, int finishingGroup)
		throws SQLException
	{
		TournamentRound tourRound = null;
		try {
		...
		String SQLString =
			"SELECT * FROM TOURNAMENT_ROUND_STARTINGTIME, STARTINGTIME WHERE TOURNAMENT_ROUND_STARTINGTIME.TOURNAMENT_ROUND_ID = "
				+ tournament_round_id
				+ " AND STARTINGTIME.STARTINGTIME_ID = TOURNAMENT_ROUND_STARTINGTIME.STARTINGTIME_ID AND FIELD_ID ="
				+ tournament.getFieldId()
				+ " AND STARTINGTIME_DATE = '"
				+ stamp.toSQLDateString()
				+ "' and member_id = 1 and grup_num >= "
				+ startingGroup
				+ " and grup_num <="
				+ finishingGroup;
	...
	public static boolean isMemberRegisteredInTournament(Tournament tournament, is.idega.idegaweb.golf.entity.Member member)
		throws SQLException
	{
		boolean returner = false;
		List listi =
			EntityFinder.findAll(
				(Member) IDOLookup.instanciateEntity(Member.class),
				"Select member.* from member,tournament_member where member.member_id = tournament_member.member_id AND member.member_id = "
					+ member.getID()
					+ " AND tournament_member.tournament_id = "
					+ tournament.getID()
					+ "");
		if (listi != null)
		{
	...
	public static boolean isMemberRegisteredInTournament(
		Tournament tournament,
		...
		throws SQLException
	{
		...
		Startingtime[] startingtimes =
			(Startingtime[]) ((Startingtime) IDOLookup.instanciateEntity(Startingtime.class)).findAll(
				"SELECT startingtime.* FROM STARTINGTIME, tournament_round_STARTINGTIME WHERE tournament_round_startingtime.tournament_round_id = "
					+ tourRound.getID()
					+ " AND tournament_round_startingtime.startingtime_id = startingtime.startingtime_id AND STARTINGTIME_DATE = '"
					+ startStamp.toSQLDateString()
					+ "' AND field_id="
					+ tournament.getFieldId()
					+ " AND member_id = "
					+ member.getID());
	...
	public static List getMembersInStartingGroup(Tournament tournament, TournamentRound tournamentRound, int startingGroupNumber)
	{
		...
		{
			members =
				com.idega.data.EntityFinder.findAll(
					(Member) IDOLookup.instanciateEntity(Member.class),
					"SELECT member.* FROM member,STARTINGTIME, TOURNAMENT_ROUND_STARTINGTIME WHERE TOURNAMENT_ROUND_STARTINGTIME.tournament_round_id = "
						+ tournamentRound.getID()
						+ " AND TOURNAMENT_ROUND_STARTINGTIME.startingtime_id = startingtime.startingtime_id AND member.member_id = startingtime.member_id AND STARTINGTIME_DATE = '"
						+ SQLDate
						+ "' AND grup_num ="
						+ startingGroupNumber
						+ " AND field_id="
	...
	public static List getUnionTournamentGroups(IWContext modinfo) throws SQLException
	{
		try {
			Union union =
				((is.idega.idegaweb.golf.entity.Member) AccessControl.getMember(modinfo)).getMainUnion();
	...
	public static List getUnionTournamentGroups(Union union) throws SQLException
	{
		...
		if (union != null)
		{
			...
			else
			{
				tGroup =
					EntityFinder.findAll(
						(TournamentGroup) IDOLookup.instanciateEntity(TournamentGroup.class),
						"SELECT * FROM TOURNAMENT_GROUP WHERE union_id =" + union.getID() + " OR union_id = 3 ORDER BY name");
			}
		}
		else
		{
	...
	public static SubmitButton getAheadButton(IWContext modinfo, String name, String value)
	{
		IWMainApplication iwma = modinfo.getIWMainApplication();
		IWBundle iwb = iwma.getBundle(TournamentController.getBundleIdentifier());
		IWResourceBundle iwrb = iwb.getResourceBundle(modinfo.getCurrentLocale());
	...
	public static Form getStartingtimeTable(
		IWContext modinfo,
		Tournament tournament,
		...
		throws SQLException
	{
		return TournamentController.getStartingtimeTable(
			modinfo,
			tournament,
			tournament_round_id,
			viewOnly,
	...
	public static Form getStartingtimeTable(
		...
		throws SQLException
	{
		return TournamentController.getStartingtimeTable(modinfo, tournament, tournament_round_id, viewOnly, false, true, iwrb);
	}
		...
		throws SQLException
	{
		...
		if (cachedForm != null && !onlineRegistration)
		{
			...
			//THE PROBLEM IS THAT THE form.maintainParameter("action") is causing a hiddenInput(action) 
			//being added endlessly to the form from the cache and therefor ruining the online registration 
			//because the last action never changes (unless you restart) 
			//lower in the method we also don't add to the cache is onlineRegistration is true

			//System.out.println("Getting startingtime table from cache: "+cacheString);
			return cachedForm;
		}
		else
		{
			...
			if (!onlineRegistration)
			{
				for (int i = 0; i < tourRounds.length; i++)
				{
					...
					rounds.addMenuElement(
						tourRounds[i].getID(),
						iwrb.getLocalizedString("tournament.round", "Round")
							+ " "
							+ tourRounds[i].getRoundNumber()
							+ " "
							+ tourDay.getISLDate(".", true));
				}
			...
			if (forPrinting)
			{
				Text tepText = (Text) textLook.clone();
				Text cepText = (Text) textLook.clone();
				tourDay = new IWTimestamp(tournamentRound.getRoundDate());
				tepText.setText(
					iwrb.getLocalizedString("tournament.round", "Round")
						+ " "
						...
						+ " "
						+ tourDay.getISLDate(".", true));
				cepText.setText(tournament.getName());
			...
			{
				Text paid = (Text) headerLook.clone();
				paid.setText(iwrb.getLocalizedString("tournament.paid", "Paid"));
			...
			{
				tTime.setFontStyle(
					"letter-spacing:1px;font-family:Arial,Helvetica,sans-serif;background-color:#FFFFFF;font-size:18px;color:#000000;border-width:1px;font-weight:bold;border-style:solid;");
			}
			else
			{
				tTime.setFontStyle(
					"letter-spacing:1px;font-family:Arial,Helvetica,sans-serif;background-color:#FFFFFF;font-size:18px;color:#2C4E3B;border-width:1px;font-weight:bold;border-style:solid;");
			}
				...
				while (endHour.isLaterThan(startHour))
				{
					...
					timeText.setText(
						Text.NON_BREAKING_SPACE
							...
							+ ":"
							+ extraZero.format(startHour.getMinute())
							+ Text.NON_BREAKING_SPACE);
					table.add(timeText, 1, row);
					//time = new Flash("http://jgenerator.sidan.is/time.swt?type=gif&grc=true&time=" + extraZero.format(startHour.getHour()) + ":" + extraZero.format(startHour.getMinute()), 60, 38);
					...
					sView =	TournamentController.getStartingtimeView(
							tournamentRound.getID(),
							"",
							"",
							"grup_num",
					...
					for (int i = 0; i < sView.length; i++)
					{
						...
						if (display)
						{
							...
							{
								dMemberSsn.setText("-");
								dMemberName.setText(iwrb.getLocalizedString("tournament.reserved", "Frátekið"));
								dMemberUnion.setText("-");
								dMemberHand.setText("-");
							}
							...
							if (!viewOnly)
							{
								if (!onlineRegistration)
								{
									paid = new CheckBox("paid", Integer.toString(sView[i].getMemberId()));
					...
					for (int i = startInGroup; i < (numberInGroup); i++)
					{
						...
						if ((!viewOnly) && (roundNumber == 1))
						{
							...
							socialNumber.setStyleAttribute("font-size: 8pt");
					...
					*/
					...
					if (forPrinting)
					{
						...
						table.add(hr, 1, row);
						//table.setColor(1, row, headerColor);
					}
					else
					{
						--row;
			...
			//System.out.println("Setting startingtime table to cache: "+cacheString);
			//REMOVE CHECKK SEE COMMENT IN THE TOP OF THIS METHOD (EIKI)
			if (!onlineRegistration)
				modinfo.setApplicationAttribute(cacheString, form);

			return form;
		}
	}

	public static boolean hasMemberStartingtime(
		Tournament tournament,
		TournamentRound tourRound,
		is.idega.idegaweb.golf.entity.Member member)
	{
		...
		try
		{
			...
			Startingtime[] startingtimes =
				(Startingtime[]) ((Startingtime) IDOLookup.instanciateEntity(Startingtime.class)).findAll(
					"SELECT * FROM STARTINGTIME WHERE STARTINGTIME_DATE = '"
						+ startStamp.toSQLDateString()
						+ "' AND member_id ="
						+ member.getID()
						+ " AND field_id="
	...
	public static int getNextAvailableStartingGroup(
		Tournament tournament,
		TournamentRound tourRound,
		...
		int minimumGroupNumber)
	{
		...
		try
		{
			...
			while (!done)
			{
				...
				startingtimes =
					(Startingtime[]) ((Startingtime) IDOLookup.instanciateEntity(Startingtime.class)).findAll(
						"SELECT * FROM STARTINGTIME s, TOURNAMENT_ROUND_STARTINGTIME trs WHERE trs.startingtime_id = s.startingtime_id AND trs.tournament_round_id = "
							+ tourRound.getID()
							+ " AND s.STARTINGTIME_DATE = '"
							+ startStamp.toSQLDateString()
							+ "'  AND s.field_id="
							+ tournament.getFieldId()
							+ " AND s.grup_num="
	...
	public static List getStartingtimeOrder(Tournament tournament, TournamentRound tournamentRound)
	{
		...
		try
		{
			...
			members =
				EntityFinder.findAll(
					(Member) IDOLookup.instanciateEntity(Member.class),
					"SELECT member.*,grup_num from startingtime, member, tournament_round_startingtime, tournament, tournament_member where tournament.tournament_id = "
						+ tournament.getID()
						+ " and tournament.tournament_id = tournament_member.tournament_id AND tournament_round_startingtime.tournament_round_id = "
						+ tournamentRound.getID()
						+ " AND tournament_round_startingtime.startingtime_id = startingtime.startingtime_id AND startingtime.field_id = "
						+ tournament.getFieldId()
						+ " AND STARTINGTIME.STARTINGTIME_DATE >= '"
						+ startStamp.toSQLDateString()
						+ "' AND STARTINGTIME.STARTINGTIME_DATE <= '"
						+ endStamp.toSQLDateString()
						+ "' AND member.member_id = startingtime.member_id and member.member_id = tournament_member.member_id ORDER by grup_num");
		}
		catch (Exception ex)
		{
			ex.printStackTrace(System.err);
	...
	public static DropdownMenu getAvailableGrupNums(String dropdownName, Tournament tournament, TournamentRound tRound) throws SQLException
	{
		...
		while (end.isLaterThan(start))
		{
			...
			if (displayTee)
			{
				menu.addMenuElement(
					grupNum,
					extraZero.format(start.getHour()) + ":" + extraZero.format(start.getMinute()) + "&nbsp;&nbsp; (1)");
				menu.addMenuElement(
					grupNum + "_",
					extraZero.format(start.getHour()) + ":" + extraZero.format(start.getMinute()) + "&nbsp;&nbsp;  (10)");
			}
			else
			{
	...
	public static int registerMember(is.idega.idegaweb.golf.entity.Member member, Tournament theTournament, String tournament_group_id)
		throws SQLException
	{
		int returner = 0;
		try
	...
	public static void createScorecardForMember(is.idega.idegaweb.golf.entity.Member member, Tournament tournament) throws SQLException
	{
		try
		{
			...
			TournamentTournamentGroup[] tTGroup =
				(TournamentTournamentGroup[]) ((TournamentTournamentGroup) IDOLookup.instanciateEntity(TournamentTournamentGroup.class)).findAllByColumn(
					"tournament_id",
					tournament.getID() + "",
					"tournament_group_id",
					tournament_group_id + "");
	...
	public static void createScorecardForMember(
		is.idega.idegaweb.golf.entity.Member member,
		Tournament tournament,
		String tournament_group_id)
		throws SQLException
	{
		...
		throws SQLException
	{
		...
		{
			Tee[] tee =
				(Tee[]) ((Tee) IDOLookup.instanciateEntity(Tee.class)).findAllByColumn(
					"field_id",
					field.getID() + "",
					"tee_color_id",
					tTGroup.getTeeColorId() + "",
					"hole_number",
					"1");
			if (tee.length > 0)
			{
		...
		try
		{
			...
			playingHandicap =
				(float) handicapper.getLeikHandicap((double) slope, (double) CR, (double) tournament.getField().getFieldPar());
	...
	public static TournamentParticipants[] getTournamentParticipants(String column_name, String column_value, String order)
		throws SQLException
	{
		...
		String queryString =
			"select m.member_id, m.social_security_number, m.first_name, m.middle_name,"
				...
				+ " s.total_points, sum(str.hole_par) total_par, sum(str.stroke_count) - sum(str.hole_par) difference, tg.name as group_name, tm.paid"
				...
				+ " = "
				+ column_value
				+ " group by m.member_id, m.social_security_number, m.first_name, m.middle_name, m.last_name, u.abbrevation, tm.tournament_id, tm.tournament_group_id,s.scorecard_id, s.scorecard_date, f.field_par,tr.tournament_round_id, tr.round_number, s.total_points, s.handicap_before, s.slope, s.course_rating, tg.name,tm.paid"
				+ " order by "
				+ order;

		//String queryString = "select * from tournament_participants where "+column_name+"="+column_value+" order by "+order;
	...
	public static TournamentRoundParticipants[] getTournamentRoundParticipants(String column_name, String column_value, String order)
		throws SQLException
	{
		...
		String queryString =
			"SELECT m.member_id, m.social_security_number, m.first_name, m.middle_name,"
				...
				+ " s.total_points, sum(str.hole_par) as total_par, sum(str.stroke_count) - sum(str.hole_par) as difference, start.grup_num, tg.name as group_name, tm.paid"
				+ " FROM tournament_round tr,member m, field f, union_ u, tournament_member tm, tournament t," 
				+ " tournament_group tg, startingtime start, tournament_ROUND_startingtime ts, scorecard s"
				+ " LEFT JOIN stroke str on str.scorecard_id = s.scorecard_id"
				+ " WHERE s.tournament_round_id = tr.tournament_round_id"
				+ " AND tr.tournament_id = t.tournament_id"
				+ " AND tm.union_id = u.union_id"
				+ " AND t.tournament_id = tm.tournament_id"
				+ " AND tm.member_id = m.member_id"
				+ " AND s.member_id = m.member_id"
				+ " AND t.field_id = f.field_id"
				+ " AND tR.tournament_ROUND_id = ts.tournament_ROUND_id" 
				+ " AND ts.startingtime_id = start.startingtime_id"
				...
				+ " AND m.member_id = start.member_id"
				+ " AND tm.tournament_group_id = tg.tournament_group_id"
				+ " AND "
				+ column_name
				+ " = "
				+ column_value
				+ " GROUP BY m.member_id, m.social_security_number, m.first_name, m.middle_name, m.last_name, u.abbrevation, tm.tournament_id, tm.tournament_group_id,s.scorecard_id, s.scorecard_date, f.field_par,tr.tournament_round_id, tr.round_number, s.total_points, s.handicap_before, s.slope, s.course_rating, start.grup_num, tg.name,tm.paid"
				+ " ORDER BY "
				+ order;

		//String queryString = "select * from tournament_round_participants where "+column_name+"="+column_value+" order by "+order;
		TournamentRoundParticipants[] tournParticipants =
			(TournamentRoundParticipants[]) ((TournamentRoundParticipants) IDOLookup.instanciateEntity(TournamentRoundParticipants.class)).findAll(queryString);

		return tournParticipants;

	}

	public static StartingtimeView[] getStartingtimeView(
		int tournament_round_id,
		String column_name,
		String column_value,
		...
		throws SQLException
	{
		return TournamentController.getStartingtimeView(
			tournament_round_id,
			column_name,
			column_value,
			column_name_1,
	...
	private static String getStartingtimeViewSql(int tournament_round_id, String column_name, String column_value, String column_name_1, String column_value_1, int tee_number, String order, int first_group_number, int last_group_number, boolean onlyReserved) {
		String queryString =
			"select distinct (st.startingtime_id), st.field_id, st.member_id, st.startingtime_date,st.grup_num, "
				...
				queryString = queryString + " from startingtime st, member m, union_member_info umi, union_ u, member_info mi, tournament_round_startingtime trs, tournament_round tr , tournament_member tm where "
	...
	public static void removeMemberFromTournament(IWContext modinfo, Tournament tournament, is.idega.idegaweb.golf.entity.Member member)
	{
		removeMemberFromTournament(modinfo, tournament, member, -10);
	}

	public static void removeMemberFromTournament(
		IWContext modinfo,
		Tournament tournament,
		is.idega.idegaweb.golf.entity.Member member,
		int startingGroupNumber)
	{
		...
		try
		{
			...
			for (int j = 0; j < tRounds.length; j++)
			{
				...
				{
					SQLString =
						"SELECT * FROM STARTINGTIME WHERE FIELD_ID ="
							+ tournament.getFieldId()
							+ " AND MEMBER_ID = "
							+ member_id
							+ " AND GRUP_NUM = "
				...
				scorecards =
					(Scorecard[]) ((Scorecard) IDOLookup.instanciateEntity(Scorecard.class)).findAllByColumn("MEMBER_ID", member_id, "TOURNAMENT_ROUND_ID", tRounds[j].getID() + "");
				if (scorecards != null)
				{
	...
	public static boolean getHasMemberPaid(Tournament tournament, Member member) throws Exception
	{
		String[] repps =
			SimpleQuerier.executeStringQuery(
				"select paid from tournament_member where member_id = " + member.getID() + " AND tournament_id = " + tournament.getID());
		if (repps != null && repps.length > 0 && repps[0] != null)
		{
			return "Y".equalsIgnoreCase(repps[0]);
		}
		return false;

	}
	public static void setAllMemberToNotPaid(Tournament tournament) throws Exception
	{
		SimpleQuerier.execute("update tournament_member set paid = 'N' where tournament_id = " + tournament.getID());
	}

	public static boolean setHasMemberPaid(Tournament tournament, Member member, boolean hasPaid) throws Exception
	{
		String sPaid = "Y";
		if (!hasPaid)
		{
		...
		return SimpleQuerier.execute(
			"update tournament_member set paid = '"
				+ sPaid
				+ "' where member_id = "
				+ member.getID()
				+ " AND tournament_id = "
	...
	public static void invalidateStartingTimeCache(IWContext modinfo, Tournament tournament) throws SQLException
	{
		TournamentRound[] tRounds = tournament.getTournamentRounds();
	...
	public static void invalidateStartingTimeCache(IWContext modinfo, int tournamentID, String tournamentRoundID)
	{
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + false +"_" + false +"_" + false);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + true +"_" + false +"_" + false);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + true +"_" + true +"_" + false);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + true +"_" + false +"_" + true);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + true +"_" + true +"_" + true);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + false +"_" + true +"_" + false);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + false +"_" + true +"_" + true);
		modinfo.removeApplicationAttribute(
			"tournament_startingtime_" + tournamentID + "_" + tournamentRoundID + "_" + false +"_" + false +"_" + true);
