 ...
 * @author laddi
 */
public class MemberEditor extends GolfBlock {
	...
	public void main(IWContext modinfo) throws Exception {
		...
		Link general = new Link(iwrb.getImage("member/general.gif", "Almennar upplýsingar kylfingsins"));
		general.addParameter("member_id", member_id);
		Link finance = new Link(iwrb.getImage("member/finance.gif", "Fjármál kylfingsins"), "/tarif/accountview.jsp");
		finance.addParameter("member_id", member_id);
		Link handicap = new Link(iwrb.getImage("member/handicap_overview.gif", iwrb.getLocalizedString("member.handicap", "Handicap")));
		handicap.addParameter("member_id", member_id);
		handicap.addParameter("menuaction", "handicap");
		Link history = new Link(iwrb.getImage("member/history.gif", "Saga kylfingsins"));
		history.addParameter("member_id", member_id);
		history.addParameter("menuaction", "history");
		Window commentWindow = new Window("Athugasemd", 400, 300, "/clubs/membercomment.jsp");
		Link comment = new Link(iwrb.getImage("member/comments.gif", "Athugasemd"), commentWindow);
		comment.addParameter("member_id", member_id);
		comment.addParameter("union_id", union_id);
		Window loginWindow = new Window("Login", 200, 270, "/clubs/memberlogin.jsp");
		loginWindow.setScrollbar(false);
		Link login = new Link(iwrb.getImage("member/password.gif", "Lykilorð"), loginWindow);
		login.addParameter("member_id", member_id);
		login.addParameter("union_id", union_id);
		Window clubsWindow = new Window("Athugasemd", 250, 250, "/clubs/unioncorrect.jsp");
		clubsWindow.setScrollbar(false);
		Link clubs = new Link(iwrb.getImage("member/clubs.gif", "Klúbbar"), clubsWindow);
		...
		if (action == null) {
			...
			if (doSocialSecuritySearchOnNewMember && ((member_id == null) && (imagecheck == null))) {
				Table tafel = new Table(2, 2);
				Form socialForm = new Form();
				TextInput input = new TextInput("socialnumber");
				String text1 = iwrb.getLocalizedString("member.newmembermessage", "Enter the SocialSecurityNumber of the new member");
				//Text texti = new Text("Skrifaðu inn kennitölu nýja félagans");
				Text texti = new Text(text1);
				texti.setBold();
				HiddenInput actionInput = new HiddenInput("menuaction", "checksocial");
			...
			if ((socialnumber != null) && !(socialnumber.equalsIgnoreCase(""))) {
				List members = EntityFinder.findAllByColumn((Member) IDOLookup.instanciateEntity(Member.class), "social_security_number", socialnumber);
				if (members != null) {
			...
			if (member_id != null)
				add(new HandicapOverview(member_id));
		}
			...
			if (member_id != null) {
				...
				Text texti = new Text(iwrb.getLocalizedString("member.history", "History of") + " " + member.getName());
	  ...
	  public void main2(String memberString , String unionString, IWContext modinfo)throws IOException {
	      ...
	      if(unionString == null) {
	          ...
	          vError.add("Félagsnúmer, Session-ið er útrunnið");
	          modinfo.getSession().setAttribute("error", vError);
	          //modinfo.getResponse().sendRedirect("membererror.jsp");
	          getParentPage().setToRedirect("membererror.jsp");
	  ...
	  public class GolfMemberInsert {
	      ...
	      public GolfMemberInsert(IWContext modinfo, int unionId)throws SQLException {
	      ...
	      public GolfMemberInsert(IWContext modinfo, int memberId, int unionId)throws SQLException, FinderException {
	        this.modinfo = modinfo;
	        memberID = memberId;
	        this.iMemberId = memberID;
	        this.iUnionId = unionId;
	        eUMIs = EntityFinder.findAllByColumn((UnionMemberInfo) IDOLookup.instanciateEntity(UnionMemberInfo.class),"member_id",iMemberId);
	        isUpdate = true;
	        strUnionId = String.valueOf(unionId);
	        ...
	        if(addressLenght > 1)
	            addrInsert2 = new AddressInsert(modinfo, addressArr[1].getID(), "heimili2", "heimililand2", "postnr2");
	        else
	            addrInsert2 = new AddressInsert(modinfo, "heimili2", "heimililand2", "postnr2");
	  ...
	  public void showInputForm()throws IOException, SQLException, FinderException {
...
//	    imageInsert.setDefaultImageURL(sMemberImageURL);
	    ...
	    if(member != null)
	      form.add(new HiddenInput("member_id",String.valueOf(member.getID())));
	      /*
	    ...
	    */
	    ...
	    T.add(headerText(iwrb.getLocalizedString("member.photo","Photo")),1,1);
	    T.add(imageObject,1,1);
	    T.add(headerText(iwrb.getLocalizedString("member.handicap","Handicap")),1,2);
	    T.add(memberInfoTable,1,2);

	    leftTable.add(T, 1, 1);
	    leftTable.add(headerText(iwrb.getLocalizedString("member.clubs","Clubs")),1,3);
	    leftTable.add(unionsTable,1,3);

	    SubmitButton save = new SubmitButton(iwrb.getImage("/buttons/save.gif"),"save");
	...
	/*
	    String type = unionMemberInsert.getUnionMemberInfo().getMembershipType();
	    if(type.equalsIgnoreCase("main"))
	      member.setMainUnion(iUnionId);
	*/
	/*
	...
	*/
	    ...
	    getParentPage().setToRedirect(modinfo.getRequest().getRequestURI()+"?&member_id="+member.getID());
	  }
	  ...
	  public void forwardPage(String strURL)
	  {
	      ...
	      public BorderTable getAddressTable() {
	          ...
	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.address","Heimili")), 1, 1);
	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.zipcode","Póstnr")), 1, 2);
	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.country","Land")), 1, 3);

	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.address2","Heimili 2")), 1, 4);
	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.zipcode","Póstnr")), 1, 5);
	          table.add(addrInsert.formatText(iwrb.getLocalizedString("member.country","Land")), 1, 6);
	  ...
	  public PresentationObject getGroupLinkTable() throws SQLException{
	    ...
	    //Link linkInsertGroup = new Link(iwrb.getImage("buttons/find.gif"), group);
	    //GroupMemberRemoveWindow groupRem = new GroupMemberRemoveWindow(eMember, uni, false);
	    //Link linkRemoveGroup = new Link(iwrb.getImage("buttons/detach.gif"),groupRem);
	    //Table T = new Table(2, 1);
	    //T.add(linkInsertGroup, 1, 1);
	    //T.add(linkRemoveGroup, 2, 1);
	    Link link = GroupMemberInsertWindow.getLink(iwrb.getImage("buttons/find.gif"),eMember.getID(),iUnionId,3);
	  ...
	  public PresentationObject getFamilyLinkTable() throws SQLException, FinderException{
	    ...
	    Link linkFindFamily = new Link(iwrb.getImage("buttons/find.gif"),findFam);
	    FamilyDisconnecterWindow winNewFamily = new FamilyDisconnecterWindow(iMemberId, iUnionId);
	    Link linkNewFamily = new Link(iwrb.getImage("buttons/detach.gif"),winNewFamily);
	  ...
	  public AddressInsert(IWContext modinfo, String addressInputName, String countryDropDownName, String zipCodeDropDownName) {
	      super(modinfo);
	      isUpdate = false;
	      address = (Address) IDOLookup.createLegacy(Address.class);
	  ...
	  public AddressInsert(IWContext modinfo, int addressId)throws java.sql.SQLException, FinderException {
	      super(modinfo, addressId);
	      isUpdate = true;
	      address = ((AddressHome) IDOLookup.getHomeLegacy(Address.class)).findByPrimaryKey(addressId);
	  ...
	  public AddressInsert(IWContext modinfo, int addressId, String addressInputName, String countryDropDownName, String zipCodeDropDownName)throws java.sql.SQLException, FinderException {
	      super(modinfo, addressId);
	      isUpdate = true;
	      address = ((AddressHome) IDOLookup.getHomeLegacy(Address.class)).findByPrimaryKey(addressId);
	      addressName = addressInputName;
	  ...
	  private DropdownMenu countryDropDown(String name, String selected) {
		...
		try {
	            Country country = ((CountryHome) IDOLookup.getHomeLegacy(Country.class)).findByPrimaryKey(COUNTRY_ID);
	  ...
	  private DropdownMenu zipDropDown(String name, String selected) {
	    ...
	    try {
	        ...
	        ZipCode [] zipArr = (ZipCode[]) zip.findAll("select * from zipcode where country_id="+COUNTRY_ID+" order by code");
	  ...
	  public void store(Member member)throws SQLException, IOException {
	      ...
	      else if((address.getStreet() != null) && (! address.getStreet().equals("")) ){
	          address.insert();
	...
	public class MemberInsert extends EntityInsert {
	  ...
	  public MemberInsert(IWContext modinfo) throws java.sql.SQLException{
	      ...
	      inputName.setAsNotEmpty(iwrb.getLocalizedString("member.please_insert_name","Please enter a name"));
	      inputSocial.setAsNotEmpty(iwrb.getLocalizedString("member.please_insert_ssn","Please enter Social Security Number"));
	  ...
	  public MemberInsert(IWContext modinfo, int memberId)throws SQLException, FinderException {
	  ...
	  public Vector getEmptyFields() {
	      ...
	      if ( isInvalid(inputSocialValue)) {
	          ...
	          vec.addElement(iwrb.getLocalizedString("member.wrong_ssn","Invalid social security number"));
	      }
	  ...
	  public BorderTable getInputTable() {
	      ...
	      table.add(formatText(iwrb.getLocalizedString("member.name","Nafn")), 1, 1);
	      table.add(formatText(iwrb.getLocalizedString("member.social_security_number","SSN")), 1, 2);
	      table.add(formatText(iwrb.getLocalizedString("member.gender","Kyn")), 1, 3);
	      table.add(formatText(iwrb.getLocalizedString("member.email","Netfang")), 1, 4);
	      table.add(formatText(iwrb.getLocalizedString("member.work_title","Starfsheiti")), 1, 5);
	      table.add(formatText(iwrb.getLocalizedString("member.workplace","Vinnustaður")), 1, 6);
	  ...
	  public void showInputForm() throws java.io.IOException, java.sql.SQLException {
	      PrintWriter out  = modinfo.getResponse().getWriter();
	      try {
	          ...
	          form.setAction(modinfo.getRequest().getRequestURI()+"?cmd=submit");
	          table.add("Nafn", 1, 1);
	          table.add(iwrb.getLocalizedString("member.social_security_number","SSN"), 1, 2);
	          table.add("Kyn", 1, 3);
	          ...
	          if(getValue("cmd") != null && getValue("cmd").equalsIgnoreCase("submit")) {
	              store();
	          }
	          form.add(table);
	          add(form);
	  ...
	  private void setEntity() {
	      ...
	      if (! isInvalid(inputSocialValue)) {
	          if(SocialSecurityNumber.isValidIcelandicSocialSecurityNumber(inputSocialValue)) {
	              ...
	              member.setDateOfBirth(SocialSecurityNumber.getDateFromSocialSecurityNumber(inputSocialValue));
	          }
	      }
	      if (! isInvalid(inputNameValue)) {
	          Name name = new Name(inputNameValue);
	  ...
	  public DropdownMenu genderDrop(String name, String selected) {
	          ...
	          drp.addMenuElement("F", iwrb.getLocalizedString("member.female","KVK"));
	          drp.setSelectedElement(selected);
	          return drp;
	  }
	}
	  ...
	  public UnionMemberInfoInsert(IWContext modinfo) throws java.sql.SQLException{
	      super(modinfo);
	      isUpdate = false;
	      ...
	      member = (UnionMemberInfo) IDOLookup.createLegacy(UnionMemberInfo.class);
	  ...
	  public UnionMemberInfoInsert(IWContext modinfo, int unionMemInfoId, int memberID, int unionID)throws SQLException, FinderException {
	      super(modinfo, unionMemInfoId);
	      isUpdate = true;
	      GregorianCalendar cal = new GregorianCalendar();
	      member = ((UnionMemberInfoHome) IDOLookup.getHomeLegacy(UnionMemberInfo.class)).findByPrimaryKey(unionMemInfoId);
	      eMember = ((MemberHome) IDOLookup.getHomeLegacy(Member.class)).findByPrimaryKey(memberID);
	      ...
	      if(member.getMembershipType()!=null){
	        dropMemberShipType = mbshiptypeDropdown(dropMemberShipTypeName,member.getMembershipType());
	  ...
	  public void store() throws java.io.IOException, java.sql.SQLException {
	    ...
	    public BorderTable getInputTable() {
	      ...
	      Table table = null;
				...
				Window pcWindow = new Window("Pricecatalogue",400,400,"/tarif/pcmake.jsp");
	      ...
	      if(isUpdate()) {
	        table = new Table(2, 8);
	        table.add(formatText(iwrb.getLocalizedString("member.number","Númer")), 1, 1);
	        table.add(formatText(iwrb.getLocalizedString("member.status","Staða")), 1, 2);
	        table.add(formatText(iwrb.getLocalizedString("member.payments","Greiðslur")), 1, 3);
	        table.add(formatText(iwrb.getLocalizedString("member.first_payment","1.Gjaldd.")), 1, 4);
	        //table.add(formatText("Greiðslumáti"), 1, 5);
	        table.add(formatText(iwrb.getLocalizedString("member.web","Vefur")), 1, 5);
	        table.add(formatText(iwrb.getLocalizedString("member.locker","Skápur")), 1, 6);
	        table.add(formatText(iwrb.getLocalizedString("member.family_status","Fj.staða")), 1, 7);
	        table.add(formatText(iwrb.getLocalizedString("member.special_price","Sérgjald")), 1, 8);
	  ...
	  public void setVariables() {
	    ...
	    mbshpChange = dropMemberShipTypeValue.equalsIgnoreCase(hmbshp)?false:true;
	  ...
	  private void setEntity() {
	      ...
	      if(mbshpChange && eMember != null){
	        ...
	        catch(SQLException ex){System.err.println("club changes fail");}
	      }
	  ...
	  public DropdownMenu mbshiptypeDropdown(String name,String selected){
	    DropdownMenu drp = new DropdownMenu(name);
	    drp.addMenuElement("main",iwrb.getLocalizedString("member.main_club","Aðalkl."));
	    drp.addMenuElement("sub",iwrb.getLocalizedString("member.extra_club","Aukakl."));
	    drp.setSelectedElement(selected);
	    return drp;
	  }
	  ...
	  public DropdownMenu familyStatusDropdown(String name, String selected) {
		DropdownMenu drp = new DropdownMenu(name);
		drp.addMenuElement("head", iwrb.getLocalizedString("member.head","Höfuð"));
		drp.addMenuElement("partner", iwrb.getLocalizedString("member.spouse","Maki"));
	        drp.addMenuElement("child", iwrb.getLocalizedString("member.child","Barn"));
	        drp.setSelectedElement(selected);
		return drp;
	  }
	  ...
	  public DropdownMenu visibleDropdown(String name, String selected) {
		DropdownMenu drp = new DropdownMenu(name);
		drp.addMenuElement("Y", iwrb.getLocalizedString("member.visible","Sjást á vef"));
		drp.addMenuElement("N", iwrb.getLocalizedString("member.not_visible","Ekki sjást á vef"));
	        drp.setSelectedElement(selected);
		return drp;
	  }
	...
	public class PhoneInsert extends EntityInsert {
	  ...
	  public PhoneInsert(IWContext modinfo, String inputPhoneName, String dropdownTypeName) {
	      super(modinfo);
	      isUpdate = false;
	      phone = (Phone) IDOLookup.createLegacy(Phone.class);
	      phone.setDefaultValues();
	  ...
	  public PhoneInsert(IWContext modinfo, String inputPhoneName, String countryDropName, String dropdownTypeName) {
	      super(modinfo);
	      isUpdate = false;
	      phone = (Phone) IDOLookup.createLegacy(Phone.class);
	      phone.setDefaultValues();
	  ...
	  public PhoneInsert(IWContext modinfo, int phoneId)throws java.sql.SQLException, FinderException {
	      super(modinfo, phoneId);
	      isUpdate = true;
	      phone = ((PhoneHome) IDOLookup.getHomeLegacy(Phone.class)).findByPrimaryKey(phoneId);
	      phone.setDefaultValues();
	  ...
	  public PhoneInsert(IWContext modinfo, int phoneId, String inputPhoneName, String countryDropName, String dropdownTypeName)throws java.sql.SQLException, FinderException {
	      super(modinfo, phoneId);
	      isUpdate = true;
	      phone = ((PhoneHome) IDOLookup.getHomeLegacy(Phone.class)).findByPrimaryKey(phoneId);
	      phone.setDefaultValues();
	  ...
	  public void store(Member member)throws SQLException, IOException {
	    ...
	    else if((phone.getNumber() != null) && (! phone.getNumber().equals("")) ){
	        phone.insert();
	        phone.addTo(member);
	    }
	  }
	  ...
	  public CardInsert(IWContext modinfo, int cardId)throws SQLException, FinderException {

	    super(modinfo, cardId);

	    card = ((CardHome) IDOLookup.getHomeLegacy(Card.class)).findByPrimaryKey(cardId);
	  ...
	  public boolean areAllFieldsEmpty()  {
	      ...
	      isEmpty = (isInvalid(expireMonthValue) && isInvalid(expireYearValue)/*expireValue == null*/ && isEmpty(ownarName) &&
	  ...
	  public boolean areSomeFieldsEmpty() {
	    return (isInvalid(expireMonthValue) || isInvalid(expireYearValue) || (ownarValue == null) ||
	      (numberValue == null) || (socialValue == null) || (typeValue == null) || ! ownarValue.equals("") ||
	...
	public Vector getNeetedEmptyFields(){return getEmptyFields();}
	...
	public Vector getEmptyFields() {
	    ...
	    if (isInvalid(socialValue)) {
	        vec.addElement(iwrb.getLocalizedString("member.social_security_number","SSN"));
	    }
	    if (isInvalid(typeValue)) {
	  ...
	  public void showInputForm()throws SQLException, IOException {
	      ...
	      form.add(table);
	      hTable.add(form);
	      table.add(iwrb.getLocalizedString("member.social_security_number","SSN"), 1, 1);
	    ...
	    public BorderTable getInputTable() {
	      ...
	      table.add(formatText(iwrb.getLocalizedString("member.social_security_number","Kennitala")), 1, 1);
	      table.add(getSocialSequrityNumberInput(), 2, 1);

	      table.add(formatText(iwrb.getLocalizedString("member.valid_through","Gildistími")), 1, 2);
	      table.add(getDropMonth(), 2, 2);
	      table.add(getDropYear(), 2, 2);

	      table.add(formatText(iwrb.getLocalizedString("member.card_owner","Korthafi")), 1, 3);
	      table.add(getNameInput(), 2, 3);

	      table.add(formatText(iwrb.getLocalizedString("member.card_number","Kortanúmer")), 1, 4);
	      table.add(getNumberInput(), 2, 4);

	      table.add(formatText(iwrb.getLocalizedString("member.card_type","Tegund")), 1, 5);
	      table.add(getTypeDrop(), 2, 5);
	  ...
	  private DropdownMenu yearDropDown(String name, String selected, int from, int to) {
		DropdownMenu drp = new DropdownMenu(name);
	        ...
	        drp.addMenuElement(String.valueOf(year), iwrb.getLocalizedString("member.year","Ár"));
	...
	public class FamilyDisconnecterWindow extends com.idega.presentation.ui.Window{
	  ...
	  private String headerText = iwrb.getLocalizedString("member.family.disconnect","Disconnect family ties");
	  private int unionId;


	  public FamilyDisconnecterWindow(int memberId, int unionId)throws java.sql.SQLException, FinderException {

	      setTitle(iwrb.getLocalizedString("member.family.findfamily","Find family"));
	  ...
	  public Form getInputTable(IWContext modinfo){
	      ...
	      try {
	          ...
	          Text t = new Text(iwrb.getLocalizedString("member.family.disconnectconfirm","Are you sure?"), true, false , false);
	          t.setFontColor("red");
	          ...
	          buttonTable.add(new SubmitButton(iwrb.getImage("buttons/confirm.gif"), STORE_NAME, "storeval"), 1, 1);
	          buttonTable.add(new CloseButton(iwrb.getImage("buttons/close.gif")), 2, 1);
	...
	public class FamilyInsertWindow extends com.idega.presentation.ui.Window{
	  ...
	  private String headerText = iwrb.getLocalizedString("member.family","Family");
	  private int unionId;
	  ...
	  public Form getInputTable(IWContext modinfo){
	      ...
	      try {
	          ...
	          table.add(new SubmitButton(iwrb.getImage("/buttons/search.gif"), FIND_NAME, "findval"), 1, 4);

	          buttonTable.add(new SubmitButton(iwrb.getImage("/buttons/register.gif"), STORE_NAME, "storeval"), 1, 1);
	          buttonTable.add(new CloseButton(iwrb.getImage("buttons/close.gif")), 2, 1);
	          ...
	          if(strStore != null) {
	            ...
	            if(strFind != null) {
	              ...
	              table.add(iwrb.getLocalizedString("member.family.numberofrecords","Number of records: ")+numRecords, 1, 1);
	  ...
	  private DropdownMenu getChoises(String name) {
	      DropdownMenu drp = new DropdownMenu(name);
	      drp.addMenuElement(NAME, iwrb.getLocalizedString("member.family.first_full_name"));
	      drp.addMenuElement(NAME_AND_MIDDLE,  iwrb.getLocalizedString("member.family.first_middle_name","First name and middle name"));
	      drp.addMenuElement(SOSIAL_SEC_NUM,  iwrb.getLocalizedString("member.social_security_number","SSN"));
	      drp.addMenuElement(ALL,  iwrb.getLocalizedString("member.family.all","All"));
	      return drp;
	  }
	  ...
	  private void setSelectionBox(List list)throws Exception {
	      ...
	      if((list == null) || list.isEmpty()) {
	          selectFamily.addMenuElement("", iwrb.getLocalizedString("member.family.nosearchresult","Nothing found"));
	      }
	      else if(list != null) {
	...
	public class MemberInfoInsert extends EntityInsert {
	  ...
	  private String headerText = iwrb.getLocalizedString("member.handicap","Handicap");
	  ...
	  public MemberInfoInsert(IWContext modinfo, int memInfoId)throws java.sql.SQLException, FinderException {
	      super(modinfo, memInfoId);
	      isUpdate = true;
	      memInfo = ((MemberInfoHome) IDOLookup.getHomeLegacy(MemberInfo.class)).findByPrimaryKey(memInfoId);
	      memInfo.setDefaultValues();
	      String h = com.idega.util.text.TextSoap.singleDecimalFormat((double)memInfo.getHandicap());
	      handicap = formatText(h);
	      handicap.setFontSize(6);
	      hiddenHandicap = new HiddenInput(inputHandicapName,String.valueOf(memInfo.getHandicap()));
	  ...
	  public Vector getEmptyFields() {
	      ...
	      if (isInvalid(inputHandicapValue)) {
	          vec.addElement(iwrb.getLocalizedString("member.handicap","Handicap"));
	      }
	  ...
	  public void showInputForm()throws SQLException, IOException {
	      ...
	      try {
	          ...
	          Form form = new Form();
	          form.setAction(modinfo.getRequest().getRequestURI()+"?cmd=submit");
	          table.add(iwrb.getLocalizedString("member.handicap","Handicap"), 1, 1);
	  ...
	  public BorderTable getInputTable() {
	      ...
	      if(isUpdate){
	        ...
	        Table table = new Table(1, 1);
	        hTable.add(table);
	        table.add(formatText(iwrb.getLocalizedString("member.handicap","Handicap")), 1, 1);
	  ...
	  public void store()throws SQLException, IOException {
	      ...
	      setVariables();
	      if(!isUpdate()){
	        IWTimestamp stamp = new IWTimestamp(new java.sql.Date(System.currentTimeMillis()));
	        memInfo.setHistory(stamp.toString()+": "+iwrb.getLocalizedString("member.member_added","Félagi nýskráður í kerfið"));
	  ...
	  public void setVariables() {
	      ...
	      if (! isInvalid(inputHandicapValue)) {
	          inputHandicapValue = com.idega.util.text.TextSoap.findAndReplace(inputHandicapValue,",",".");
