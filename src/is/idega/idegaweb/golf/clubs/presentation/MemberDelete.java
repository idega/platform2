 ...
 * @author laddi
 */
public class MemberDelete extends Editor {
	...
	public void delete(Member M) throws SQLException {
		...
		try {
			...
			try {
				...
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(M, address) + " where " + M.getIDColumnName() + "='" + getID() + "'");
			...
			try {
				...
				Stmt = conn.createStatement();
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(M, phone) + " where " + M.getIDColumnName() + "='" + M.getID() + "'");
			...
			try {
				...
				Stmt = conn.createStatement();
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(M, card) + " where " + M.getIDColumnName() + "='" + M.getID() + "'");
			...
			try {
				...
				Stmt = conn.createStatement();
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(tournament, M) + " where " + M.getIDColumnName() + "='" + M.getID() + "'");
			}
			catch (SQLException ex) {
				if (debug)
					ex.printStackTrace();
			...
			try {
				...
				Stmt = conn.createStatement();
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(group, M) + " where " + M.getIDColumnName() + "='" + M.getID() + "'");
			}
			catch (SQLException ex) {
				if (debug)
					ex.printStackTrace();
			...
			try {
				...
				Stmt = conn.createStatement();
				Stmt.executeUpdate("delete from " + M.getNameOfMiddleTable(union, M) + " where " + M.getIDColumnName() + "='" + M.getID() + "'");
			}
			catch (SQLException ex) {
				if (debug)
					ex.printStackTrace();
			 ...
			 */
			try {
				Scorecard[] scores = (Scorecard[]) ((Scorecard) IDOLookup.instanciateEntity(Scorecard.class)).findAllByColumn("member_id", M.getID());
			...
			try {
				...
				Startingtime[] times = (Startingtime[]) time.findAllByColumn("member_id", M.getID());
			...
			try {
				LoginTable[] login = (LoginTable[]) ((LoginTable) IDOLookup.instanciateEntity(LoginTable.class)).findAllByColumn("member_id", this.getID());
			...
			try {
				Member m = ((MemberHome) IDOLookup.getHomeLegacy(Member.class)).findByPrimaryKey(M.getID());
				m.delete();
			}
