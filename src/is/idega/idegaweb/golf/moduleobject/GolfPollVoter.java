...
*/
	...
	public void handleInsert(IWContext iwc)throws SQLException,IOException{
...

		{
			...
			Poll_option[] spurn_id = (Poll_option[]) (((is.idega.idegaweb.golf.entity.Poll_optionHome)com.idega.data.IDOLookup.getHomeLegacy(Poll_option.class)).createLegacy()).findAllByColumn("poll_id",poll_id);
	...
	//		add(Integer.toString(spurn_id.length));
			...
			for (int i = 0 ; i< spurn_id.length ; i++) {
				...
				Poll_result[] usedIPAddresses = (Poll_result[]) (((is.idega.idegaweb.golf.entity.Poll_resultHome)com.idega.data.IDOLookup.getHomeLegacy(Poll_result.class)).createLegacy()).findAllByColumn("poll_user_address",userIPAddress,"poll_option_id",Integer.toString(spurn_id[i].getID()));
	...
	public void main(IWContext iwc)throws SQLException,IOException{
		...
		Poll[] poll1 = (Poll[]) (((is.idega.idegaweb.golf.entity.PollHome)com.idega.data.IDOLookup.getHomeLegacy(Poll.class)).createLegacy()).findAllByColumn("union_id",union_id);
		...
		if (this.poll == null){
			...
			poll = ((is.idega.idegaweb.golf.entity.PollHome)com.idega.data.IDOLookup.getHomeLegacy(Poll.class)).findByPrimaryKeyLegacy(poll_id);
