 ...
 * @author laddi
 */
public class HandicapTable extends GolfBlock {
	...
	public void getAllTables(IWContext modinfo, int field_id) {
		...
		try {
			...
			getParentPage().setTitle(field.getName() + " - " + iwrb.getLocalizedString("handicap.handicap_tables", "Handicap tables"));

			TeeColor[] teeGender = (TeeColor[]) ((TeeColor) IDOLookup.instanciateEntity(TeeColor.class)).findAll("select distinct gender from tee, tee_color where tee.tee_color_id = tee_color.tee_color_id and field_id = " + field_id + " order by gender desc");
			...
			for (int a = 0; a < teeGender.length; a++) {
				...
				Tee[] tee = (Tee[]) ((Tee) IDOLookup.instanciateEntity(Tee.class)).findAll("select distinct tee_color.tee_color_id,slope,course_rating from tee, tee_color where tee.tee_color_id = tee_color.tee_color_id and gender = '" + teeGender[a].getGender() + "' and field_id = " + field_id);
				...
				for (int b = 0; b < tee.length; b++) {
					...
					String teeName = ((TeeColorHome) IDOLookup.getHomeLegacy(TeeColor.class)).findByPrimaryKey(tee[b].getTeeColorID()).getName();
					...
					Table teeTable = getTeeTable(modinfo, (double) slope, (double) CR, fieldPar);
	...
	public Table getTeeTable(IWContext modinfo, double slope, double course_rating, int par) {
		IWResourceBundle iwrb = getResourceBundle();
		...
		Text grunnText = new Text(iwrb.getLocalizedString("handicap.handicap", "Handicap"));
		grunnText.setFontSize(1);
		grunnText.setBold();
		grunnText.setFontFace("Verdana,Arial,sans-serif");
		Text leikText = new Text(iwrb.getLocalizedString("handicap.course_handicap", "Course"));
		leikText.addBreak();
		leikText.addToText(iwrb.getLocalizedString("handicap.handicap_lowercase", "handicap"));
