...
public class RegisterTime extends GolfBlock {
	...
	public RegisterTime() {
		...
		unionDropdown = (DropdownMenu) GolfCacher.getUnionAbbreviationDropdown(
				"club").clone();
	...
	public DropdownMenu insertUnionDropdown(String name, String text, int size)
			throws SQLException {
	...
	private SubmitButton insertButton(String btnName, String Action,
			String Method, Form theForm) {
	...
	private SubmitButton insertButton(Image image, String imageName,
			String Action, String Method, Form theForm) {
	...
	private SubmitButton insertButton(String btnName, String Action,
			String Method, String onSub, Form theForm) {
		SubmitButton mySubmit = new SubmitButton(btnName);
	...
	public void lineUpTable(int skraMarga, IWContext modinfo)
			throws IOException {

		int memberId = -1;
		boolean memberAvailable = false;
		// f· member id fyrir member til a finna hann og setja inn Ì
		// textinputi fyrir hann
		if (modinfo.getSession().getAttribute("member_id") != null) {
			memberId = Integer.parseInt((String) modinfo.getSession()
		...
		try {
			...
			String MemberId = modinfo.getSession().getAttribute("member_id")
					.toString();
			GolfField myGolfField = getFieldInfo(Integer.parseInt(FieldID),
					Date);
			int Line = Integer.parseInt(modinfo.getParameter("line"));
			int max = business.countEntriesInGroup(Line, this.currentField,
					this.currentDay);
			...
			for (int j = 0; j < skraMarga; j++) {
				if (max > 3) {
					while (max > 3) {
						Line++;
						max = business.countEntriesInGroup(Line,
								this.currentField, this.currentDay);
					}
				}
				max++;
			...
			myTable.add(
					getSmallText(localize("start.social_nr", "Social nr.")), 3,
					1);
			myTable.add(getSmallText(localize("start.vip_card", "VIP card")),
					5, 1);
			myTable.add(getSmallText(localize("start.card_number",
					"Card number")), 6, 1);
			...
			if (memberAvailable) {
				admin = is.idega.idegaweb.golf.access.AccessControl
						.isAdmin(modinfo);
				clubadmin = is.idega.idegaweb.golf.access.AccessControl
						.isClubAdmin(modinfo);
				clubworker = is.idega.idegaweb.golf.access.AccessControl
						.isClubWorker(modinfo);
			...
			for (; i < skraMarga + 1; i++) {
				...
				myTable.add(new HiddenInput("group_num", Integer
						.toString(groupNums[i - 1])), 2, i + 1);
				myTable.setAlignment(2, i + 1, "left");

				if (i == 1 && memberAvailable) {
					myTable.add(insertEditBox("secure_num", member
							.getSocialSecurityNumber()), 3, i + 1);
				} else {
			...
			myTable.add(getButton(new SubmitButton(localize("teetime.book",
					"Book"))), 4, i + 2);
			myForm.setMethod("post");
			myTable.add(getButton(new CloseButton(localize("teetime.cancel",
					"Cancel"))), 4, i + 2);
	...
	public void lineUpWMLTable(int skraMarga, IWContext modinfo)
			throws IOException {

		int memberId = -1;
		boolean memberAvailable = true;
		// get member_id for member to find him and put his SSN into the
		// textinput
		...
		try {
			...
			String FieldID = currentField;
			String Date = modinfo.getSession().getAttribute("date").toString();
			String MemberId = String.valueOf(AccessControl.getMember(modinfo)
					.getID());
			GolfField myGolfField = getFieldInfo(Integer.parseInt(FieldID),
					Date);
			int Line = Integer.parseInt(modinfo.getParameter("line"));
			...
			Text dateText = new Text(localize("start.Date", "Date")
					+ ": "
					+ (new IWTimestamp(Date)).getDateString("EEE d MMM",
							modinfo.getCurrentLocale()));
			Text fieldText = new Text(localize("start.Field", "Field")
					+ ": "
					+ getFieldName(Integer.parseInt(modinfo
							.getParameter("field_id"))));
			Text teetimeText = new Text(localize("start.Teetime", "Teetime")
					+ ": " + lines[0]);
			Text numberOfPlayersText = new Text(localize(
					"start.Number_of_players", "Number of players")
					+ ": " + skraMarga);
			...
			String sMessage = localize(
					"start.are_you_sure_you_want_to_reserve_this_teetime",
					"Are you sure you want to reserve this teetime");
			// sMessage += " "+lines[0]+" "+(new
			// IWTimestamp(Date)).getDateString("EEE d MMM",
			// modinfo.getCurrentLocale())+" ";
			// sMessage +=
			// localize("start.at_the_field","at the field")+....;
			sMessage += "?";
			...
			BackButton cancel = new BackButton(localize("start.cancel",
					"Cancel"));
			links.add(new Text(" | "));
			Strong s = new Strong();
			s.add(cancel);
	...
	public String getFieldName(int field_id) throws SQLException, IOException,
			FinderException {
		return service.getFieldName(field_id);
	}

	public void handleFormInfo(IWContext modinfo) throws SQLException,
			IOException {
		...
		if (modinfo.getParameter("secure_num") != null) {
			...
			if (sentSecureNums != null) {
				for (int j = 0; j < sentSecureNums.length; j++) {
					try {
						if (sentSecureNums[j] != null
								&& !"".equals(sentSecureNums[j])) {
							boolean ssn = false; // social security number
							if (sentSecureNums[j].length() == 10) {
								try {
									Integer.parseInt(sentSecureNums[j]
											.substring(0, 5));
									Integer.parseInt(sentSecureNums[j]
											.substring(6, 9));
									ssn = true;
							...
							if (ssn) {
								Member tempMemb = (is.idega.idegaweb.golf.entity.Member) MemberBMPBean
										.getMember(sentSecureNums[j]);
								if (tempMemb != null) {
									int otherPlayers = -1;
									do {
										if (business.countEntriesInGroup(
												...
												this.currentField,
												this.currentDay) >= maxCountInGroups) {
											illegal.add(k++, new Integer(j));
											fullGroup = true;
										} else if (business
												.countOwnersEntries(
														Integer
																.parseInt(this.currentMember),
														this.currentField,
														this.currentDay) >= maxPerOwnerPerDay) {
											illegal.add(k++, new Integer(j));
											fullOwnerQuota = true;
												...
												&& business
														.countMembersEntries(
																tempMemb
																		.getID(),
																this.currentField,
																this.currentDay) >= maxPerMemberPerDay) {
											illegal.add(k++, new Integer(j));
											...
											if (playerCard != null
													&& playerCard.length > j) {
												Union union = GolfCacher
														.getCachedUnion(tempMemb
																.getMainUnionID());
												if (union != null) {
													unionAbbr = union
															.getAbbrevation();
												...
												if (otherPlayers > -1) {
													business
															.setStartingtime(
																	...
																	unionAbbr,
																	"", "");
													legal.add(l++, new Integer(
															j));
												} else {
													if (sentSecureNums.length == 1) {
														String howMany = modinfo
																.getParameter("num_of_players");
														if (howMany != null) {
															otherPlayers = Integer
																	.parseInt(howMany);
														}
													}
				...
				if (legal.size() > 0) {
					if (lockedAsWapLayout || modinfo.isClientHandheld()) {
						...
						myText.setText(localize(
								"start.you_have_been_registered",
						...
						Text myText = (Text) templText.clone();
						myText.setText(localize(
								"start.the_following_where_registered",
								"The following where registered:"));
					...
					// frameTable.add(Text.getBreak());
					// frameTable.add(new
					// CloseButton(localize("start.close_window","Close Window")));

				}

				if (illegal.size() > 0) {
					Text myText = (Text) templText.clone();
					myText.setText(localize("start.error_in_following",
							"Error in following:"));
					...
					if (ones) {
						...
						noError.setText(localize(
								"start.all_others_where_registered",
								"All others where registered"));
						p.add(noError);
						frameTable.add(p);
					}
					...
					if (fullGroup) {
						...
						Error.setText(localize("start.group_is_full",
								"Group is full"));// "Holl sem reynt var a› skrá í er fullt ");
						p.add(Error);
						frameTable.add(p);
					}
					...
					if (fullOwnerQuota) {
						...
						ownerQuota.setText(localize("start.ownerquota",
								"Not allowed to register more golfers to day"));// "Hefur ekki réttindi til a› skrá fleiri á ﬂessum velli í dag");
						p.add(ownerQuota);

						Text comment = (Text) templText.clone();
						comment.setText(localize(
								"start.contact_club_to_register",
								"Contact the club to register"));// "Hafi› samband vi› klúbbinn ef skrá á fleiri");
						p.add(comment);
						frameTable.add(p);
						...
						memberQuota
								.setText(localize("start.memberquota",
										"Not allowed to register golfer again in public registration to day"));// "Ekki má skrá sama mann oftar en einu sinni á dag í netskráningu");
						p.add(memberQuota);
						...
						Text comment = (Text) templText.clone();
						comment.setText(localize(
								"start.try_again_or_contact_the_club",
								"Try again or contact the club"));// "Reyni› aftur e›a hafi› samband vi› klúbbinn");
						p.add(Text.getBreak());
						p.add(comment);
						frameTable.add(p);
					}
					...
					if (lockedAsWapLayout || modinfo.isClientHandheld()) {
						if (backPage != -1) {
							Paragraph p = new Paragraph();
							Link link = new Link(localize(
									"start.wml_back_link", "Back to overview"));
							link.setPage(backPage);
							p.add(link);
							frameTable.add(p);
						}
					} else {
						// this.add(new BackButton(new
						// Image("/pics/rastimask/Takkar/Ttilbaka1.gif")));
						frameTable.add(Text.getBreak());
						frameTable.add(Text.getBreak());
						frameTable.add(new CloseButton(localize(
								"start.close_window", "Close Window")));
					}
				} else {
					if (lockedAsWapLayout || modinfo.isClientHandheld()) {
						if (backPage != -1) {
				...
				Text comment = (Text) templText.clone();
				comment.setText(localize("start.no_one_was_registered",
						"No one was registered"));// "Enginn skrá›ist");
				p.add(Text.getBreak());
				p.add(comment);
				...
				if (lockedAsWapLayout || modinfo.isClientHandheld()) {
					if (backPage != -1) {
						p.add(new Break());
						Link link = new Link(localize("start.wml_back_link",
								"Back to overview"));
						link.setPage(backPage);
						frameTable.add(link);
					}
				} else {
					p.add(new CloseButton(localize("start.close_window",
							"Close Window")));
				}
			...
			if (lockedAsWapLayout || modinfo.isClientHandheld()) {
				...
				p.add(getButton(new CloseButton(localize("start.close_window",
						"Close Window"))));
			}
			frameTable.add(p);
		}
	...
	public void setErroResponse(Form myForm, boolean inputErr) {
		...
		if (inputErr) {
			myTable.add(getErrorText(localize(
					"start.you_have_to_register_as_many_as_reserved",
					"You have to register as many as reserved")), 2, 1);// "Nau›synlegt er a› skrá eins marga og teknir voru frá"),
			// 2,
			// 1);
			myTable.add(getButton(new BackButton()), 2, 3);
		} else {
			myTable.add(getErrorText(localize("start.group_is_full",
					"This group is full. Choose another time.")), 2, 1);// "ﬁetta holl er ﬂví mi›ur fullt. Gjör›u svo vel a› velja ﬂér n‡jan tíma"),
			// 2,
			// 1);
			myTable.add(getButton(new CloseButton(localize(
					"start.close_window", "Close Window"))), 2, 3);
		}
	...
	public GolfField getFieldInfo(int field, String date) throws SQLException,
			IOException {
		StartingtimeFieldConfig FieldConfig = business.getFieldConfig(field,
				date);
		GolfField field_info = new GolfField(new IWTimestamp(FieldConfig
				.getOpenTime()).toSQLTimeString(), new IWTimestamp(FieldConfig
				.getCloseTime()).toSQLTimeString(), FieldConfig
				.getMinutesBetweenStart(), field, date, FieldConfig
				.getDaysShown(), FieldConfig.publicRegistration());
		return field_info;
	}
	...
	public void noPermission() {
		Text satyOut = getErrorText(this.localize("start.no_permission",
				"No permission"));
	...
	public void main(IWContext modinfo) throws Exception {
		...
		this.getParentPage().setTitle(
				this.localize("start.register_tee_time", "Register tee time"));
		...
		try {
			...
			try {
				...
				// String field_id =
				// modinfo.getSession().getAttribute("field_id").toString();
				...
				if (date == null) {
					...
					// String field_id =
					// modinfo.getSession().getAttribute("field_id").toString();
					currentField = (String) modinfo.getSession().getAttribute(
							"field_id");
					currentUnion = (String) modinfo.getSession().getAttribute(
							"union_id");
				}
				...
				try {
					currentMember = Integer.toString(AccessControl.getMember(
							modinfo).getID());
				} catch (NullPointerException e) {
					System.out
							.println("RegisterTime: Unable to load currentMember, NullPointerException: "
									+ e.getMessage());
					keepOn = false;
					this.noPermission();
				}
			...
			// if(modinfo.getParameter(saveParameterString+".x") != null ||
			// modinfo.getParameter(saveParameterString) != null){
			...
			if (keepOn) {
				...
				int line = Integer.parseInt(modinfo.getParameter("line"));
				int check = business.countEntriesInGroup(line, currentField,
						currentDay);
				...
				if (check > 3) {
					...
					if (modinfo.getParameter("secure_num") != null) {
						...
						fieldInfo = business.getFieldConfig(Integer
								.parseInt(currentField), currentDay);
