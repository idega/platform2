...
//                             Tournament[] tours = (Tournament[]) new Tournament().findAllByColumnOrdered("union_id",union_id+"","START_TIME");
                             Tournament[] tours = (Tournament[]) new Tournament().findAll("select * from tournament where union_id = "+union_id+" and START_TIME > '2001-12-31' order by START_TIME");
                        ...
                        for (int i = 0; i < unions.length; i++) {
                            ...
                            tours = (Tournament[]) new Tournament().findAll("select * from tournament where union_id = "+unions_id+" and START_TIME > '2001-12-31' order by START_TIME");
                            for (int j = 0; j < tours.length; j++) {
                                menu.addMenuElement(tours[j].getID(),union_abb+"&nbsp;&nbsp;&nbsp;"+tours[j].getName());
                            }
                        }
    ...
    public static List getMembersInTournamentGroup(Tournament tournament, TournamentGroup tourGroup, is.idega.idegaweb.golf.entity.Member[] members_to_check) throws SQLException{
        ...
        for (int i = 0; i < membersToCheck.length; i++) {
            try {
                tournamentGroupMembers = EntityFinder.findAll(new is.idega.idegaweb.golf.entity.Member(),"SELECT member.* from member, tournament_member where member.member_id = tournament_member.member_id and tournament_member.tournament_id = "+tournament.getID()+" AND tournament_member.tournament_group_id = "+tourGroup.getID());
...
//                }
    ...
    public static List getMembersInTournamentList(Tournament tournament) throws SQLException {
        List members = null;
        try {
            members = EntityFinder.findReverseRelated(tournament,new is.idega.idegaweb.golf.entity.Member());
        }
        catch (Exception e) {
    ...
    public static is.idega.idegaweb.golf.entity.Member[] getMembersInTournament(Tournament tournament) throws SQLException {
        ...
        is.idega.idegaweb.golf.entity.Member member = new is.idega.idegaweb.golf.entity.Member();
        try {
    ...
    public static List getTournamentRoundMembersList(int tournament_round_id) throws SQLException {
        List members = null;
        try {
            members = EntityFinder.findAll(new is.idega.idegaweb.golf.entity.TournamentParticipants(),"SELECT * from tournament_round_participants where tournament_round_id = "+tournament_round_id+" ORDER by grup_num");
        }
        catch (Exception ex) {
    ...
    public static is.idega.idegaweb.golf.entity.TournamentRoundParticipants[] getTournamentRoundMembers(int tournament_round_id) throws SQLException {
        is.idega.idegaweb.golf.entity.TournamentRoundParticipants[] members =  (is.idega.idegaweb.golf.entity.TournamentRoundParticipants[]) (new is.idega.idegaweb.golf.entity.TournamentRoundParticipants()).findAll("SELECT * from tournament_round_participants where tournament_round_id = "+tournament_round_id+" ORDER by grup_num");
        return members;
    }
    ...
    public static boolean isMemberRegisteredInTournament(Tournament tournament, is.idega.idegaweb.golf.entity.Member member) throws SQLException {
        boolean returner = false;
        List listi = EntityFinder.findAll(new is.idega.idegaweb.golf.entity.Member(),"Select member.* from member,tournament_member where member.member_id = tournament_member.member_id AND member.member_id = "+member.getID()+" AND tournament_member.tournament_id = "+tournament.getID()+"");
        if (listi != null) {
            returner = true;
        }
    ...
    public static boolean isMemberRegisteredInTournament(Tournament tournament,TournamentRound tourRound,int howManyEachGroup, is.idega.idegaweb.golf.entity.Member member) throws SQLException {
        ...
        Startingtime[] startingtimes = (Startingtime[]) (new Startingtime()).findAll("SELECT startingtime.* FROM STARTINGTIME, tournament_round_STARTINGTIME WHERE tournament_round_startingtime.tournament_round_id = "+tourRound.getID()+" AND tournament_round_startingtime.startingtime_id = startingtime.startingtime_id AND STARTINGTIME_DATE = '"+startStamp.toSQLDateString()+"' AND field_id="+tournament.getFieldId()+" AND member_id = "+member.getID());
    ...
    public static List getMembersInStartingGroup(Tournament tournament, TournamentRound tournamentRound, int startingGroupNumber) {
        ...
        try {
            members = com.idega.data.EntityFinder.findAll(GenericEntity.getStaticInstance("is.idega.idegaweb.golf.entity.Member"),"SELECT member.* FROM member,STARTINGTIME, TOURNAMENT_ROUND_STARTINGTIME WHERE TOURNAMENT_ROUND_STARTINGTIME.tournament_round_id = "+tournamentRound.getID()+" AND TOURNAMENT_ROUND_STARTINGTIME.startingtime_id = startingtime.startingtime_id AND member.member_id = startingtime.member_id AND STARTINGTIME_DATE = '"+SQLDate+"' AND grup_num ="+startingGroupNumber+" AND field_id="+tournament.getFieldId());
        }
    ...
    public static int getNextAvailableStartingGroup(Tournament tournament, TournamentRound tourRound, boolean useEmptyStartingGroup, int minimumGroupNumber) {
        ...
        try {
            ...
            while (!done) {
                ...
                startingtimes = (Startingtime[]) (new Startingtime()).findAll("SELECT * FROM STARTINGTIME s, TOURNAMENT_ROUND_STARTINGTIME trs WHERE trs.startingtime_id = s.startingtime_id AND trs.tournament_round_id = "+tourRound.getID()+" AND s.STARTINGTIME_DATE = '"+startStamp.toSQLDateString()+"'  AND s.field_id="+tournament.getFieldId()+" AND s.grup_num="+counter);
...
public static void createScorecardForMember(is.idega.idegaweb.golf.entity.Member member, Tournament tournament, TournamentTournamentGroup tTGroup) throws SQLException {
  ...
  public static StartingtimeView[] getStartingtimeView(int tournament_round_id, String column_name,String column_value,String column_name_1,String column_value_1,int tee_number,String order) throws SQLException {
    ...
    try {
        startView = (StartingtimeView[]) (new StartingtimeView()).findAll(queryString);
    }
              ...
              if (startingTimes != null) {
                  try {
                      for (int i = 0; i < startingTimes.size(); i++) {
                        ...
                        sTime.removeFrom(Tournament.getStaticInstance("is.idega.idegaweb.golf.entity.Tournament"));
                        sTime.delete();
                      }
                  }
                  catch (Exception ex) {
