...
public class EuroFile  {
  ...
  public void writeFile(IWContext iwc,Payment[] ePayments, String sContract, double dPercent, int iAmount, int iUnionId)throws SQLException,IOException{
    ...
    PaymentType PayType = ((is.idega.idegaweb.golf.entity.PaymentTypeHome)com.idega.data.IDOLookup.getHomeLegacy(PaymentType.class)).findByPrimaryKeyLegacy(2);
    ...
    for(int i = 0; i < ePayments.length ;i++){
      ...
      if(newMemberID == oldMemberID){
        ...
        M = ((is.idega.idegaweb.golf.entity.MemberHome)com.idega.data.IDOLookup.getHomeLegacy(Member.class)).findByPrimaryKeyLegacy(newMemberID);
  ...
  private void euroFiling(IWContext iwc, int roundid, String sContract, double dPercent, int iAmount) throws SQLException,IOException{
    ...
    Payment[] payments = (Payment[]) (((is.idega.idegaweb.golf.entity.PaymentHome)com.idega.data.IDOLookup.getHomeLegacy(Payment.class)).createLegacy()).findAll("select * from payment where round_id = '"+prID+"' and status = 'N' and payment_type_id = '"+euroID+"' order by member_id ");
