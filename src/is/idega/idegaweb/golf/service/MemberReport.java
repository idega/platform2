...
public class MemberReport
	...
	private String[] sSelectTables =
		{
			...
			"union_member_info",
			"union_member_info" };
	private String[] sSelectJoin =
		{
			"",
			"",
			"",
			"address.address_id = member_address.address_id and member.member_id = member_address.member_id",
			"phone.phone_id = member_phone.phone_id and member.member_id = member_phone.member_id",
			"member.member_id = member_info.member_id",
			"member.member_id = account.member_id",
			"member.member_id = union_member_info.member_id ",
			"member.member_id = union_member_info.member_id " };
	private final int CH1 = 0,
		CH2 = 1,
		CH3 = 2,
		CH4 = 3,
		CH5 = 4,
	...
	private void control(IWContext iwc) {
		try {
			if (iwc.getSession().getAttribute("golf_union_id") != null) {
				sUnionId =
					(String) iwc.getSession().getAttribute("golf_union_id");
			}
	...
	private void doMain(IWContext iwc) throws SQLException {
		...
		SelectionDoubleBox box =
			new SelectionDoubleBox(boxName, "Reitir", "Val/Röðun");
		T.add(box, 1, 1);
		T.add(T2, 4, 1);
		form.add(T);
	...
	private void doUpdate(IWContext iwc) throws SQLException {
		...
		if (selectedValues != null) {
			...
			int iHndCpfrom =
				Integer.parseInt(iwc.getParameter("list_hndcpfrom"));
			...
			if (iGroup != 0) {
				...
				if (!vJoin
					.contains("member.member_id = group_member.member_id"))
					vJoin.addElement(
						"member.member_id = group_member.member_id");
			...
			if (!sStatus.equalsIgnoreCase("0")) {
				...
				vWhere.addElement(
					" union_member_info.member_status = '" + sStatus + "' ");
			}
			...
			if (iHndCpfrom != 0) {
				...
				vWhere.addElement(
					" member_info.handicap >= '" + iHndCpfrom + "'");
			}
			if (iHndCpto != 0) {
				if (!vTables.contains(sSelectTables[CH6])) {
					vTables.addElement(sSelectTables[CH6]);
	...
	private void doTable(IWContext iwc) {
		if (iwc.getSession().getAttribute("mbsvector") != null) {
			...
			if (iwc.getSession().getAttribute("lastorder") != null)
				this.sLastOrder =
					(String) iwc.getSession().getAttribute("lastorder");
			else
				this.sLastOrder = "";
	...
	private void OrderVector(Vector mbs, int order, boolean reverse) {
		...
		switch (order) {
			case F :
				sortint = ReportMemberComparator.NAME;
				break;
			case S :
				sortint = ReportMemberComparator.SOCIAL;
				break;
			case E :
				sortint = ReportMemberComparator.EMAIL;
				break;
			case G :
				sortint = ReportMemberComparator.ADDRESS;
				break;
			case H :
				sortint = ReportMemberComparator.HANDICAP;
				break;
			case B :
				sortint = ReportMemberComparator.BALANCE;
				break;
			case C :
				sortint = ReportMemberComparator.LOCKER;
				break;
			case U :
				sortint = ReportMemberComparator.STATUS;
				break;
		}
	...
	private String getReportMemberString(ReportMember rm, int col) {
		...
		switch (colnr) {
			...
			case G :
				...
				break;
			case H :
				rs = rm.getHandicap().toString();
				break;
			case B :
				rs = String.valueOf(rm.getBalance());
				break;
			case C :
				rs = rm.getLocker();
				break;
			case U :
				rs = getStatus(rm.getStatus());
				break;
		}
		return rs != null ? rs : "";
	}
		...
		try {
			...
			while (RS.next()) {
				...
				for (int i = 0; i < columnCount; i++) {
					...
					switch (colnr) {
						case F :
							f = RS.getString(sCols[F]);
							break;
						case M :
							m = RS.getString(sCols[M]);
							break;
						case L :
							l = RS.getString(sCols[L]);
							break;
						case S :
							s = RS.getString(sCols[S]);
							break;
						case E :
							e = RS.getString(sCols[E]);
							break;
						case G :
							g = RS.getString(sCols[G]);
							break;
						case N :
							n = RS.getString(sCols[N]);
							break;
						case H :
							h = RS.getFloat(sCols[H]);
							break;
						case B :
							b = RS.getInt(sCols[B]);
							break;
						case C :
							c = RS.getString(sCols[C]);
							break;
						case U :
							u = RS.getString(sCols[U]);
							break;
					}
				}
	...
	private DropdownMenu drpInt(String sPrm, String init, int count)
		throws SQLException {
		DropdownMenu drp = new DropdownMenu(sPrm);
	...
	private DropdownMenu drpZip() throws SQLException {
		ZipCode[] zips =
			(ZipCode[]) com
				...
				.data
				.GenericEntity
				.getStaticInstance(ZipCode.class)
				.findAllOrdered("code");
