...
public class TariffService  {
  ...
  static public Account[] getAccounts(int iMemberId, int iUnionId){
    try{
      Account[] A = (Account[])new Account().findAllByColumn("member_id",String.valueOf(iMemberId),"union_id",String.valueOf(iUnionId));
      return A;
    }
    catch(SQLException sql){
  ...
  static public Account findAccount(int member_id, int union_id,int accountYearId){
    try{
      ...
      Account[] A = (Account[]) (new Account()).findAllByColumn(
  ...
  static public Payment[] getMemberPayments(int iMemberId,int iUnionId){
     Payment[] P;
    try{
      P = (Payment[]) new Payment().findAllByColumnOrdered("member_id",String.valueOf(iMemberId),"union_id",String.valueOf(iUnionId),"last_updated");
    }
    catch(SQLException e){
      P = new Payment[0];
    }
   ...
   static public Payment[] getMemberPayments(int iAccountId,int iUnionId,idegaTimestamp from,idegaTimestamp to){
    ...
    try{
      P = (Payment[]) new Payment().findAll(sql.toString());
    }
    catch(SQLException e){
      P = new Payment[0];
    }
  ...
  static public AccountEntry[] getAccountEntrys(int iAccountId){
    AccountEntry[] E;
    try{
      E = (AccountEntry[]) new AccountEntry().findAllByColumnOrdered("account_id",String.valueOf(iAccountId),"last_updated");
    }
    catch(SQLException e){
  ...
  static public AccountEntry[] getAccountEntrys(int iAccountId,idegaTimestamp from,idegaTimestamp to){
    ...
    //System.err.println(sql.toString());
    try{
      E = (AccountEntry[]) new AccountEntry().findAll(sql.toString());
    }
    catch(SQLException e){
      E = new AccountEntry[0];
    }
  ...
  static public AccountEntry[] getTariffEntrys(int iAccountId){
     AccountEntry[] E;
    try{
      E = (AccountEntry[]) new AccountEntry().findAllByColumnOrdered("account_id",String.valueOf(iAccountId),"info","Álagning","last_updated");
    }
    catch(SQLException e){
      E = new AccountEntry[0];
    }
  ...
  static public List getMemberFamily(int iMemberId,int iUnionId){
    try{
      UnionMemberInfo umi = new Member(iMemberId).getUnionMemberInfo(iUnionId);
      List M = EntityFinder.findAll(new Member(),getFamilySql(iUnionId,umi.getFamilyId()));
      return M;
    }
    catch(SQLException sql){
      sql.printStackTrace();
