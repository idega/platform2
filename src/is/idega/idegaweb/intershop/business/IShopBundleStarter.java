/*
 *  $Id: IShopBundleStarter.java,v 1.2 2002/04/03 12:41:53 palli Exp $
 *
...
public class IShopBundleStarter implements IWBundleStartable {
  ...
  private void insertInitialData(String pathToPropertiesFile) {
    ...
    int folderId = IBPageHelper.getInstance().createNewPage(parent,"Intershop folder",IBPage.PAGE,"",null);
    ...
    try {
      ...
      sql.append(", ");
      sql.append(IShopTemplateBean.getIShopDescriptionColumnName());
      sql.append(", ");
      sql.append(IShopTemplateBean.getIShopNameColumnName());
      sql.append(" from ");
      sql.append(dbname);
      sql.append("..");
      ...
      while (set.next()) {
        String className = set.getString(IShopTemplateBean.getIShopClassColumnName());
        String id = set.getString(IShopTemplateBean.getIShopIDColumnName());
        String langnr = set.getString(IShopTemplateBean.getIShopLanguageNrColumnName());
        String name = set.getString(IShopTemplateBean.getIShopNameColumnName());
        String desc = set.getString(IShopTemplateBean.getIShopDescriptionColumnName());
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        int pageId = IBPageHelper.getInstance().createNewPage(Integer.toString(folderId),name,IBPage.PAGE,"",null,null,IShopTemplate.SUBTYPE_NAME);
