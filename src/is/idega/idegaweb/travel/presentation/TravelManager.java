...
public class TravelManager extends TravelBlock {
	...
	public void draw(IWContext iwc)  throws RemoteException{
		...
		if (isTravelAdministrator(iwc)){
			...
			Link lCreditCard = new Link(tsm.getIWResourceBundle().getLocalizedImageButton("creditcard", "Creditcard"), CreditCardMerchantEditor.class);
			table.add(lCreditCard, 1, 1);
...
//			if (super.getTravelSessionManager(iwc).hasRole(TradeConstants.ROLE_CREATE_EDIT_PRODUCTS)) {
