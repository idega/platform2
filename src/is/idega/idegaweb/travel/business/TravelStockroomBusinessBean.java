...
//					product.addTo(timeframe);
			...
			//transaction.commit();
			invalidateServiceCache(Integer.toString(id));
			return id;
	...
	public boolean invalidateServiceCache(String serviceID) {
		return invalidateServiceCache(serviceID, null);
	}

	public boolean invalidateServiceCache(String serviceID, String remoteDomainToExclude) {
		serviceMap.remove(serviceID);
		executeRemoteService(remoteDomainToExclude, "invalidateServiceCache&serviceID="+serviceID);
		return true;
	}
	
	public Service getService(Product product) throws ServiceNotFoundException, RemoteException {
		Service service = (Service) serviceMap.get(product.getPrimaryKey().toString());
		if (service == null) {
			try {
				...
				serviceMap.put(product.getPrimaryKey().toString(), service);
			}
...
//					System.err.println("sraneson");
	...
	public void removeResellerHashtables(IWContext iwc) {
		removeResellerHashtables(iwc, null);
	}
	public boolean removeResellerHashtables(IWContext iwc, String remoteDomainToExclude) {
		...
		executeRemoteService(remoteDomainToExclude, "removeResellerHashtables");
		return true;
	}
	...
	public boolean removeServiceDayHashtable(IWContext iwc, String remoteDomainToExclude) {
		...
		executeRemoteService(remoteDomainToExclude, "removeServiceDayHashtable");
		return true;
	}
	
	public void removeServiceDayHashtable(IWContext iwc) {
		removeServiceDayHashtable(iwc, null);
	}
...
//				System.out.println("Found max for product = "+product.getPrimaryKey()+" stamp = "+stmpString+" = MAX = "+returner.intValue());
	...
	public boolean invalidateMaxDayCache(Product product, String remoteDomainToExclude) throws RemoteException {
		Vector v = new Vector();
		v.add(product);
		invalidateMaxDayCache(v);
		getStockroomBusiness().executeRemoteService(remoteDomainToExclude, "invalidateMaxDayCache&productID="+product.getPrimaryKey().toString());
		return true;
	}
	...
	public void executeRemoteService(String remoteDomainToExclude, String methodQuery) {
		executeRemoteService(remoteDomainToExclude, methodQuery, "/idegaweb/bundles/is.idega.travel.bundle/resources/services/IWTravelWS.jws");
	}
