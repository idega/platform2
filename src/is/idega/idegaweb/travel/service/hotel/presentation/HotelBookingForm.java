...
//	      table.add(new HiddenInput(this.sAction, "Test jamms"),2,row);
  ...
  public int checkBooking(IWContext iwc, boolean saveBookingIfValid, boolean bookIfTooMany, boolean bookIfTooFew) throws Exception {
    ...
    if (tFrame == null) {
      pPrices = getProductPriceHome().findProductPrices(_service.getID(), -1, -1, onlineOnly, key);
    }else {
      pPrices = getProductPriceHome().findProductPrices(_service.getID(), tFrame.getID(), -1, onlineOnly, key);
    }
