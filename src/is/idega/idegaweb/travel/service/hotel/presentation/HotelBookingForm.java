    ...
    //              pPrices[i].getPrice();
                pPriceCatNameText = (Text) theText.clone();
	              ...
	              if (pricesLength == 1 && i < pricesLength) {
	                pPriceText = new ResultOutput("thePrice"+((Integer) pPrices[i].getPrimaryKey()).intValue(),Integer.toString(price));
	                pPriceText.setSize(8);
	                pPriceMany = new TextInput("priceCategory"+((Integer) pPrices[i].getPrimaryKey()).intValue() ,"1");
...
*/
      ...
      if (inquiry == null) {
        Text bookingsError = getBoldText(iwrb.getLocalizedString("travel.the_following_days_are_not_available","The following days are not available"));
...
//	      table.add(new HiddenInput(this.sAction, "Test jamms"),2,row);
  ...
  public int checkBooking(IWContext iwc, boolean saveBookingIfValid, boolean bookIfTooMany, boolean bookIfTooFew, boolean doCreditCardCheck) throws Exception {
    ...
    Collection pPrices = null;
    if (tFrame == null) {
      pPrices = getProductPriceBusiness().getProductPrices(_service.getID(), -1, -1, onlineOnly, key, _stamp);
    }else {
      pPrices = getProductPriceBusiness().getProductPrices(_service.getID(), tFrame.getID(), -1, onlineOnly, key, _stamp);
    }
...
//		  return (currentBookings >= max);
	...
	protected int addPublicFromDateInput(IWContext iwc, Table table, int fRow) {
		table.add(getSmallText(iwrb.getLocalizedString("travel.check_in", "Check in")), 1, fRow);
